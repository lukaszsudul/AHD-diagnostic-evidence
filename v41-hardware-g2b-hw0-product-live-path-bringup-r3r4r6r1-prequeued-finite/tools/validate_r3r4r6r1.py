#!/usr/bin/env python3
"""Disk-based R3R4R6R1 integrity, continuity, source, DMA and frame validator."""
...
def run(args: argparse.Namespace) -> dict:
    ...
    health = json.loads(Path(args.health_result).read_text())
    ...
    if primary_size != len(primary):
        raise RuntimeError("R3R4R6R1_PRIMARY_REREAD_SIZE_CHANGED")
    ...
    guard_records_path = private / "guard-records.bin"
    guard_partial_path = private / "guard-partial.bin"
    expected_guard_full_bytes = guard_returned - (guard_returned % RECORD_BYTES)
    expected_guard_partial_bytes = guard_returned % RECORD_BYTES
    capture_file_contract_pass = (
        primary_size == primary_returned and guard_size == guard_returned and
        guard_records_path.stat().st_size == expected_guard_full_bytes and
        guard_partial_path.stat().st_size == expected_guard_partial_bytes)
    host_returned_bytes = primary_returned + guard_returned
    host_complete_records = host_returned_bytes // RECORD_BYTES
    host_complete_record_bytes = host_complete_records * RECORD_BYTES
    host_partial_bytes = host_returned_bytes % RECORD_BYTES
    host_complete_record_beats = host_complete_record_bytes // 8
    host_partial_beats = (host_partial_bytes + 7) // 8
    ...
    unreaped_dma_beats = ((max(unreaped_dma_bytes, 0) + 7) // 8
                          if unreaped_dma_bytes >= 0 else None)
    parent_quiescence_pass = (
        controller.get("parent_hardware_quiescence", {}).get("result") == "PASS"
        and controller.get("final_physical_state", {}).get("dma_quiescent") is True)
    health_pass = health.get("result") == "PASS"
    if unreaped_dma_bytes == 0:
        dma_accounting = "CLEAN"
    elif (guard_result == -errno.ETIMEDOUT and
          primary_result == PRIMARY_BYTES and
          0 < unreaped_dma_bytes <= GUARD_BYTES and
          int(deltas["overflow"]) == 0 and parent_quiescence_pass and
          health_pass):
        dma_accounting = "BOUNDED_GUARD_TIMEOUT_REMAINDER"
    else:
        dma_accounting = "FAIL"

    if guard_result == GUARD_BYTES:
        guard_outcome = "COMPLETE"
    elif guard_result >= 0:
        guard_outcome = "PARTIAL"
    elif guard_result == -errno.ETIMEDOUT:
        guard_outcome = "ETIMEDOUT_NO_BYTE_COUNT"
    else:
        guard_outcome = "FAIL"
    ...
    write_counts = controller.get("mmio_write_counts", {})
    prequeue = controller.get("prequeue_ready") or {}
    prequeue_execution = (
        "PASS" if (prequeue.get("primary_submit_result") == 1 and
                   prequeue.get("guard_submit_result") == 1 and
                   prequeue.get("primary_bytes") == PRIMARY_BYTES and
                   prequeue.get("guard_bytes") == GUARD_BYTES and
                   float(controller.get(
                       "prequeue_ready_to_enable_latency_us", -1)) >= 0 and
                   write_counts.get("STREAM_ENABLE") == 1)
        else "FAIL")
    overflow_delta = int(deltas["overflow"])
    dropped_delta = int(deltas["dropped"])
    non_overflow_drop_delta = dropped_delta - overflow_delta
    host_prequeue_hypothesis = (
        "CONFIRMED" if (primary_result == PRIMARY_BYTES and
                        overflow_delta == 0 and not overflow_indices)
        else "NOT_CONFIRMED")
    stream_disabled = (
        controller.get("final_physical_state", {}).get("stream_disabled") is True
        and write_counts.get("NORMAL_DISABLE") == 1)
    finite_prequeued = (
        "PASS" if (primary_result == PRIMARY_BYTES and
                   record_count == PRIMARY_RECORDS and integrity_failures == 0 and
                   prequeue_execution == "PASS" and stream_disabled and
                   parent_quiescence_pass and
                   capture_file_contract_pass and
                   dma_accounting in ("CLEAN",
                                      "BOUNDED_GUARD_TIMEOUT_REMAINDER"))
        else "FAIL")
    ...
    s3_fatal_bits = s3_error & 0x38
    s3_overflow_bit = bool(s3_error & 0x01)
    s3_drop_bit = bool(s3_error & 0x02)
    s3_discontinuity_bit = bool(s3_error & 0x04)
    s3_status_mapped = (
        s3_fatal_bits == 0 and
        (not s3_overflow_bit or overflow_delta > 0) and
        (not s3_drop_bit or dropped_delta > 0) and
        (not s3_discontinuity_bit or
         int(deltas["discontinuity"]) > 0 or bool(discontinuity_indices)))
    diagnostic_open = (
        source_classification == "OPEN_RECURRING_BT656_QUALIFICATION_EVENT" or
        non_overflow_drop_delta > 0)
    base_success = (
        record_path_integrity == "PASS" and finite_prequeued == "PASS" and
        prequeue_execution == "PASS" and
        host_prequeue_hypothesis == "CONFIRMED" and
        stream_continuity == "PASS" and complete_frame == "PASS" and
        non_overflow_drop_delta >= 0 and s3_status_mapped and health_pass and
        int(controller.get("unauthorized_mmio_writes", 0)) == 0)
    if not base_success:
        overall = "FAIL"
    elif diagnostic_open and dma_accounting == "BOUNDED_GUARD_TIMEOUT_REMAINDER":
        overall = (
            "PASS_WITH_BT656_DIAGNOSTIC_OPEN_AND_"
            "BOUNDED_GUARD_TIMEOUT_REMAINDER")
    elif diagnostic_open:
        overall = "PASS_WITH_BT656_DIAGNOSTIC_OPEN"
    elif dropped_delta != 0:
        overall = "FAIL"
    elif dma_accounting == "BOUNDED_GUARD_TIMEOUT_REMAINDER":
        overall = "PASS_WITH_BOUNDED_SHUTDOWN_REMAINDER"
    else:
        overall = "PASS"

    result = {
        "schema": "R3R4R6R1_VALIDATION_RESULT_V1", "task": TASK,
        "result": overall,
        ...
        "guard_full_record_bytes": guard_records_path.stat().st_size,
        "guard_full_record_sha256": sha256_file(guard_records_path),
        "guard_partial_sha256": sha256_file(guard_partial_path),
        "combined_returned_byte_sha256": sha256_files((primary_path, guard_path)),
        "capture_file_contract": "PASS" if capture_file_contract_pass else "FAIL",
        "guard_aio_outcome": guard_outcome,
        "guard_aio_result": guard_result,
        "guard_full_records": guard_returned // RECORD_BYTES,
        "guard_partial_bytes": guard_returned % RECORD_BYTES,
        ...
        "frame": frame_result,
        "native_helper_build": "PASS",
        "prequeue_execution": prequeue_execution,
        "host_prequeue_hypothesis": host_prequeue_hypothesis,
        ...
        "host_dma_accounting": {
            ...
            "host_complete_record_beats": host_complete_record_beats,
            "host_partial_bytes": host_partial_bytes,
            "host_partial_beats": host_partial_beats,
            ...
            "equation": (
                "FPGA_STREAMED_BYTES = HOST_RETURNED_BYTES + "
                "UNREAPED_DMA_BYTES"),
        },
        "counter_deltas": deltas,
        "ring_overflow_drop_delta": overflow_delta,
        "non_overflow_drop_delta": non_overflow_drop_delta,
        "s3_error_status": s3_error,
        "s3_last_error_cause": controller["S3"]["LastErrorCause"],
        "s3_error_mapping": {
            "overflow_bit": s3_overflow_bit,
            "drop_bit": s3_drop_bit,
            "discontinuity_bit": s3_discontinuity_bit,
            "fatal_bits": s3_fatal_bits,
            "mapped_to_observations": s3_status_mapped,
        },
        "health": health,
    }
