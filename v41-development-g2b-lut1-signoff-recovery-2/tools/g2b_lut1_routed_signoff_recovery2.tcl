# AHD v41 G2B-LUT1 governed routed-DCP sign-off recovery 2.
#
...
# without retired Groups 9 and 13 plus the byte-exact promoted BS3 and G13-A
# candidates. Groups 9-12 are validated solely from hash-bound preserved
# evidence; Groups 13-17 are validated solely from externally bounded gates.
# No Group-9 timing query and no Group-9/Group-13 report_bus_skew is issued.
# It performs
...
proc validate_source_identity {} {
  ...
  set source_status [git_value $repo_root status --porcelain=v1 --untracked-files=all]
  foreach row [split $source_status "\n"] {
    if {$row eq "" || [string first "?? " $row] == 0} { continue }
    error "governed source has a tracked/index modification: [safe_value $row]"
  }
...
proc validate_governed_static_inputs {} {
  global checkpoint base_xdc bs3_candidate_xdc g13_candidate_xdc active_xdc xpr_path xdma_xci
  global structural_proof request_ack_proof cdc_invariants
  global expected_checkpoint_sha expected_base_xdc_sha expected_bs3_candidate_xdc_sha
  global expected_g13_candidate_xdc_sha
  global expected_active_xdc_sha expected_xpr_sha expected_xdma_xci_sha
  global evidence_repo ssot_manifest meta5_manifest g13a_manifest

  foreach {path expected label} [list \
      $checkpoint $expected_checkpoint_sha {sealed routed DCP} \
      $base_xdc $expected_base_xdc_sha {full base without Groups 9 and 13} \
      $bs3_candidate_xdc $expected_bs3_candidate_xdc_sha {BS3 candidate} \
      $g13_candidate_xdc $expected_g13_candidate_xdc_sha {G13-A candidate} \
      ...
      $ssot_manifest 6515CDC8553103D08CCFE7EE57301154198CBD4A8A24D62C10D6EA36F328CC6B {SSOT revision-5 manifest} \
      $meta5_manifest 9DE9B1F3513B3F3F5960681D5F528C6D624F8017764955F0F406444050B5AD0F {META-5 manifest} \
      $g13a_manifest EF1CDA631A6B38CF4C002BF25F7CB6B2A6A878C906C6D7FB41F23EE6114B1A0D {G13-A manifest} \
  ...
  if {[git_value $evidence_repo cat-file -t bbdeb474ce9d7e5f0db3e8ca8afb5448eef8f314] ne "commit" ||
      [git_value $evidence_repo rev-parse {bbdeb474ce9d7e5f0db3e8ca8afb5448eef8f314:v41-meta-project-state-rev5-group13-reset-return-signoff}] ne "11a760c3af3a7e3db67c9961c138169c4ad79cdb"} {
    error "META-5 evidence commit or directory tree identity drift"
  }
  if {[git_value $evidence_repo cat-file -t 10c7c2898d162af8e2262b3f99861c7d560c4557] ne "commit" ||
      [git_value $evidence_repo rev-parse {10c7c2898d162af8e2262b3f99861c7d560c4557:v41-development-g2b-g13a-reset-return-signoff-audit}] ne "d4694977a5bfecfec8005d9cc0dd1c1c44f36f7f"} {
    error "G13-A evidence commit or directory tree identity drift"
  }
  if {[count_xdc_command_lines [read_text $base_xdc] set_bus_skew] != 15 ||
      [count_xdc_command_lines [read_text $base_xdc] set_max_delay] != 9} {
    error "preserved base XDC constraint census drift"
  }
  if {[count_xdc_command_lines [read_text $bs3_candidate_xdc] set_bus_skew] != 0 ||
      [count_xdc_command_lines [read_text $bs3_candidate_xdc] set_max_delay] != 3} {
    error "BS3 candidate constraint census drift"
  }
  if {[count_xdc_command_lines [read_text $g13_candidate_xdc] set_bus_skew] != 0 ||
      [count_xdc_command_lines [read_text $g13_candidate_xdc] set_max_delay] != 2} {
    error "G13-A candidate constraint census drift"
  }
  if {[string first [read_text $bs3_candidate_xdc] [read_text $active_xdc]] < 0} {
    error "byte-exact BS3 candidate is not embedded in active XDC"
  }
  if {[string first [read_text $g13_candidate_xdc] [read_text $active_xdc]] < 0} {
    error "byte-exact G13-A candidate is not embedded in active XDC"
  }
...
proc validate_preserved_results {} {
  global preserved_gate expected_preserved_gate_sha prior_evidence_root
  if {[sha256_file $preserved_gate] ne $expected_preserved_gate_sha} {
    error "preserved Groups 9-12 receipt identity drift"
  }
  set values [parse_kv $preserved_gate]
  require_kv $values RESULT PASS
  require_kv $values PUBLISHED_EVIDENCE_COMMIT 765f5a5d4760f7a685447651dc68179b2fd96846
  require_kv $values GROUP9_RESULT PRESERVED_PASS
  require_kv $values GROUP9_GATE_SHA256 4531FF587FEE60FA99DC6523C7F11E6585945390D1FE5AB66CA4B30E040BB25B
  require_kv $values GROUP9_RESULTS_CSV_SHA256 66D28113BD52EC0C32247FAAC5CC73B35E729B27F1F2E7A644643601F4B3638C
  require_kv $values GROUP9_GLOBAL_REPORT_BUS_SKEW_EXECUTED NO
  require_kv $values GROUPS10_12_RESULT PRESERVED_PASS
  require_kv $values GROUPS10_17_PRIOR_LEDGER_SHA256 666A403FC01DADE0E95D3329D119473CF8D7D0E7EAD2B1E624F495FDAB5FFFF7
  require_kv $values GROUP9_RERUN NO
  require_kv $values GROUPS10_12_RERUN NO
  require_kv $values CONSISTENCY_INVALIDATION_REASON NONE
  require_kv $values HARDWARE_ACCESSED NO
  set prior_group9_gate [file join $prior_evidence_root G2B_LUT1_GROUP9_GATE.txt]
  set prior_group9_csv [file join $prior_evidence_root G2B_LUT1_GROUP9_SIGNOFF_RESULTS.csv]
  set prior_ledger [file join $prior_evidence_root G2B_LUT1_GROUPS10_17_RESULTS.csv]
  foreach {path expected label} [list \
      $prior_group9_gate 4531FF587FEE60FA99DC6523C7F11E6585945390D1FE5AB66CA4B30E040BB25B {published Group-9 gate} \
      $prior_group9_csv 66D28113BD52EC0C32247FAAC5CC73B35E729B27F1F2E7A644643601F4B3638C {published Group-9 CSV} \
      $prior_ledger 666A403FC01DADE0E95D3329D119473CF8D7D0E7EAD2B1E624F495FDAB5FFFF7 {published Groups 10-17 ledger}] {
    if {[sha256_file $path] ne $expected} { error "$label identity drift" }
  }
  return [sha256_file $preserved_gate]
}

proc validate_group13_gate {} {
  global evidence_root g13a_root group13_gate expected_group13_gate_sha group13_csv
  global expected_source_branch expected_source_commit expected_source_tree expected_active_xdc_sha
  global expected_checkpoint_sha expected_base_xdc_sha expected_bs3_candidate_xdc_sha
  global expected_g13_candidate_xdc_sha
  if {[sha256_file $group13_gate] ne $expected_group13_gate_sha} {
    error "Group-13 gate does not match caller-provided SHA-256"
  }
  set values [parse_kv $group13_gate]
  require_kv $values GROUP13_REPLACEMENT_SIGNOFF PASS
  require_kv $values GROUP13_METHOD SETTLING_PLUS_STRUCTURAL_CDC
  require_kv $values SEMANTIC_FAMILY_COUNT 2
  require_kv $values FAMILY_1 RESET_ABANDONED_COUNT_STABLE_PAYLOAD
  require_kv $values FAMILY_1_RESULT PASS
  require_kv $values FAMILY_2 RESET_COMMIT_PHASE_COMPLETION_BARRIER
  require_kv $values FAMILY_2_RESULT PASS
  require_kv $values SUPPLEMENTAL_AGGREGATE_COVERAGE PASS
  require_kv $values SUPPLEMENTAL_AGGREGATE_IS_THIRD_FAMILY NO
  require_kv $values STRUCTURAL_CDC_PROOF_BINDING PASS
  require_kv $values STABLE_DATA_SEMANTICS PASS
  require_kv $values HANDSHAKE_COMMIT_BARRIER PASS
  require_kv $values RESET_RETURN_COHERENCY PASS
  require_kv $values BOUNDED_SETTLING PASS
  require_kv $values GLOBAL_GROUP13_REPORT_BUS_SKEW_EXECUTED NO
  require_kv $values ATTEMPT_NUMBER 1
  require_kv $values RETRY_EXECUTED NO
  require_kv $values QUERY_TIMEOUT_SECONDS 300
  require_kv $values SOURCE_BRANCH $expected_source_branch
  ...
  require_kv $values ROUTED_DCP_SHA256 $expected_checkpoint_sha
  require_kv $values DERIVED_FULL_BASE_SHA256 $expected_base_xdc_sha
  require_kv $values BS3_CANDIDATE_SHA256 $expected_bs3_candidate_xdc_sha
  require_kv $values G13A_CANDIDATE_SHA256 $expected_g13_candidate_xdc_sha
  set worker_sha [require_hash_value [require_kv $values G13A_WORKER_SHA256] G13A_WORKER_SHA256]
  set runner_sha [require_hash_value [require_kv $values RECOVERY2_RUNNER_SHA256] RECOVERY2_RUNNER_SHA256]
  set worker_path [file join $g13a_root tools G2B_G13A_CANDIDATE_CONTINUATION_WORKER.tcl]
  set runner_path [file join $evidence_root tools Invoke-G2BLut1Group13Recovery2.ps1]
  if {[sha256_file $worker_path] ne $worker_sha || [sha256_file $runner_path] ne $runner_sha} {
    error "Group-13 receipt is not bound to the present governed worker/runner"
  }
  require_kv $values G13A_EVIDENCE_COMMIT 10c7c2898d162af8e2262b3f99861c7d560c4557
  require_kv $values G13A_EVIDENCE_DIRECTORY_TREE d4694977a5bfecfec8005d9cc0dd1c1c44f36f7f
  require_kv $values BITSTREAM_PRODUCED NO
  require_kv $values HARDWARE_ACCESSED NO
  set csv_lines [list]
  foreach line [split [read_text $group13_csv] "\n"] {
    set line [string trimright $line "\r"]
    if {$line ne ""} { lappend csv_lines $line }
  }
  set header [list Family Constraint_Type Required_ns Worst_Actual_ns Slack_ns Result \
    Source_Count Destination_Count Runtime_s Notes]
  if {[llength $csv_lines] != 3 || [parse_csv_row [lindex $csv_lines 0]] ne $header} {
    error "Group-13 results CSV shape/header drift"
  }
  foreach {index family sources destinations} [list \
      1 RESET_ABANDONED_COUNT_STABLE_PAYLOAD 3 32 \
      2 RESET_COMMIT_PHASE_COMPLETION_BARRIER 4 207] {
    set fields [parse_csv_row [lindex $csv_lines $index]]
    if {[llength $fields] != 10 || [lindex $fields 0] ne $family ||
        [lindex $fields 1] ne "SET_MAX_DELAY_DATAPATH_ONLY_PLUS_STRUCTURAL_CDC" ||
        [lindex $fields 2] ne "6.000" || [lindex $fields 5] ne "PASS" ||
        [lindex $fields 6] ne $sources || [lindex $fields 7] ne $destinations ||
        ![string is double -strict [lindex $fields 3]] || [lindex $fields 3] > 6.0005 ||
        ![string is double -strict [lindex $fields 4]] || [lindex $fields 4] < -0.0005 ||
        ![string is double -strict [lindex $fields 8]] || [lindex $fields 8] > 300.001} {
      error "Group-13 results row for $family failed exact governed validation"
    }
  }
  return [sha256_file $group13_csv]
}

proc validate_groups14_17_gate {} {
  global evidence_root groups14_17_gate expected_groups14_17_gate_sha groups14_17_csv
  global expected_checkpoint_sha expected_base_xdc_sha expected_bs3_candidate_xdc_sha
  global expected_g13_candidate_xdc_sha
  if {[sha256_file $groups14_17_gate] ne $expected_groups14_17_gate_sha} {
    error "Groups 14-17 gate does not match caller-provided SHA-256"
  }
  set values [parse_kv $groups14_17_gate]
  require_kv $values STATE COMPLETE
  require_kv $values GROUPS14_17_GATE PASS
  require_kv $values GROUP_IDS 14,15,16,17
  require_kv $values SEALED_DCP_SHA256 $expected_checkpoint_sha
  require_kv $values BASE_XDC_SHA256 $expected_base_xdc_sha
  require_kv $values BS3_CANDIDATE_XDC_SHA256 $expected_bs3_candidate_xdc_sha
  require_kv $values G13_CANDIDATE_XDC_SHA256 $expected_g13_candidate_xdc_sha
  ...
  set worker_path [file join $evidence_root tools G2B_LUT1_GROUPS14_17_WORKER.tcl]
  set orchestrator_path [file join $evidence_root tools Invoke-G2BLut1Groups14To17.ps1]
  if {[sha256_file $worker_path] ne $worker_sha ||
      [sha256_file $orchestrator_path] ne $orchestrator_sha} {
    error "Groups 14-17 receipt is not bound to the present governed worker/orchestrator"
  }
  require_kv $values GROUPS_REQUIRED 4
  require_kv $values GROUPS_PASS 4
  ...
  if {$receipt_csv ne [file normalize $groups14_17_csv]} { error "Groups 14-17 CSV path drift" }
  set csv_sha [require_hash_value [require_kv $values RESULTS_CSV_SHA256] RESULTS_CSV_SHA256]
  if {[sha256_file $groups14_17_csv] ne $csv_sha} { error "Groups 14-17 CSV hash drift" }
  set csv_lines [list]
  foreach line [split [read_text $groups14_17_csv] "\n"] {
    set line [string trimright $line "\r"]
    if {$line ne ""} { lappend csv_lines $line }
  }
  set header [list Group_ID Name Command Source_Count Destination_Count Runtime_s Actual_Result \
    Actual_ns Required_ns Slack_ns Warning_Count Warnings Timeout_Phase Process_Exit_Code]
  if {[llength $csv_lines] != 5 || [parse_csv_row [lindex $csv_lines 0]] ne $header} {
    error "Groups 14-17 results CSV shape/header drift"
  }
  set names [list RELEASE_SLOT_0_AXI_TO_SOURCE RELEASE_SLOT_1_AXI_TO_SOURCE \
    RELEASE_SLOT_2_AXI_TO_SOURCE RELEASE_SLOT_3_AXI_TO_SOURCE]
  for {set index 1} {$index <= 4} {incr index} {
    set fields [parse_csv_row [lindex $csv_lines $index]]
    set expected_group [expr {$index + 13}]
    if {[llength $fields] != 14 || [lindex $fields 0] ne $expected_group ||
        [lindex $fields 1] ne [lindex $names [expr {$index - 1}]] ||
        [lindex $fields 3] ne "56" || [lindex $fields 4] ne "20" ||
        [lindex $fields 6] ne "PASS" || [lindex $fields 8] ne "3.000" ||
        [lindex $fields 12] ne "NONE" || [lindex $fields 13] ne "0" ||
        ![string is double -strict [lindex $fields 5]] || [lindex $fields 5] > 300.001 ||
        ![string is double -strict [lindex $fields 7]] || [lindex $fields 7] > 3.0005 ||
        ![string is double -strict [lindex $fields 9]] || [lindex $fields 9] < -0.0005} {
      error "Groups 14-17 CSV row for Group $expected_group failed exact governed validation"
    }
  }
  return $csv_sha
}
...
proc validate_offline_protection_gate {} {
  ...
  require_kv $values SSOT_REV5_COMPATIBILITY PASS
  require_kv $values HARDWARE_ACCESSED NO
  return PASS
}
...
# endpoint hashes remain frozen. Group 9 remains META-4/BS3 governed and the
# reset-result family is advanced exactly to META-5/G13-A authority.
proc enforce_exact_cdc_disposition {cdc_path cdc_violations} {
  ...
  foreach row $all_rows {
    ...
    if {$id eq "CDC-1"} {
      ...
      } elseif {$family eq "RESET_RESULT_SOURCE_TO_AXI"} {
        set family RESET_COMMIT_PHASE_COMPLETION_BARRIER
        set classification STABLE_DATA+COMBINATIONAL_AGGREGATION
        set disposition META5_G13A_SETTLING_PLUS_STRUCTURAL_CDC
      } else {
  ...
  lappend disposition_lines \
    ...
    {RESET_RETURN_GROUP13_METHOD=SETTLING_PLUS_STRUCTURAL_CDC} \
    {RESET_RETURN_GROUP13_SEMANTIC_FAMILY_1=RESET_ABANDONED_COUNT_STABLE_PAYLOAD} \
    {RESET_RETURN_GROUP13_SEMANTIC_FAMILY_2=RESET_COMMIT_PHASE_COMPLETION_BARRIER} \
    {RESET_RETURN_GROUP13_CLASSIFICATION=STABLE_DATA+HANDSHAKE+COMMIT_PHASE_COMPLETION_BARRIER+COMBINATIONAL_AGGREGATION} \
    {RESET_RETURN_GLOBAL_SKEW_REQUIREMENT=RETIRED_BY_META_5} \
...
proc legacy_group9_family_gate_disabled {label sources destinations destination_d report_path} {
  error "legacy Group-9 timing query is disabled in Recovery-2"
  ...
  error "unreachable legacy Group-9 report_timing"
  return [dict create \
...
proc legacy_group9_ownership_gate_disabled {slot_sources generation_sources epoch_sources destinations destination_d} {
  error "legacy Group-9 timing rerun is disabled in Recovery-2"
  ...
  error "unreachable legacy Group-9 family evaluation"
...
proc run_cdc_gate {} {
  ...
  write_lines_atomic $gate_path [list \
    ...
    {RESET_RETURN_CDC=PASS} \
    {RESET_RETURN_CLASSIFICATION=STABLE_DATA+HANDSHAKE+COMMIT_PHASE_COMPLETION_BARRIER+COMBINATIONAL_AGGREGATION} \
    {RESET_RETURN_METHOD=SETTLING_PLUS_STRUCTURAL_CDC} \
    {GLOBAL_GROUP9_REPORT_BUS_SKEW_EXECUTED=NO} \
    {GLOBAL_GROUP13_REPORT_BUS_SKEW_EXECUTED=NO} \
    "RUNTIME_SECONDS=[seconds_since $phase_start]"]
  return [dict create path $gate_path sha [sha256_file $gate_path] \
...
proc run_clock_resource_gate {timing drc cdc} {
  ...
  set methodology_path [file join $raw_root METHODOLOGY.rpt]
  ...
  set methodology_inventory [list {Name,Severity,Description,Disposition}]
  foreach violation [lsort -dictionary [get_methodology_violations -quiet]] {
    set severity [string toupper [property_or_unknown SEVERITY $violation]]
    ...
    if {$severity eq "ERROR" || $severity eq "CRITICAL WARNING"} {
      set disposition RELEASE_BLOCKING
    } elseif {[regexp {^TIMING-(34|39)#} [get_property NAME $violation]]} {
      set disposition GOVERNED_BY_META4_META5_FOCUSED_REPLACEMENT_AND_FRESH_TIMING
    } elseif {[regexp {^CDC-} [get_property NAME $violation]]} {
      set disposition GOVERNED_BY_EXACT_427_ENTRY_CDC_DISPOSITION
    } else {
      set disposition INVENTORIED_NON_BLOCKING_WARNING
    }
    lappend methodology_inventory "[csv_value [get_property NAME $violation]],[csv_value $severity],[csv_value [property_or_unknown DESCRIPTION $violation]],[csv_value $disposition]"
  }
  write_lines_atomic [file join $raw_root METHODOLOGY_INVENTORY.csv] $methodology_inventory
  ...
  set methodology_gate_path [file join $raw_root G2B_LUT1_METHODOLOGY_GATE.txt]
  write_lines_atomic $methodology_gate_path [list \
    {RESULT=PASS} \
    "ERRORS=$methodology_errors" \
    "CRITICAL_WARNINGS=$methodology_critical_warnings" \
    "WARNINGS=$methodology_warnings" \
    {UNRESOLVED_CRITICAL_METHODOLOGY_BLOCKERS=0} \
    {TIMING_34_39_DISPOSITION=META4_META5_FOCUSED_REPLACEMENT_PLUS_FRESH_TIMING} \
    {CDC_DISPOSITION=EXACT_HASH_BOUND_427_ENTRY_SET}]
  ...
  return [dict create \
    ...
    methodology_path $methodology_gate_path methodology_sha [sha256_file $methodology_gate_path] \
    methodology_errors $methodology_errors methodology_critical_warnings $methodology_critical_warnings \
...
proc legacy_recovery1_main_disabled {} {
  ...
  # Recovery-1 implementation retained solely as inert source history. Its
  # former Group-9 timing rerun is deliberately removed in Recovery-2.
  error "legacy recovery-1 entry point is permanently disabled"
...
proc revalidate_recovery2_inputs {preserved_sha group13_csv_sha groups14_csv_sha timing drc cdc clock_resource} {
  global checkpoint base_xdc bs3_candidate_xdc g13_candidate_xdc active_xdc xpr_path
  global expected_checkpoint_sha expected_base_xdc_sha expected_bs3_candidate_xdc_sha
  global expected_g13_candidate_xdc_sha expected_active_xdc_sha expected_xpr_sha
  global preserved_gate expected_preserved_gate_sha
  global group13_gate expected_group13_gate_sha group13_csv
  global groups14_17_gate expected_groups14_17_gate_sha groups14_17_csv
  global offline_gate expected_offline_gate_sha
  global finalizer_path expected_finalizer_sha runner_path expected_runner_sha

  validate_source_identity
  validate_governed_static_inputs
  validate_preserved_results
  validate_group13_gate
  validate_groups14_17_gate
  validate_offline_protection_gate
  foreach {path expected label} [list \
      $checkpoint $expected_checkpoint_sha DCP \
      $base_xdc $expected_base_xdc_sha BASE_XDC \
      $bs3_candidate_xdc $expected_bs3_candidate_xdc_sha BS3_CANDIDATE_XDC \
      $g13_candidate_xdc $expected_g13_candidate_xdc_sha G13_CANDIDATE_XDC \
      $active_xdc $expected_active_xdc_sha ACTIVE_XDC \
      $xpr_path $expected_xpr_sha XPR \
      $preserved_gate $preserved_sha PRESERVED_RESULTS \
      $group13_gate $expected_group13_gate_sha GROUP13_GATE \
      $group13_csv $group13_csv_sha GROUP13_RESULTS \
      $groups14_17_gate $expected_groups14_17_gate_sha GROUPS14_17_GATE \
      $groups14_17_csv $groups14_csv_sha GROUPS14_17_RESULTS \
      $offline_gate $expected_offline_gate_sha OFFLINE_PROTECTION_GATE \
      $finalizer_path $expected_finalizer_sha FINALIZER_TCL \
      $runner_path $expected_runner_sha WATCHDOG_RUNNER \
      [dict get $timing path] [dict get $timing sha] TIMING_GATE \
      ...
      [dict get $clock_resource resource_path] [dict get $clock_resource resource_sha] RESOURCE_GATE \
      [dict get $clock_resource config_path] [dict get $clock_resource config_sha] XDMA_CONFIG_GATE \
      [dict get $clock_resource methodology_path] [dict get $clock_resource methodology_sha] METHODOLOGY_GATE] {
    if {[sha256_file $path] ne $expected} { error "$label changed before bitstream boundary" }
  }
  if {[route_signature] ne [list 1 0 0 0]} {
    error "route signature changed before bitstream boundary"
...
proc main {} {
  ...
  global checkpoint base_xdc bs3_candidate_xdc g13_candidate_xdc active_xdc xpr_path
  global expected_checkpoint_sha expected_base_xdc_sha expected_bs3_candidate_xdc_sha
  global expected_g13_candidate_xdc_sha expected_active_xdc_sha
  global expected_source_branch expected_source_parent expected_source_commit expected_source_tree
  global expected_finalizer_sha expected_runner_sha finalizer_path runner_path
  global preserved_gate expected_preserved_gate_sha
  global group13_gate expected_group13_gate_sha group13_csv
  global groups14_17_gate expected_groups14_17_gate_sha groups14_17_csv
  global offline_gate expected_offline_gate_sha final_result_path
  ...
  validate_source_identity
  validate_governed_static_inputs
  set preserved_sha [validate_preserved_results]
  set group13_csv_sha [validate_group13_gate]
  set groups14_csv_sha [validate_groups14_17_gate]
  validate_offline_protection_gate
  ...
  set applied_xdc [file join $raw_root ROUTED_TIMING_APPLIED_BASE_PLUS_BS3_PLUS_G13A.xdc]
  write_xdc -exclude_physical -force $applied_xdc
  set applied_text [read_text $applied_xdc]
  if {[count_xdc_command_lines $applied_text set_bus_skew] != 15 ||
      [count_xdc_command_lines $applied_text set_max_delay] != 14 ||
      [count_xdc_command_lines $applied_text set_false_path] != 30 ||
      [count_xdc_command_lines $applied_text set_clock_groups] != 1} {
    error "reloaded complete timing constraint census drift"
  }
  set initialization_gate [file join $raw_root G2B_LUT1_RECOVERY2_INITIALIZATION_GATE.txt]
  write_lines_atomic $initialization_gate [list \
    {RESULT=PASS} \
    ...
    "SOURCE_PARENT=$expected_source_parent" \
    "SOURCE_COMMIT=$expected_source_commit" \
    "SOURCE_TREE=$expected_source_tree" \
    "SEALED_DCP_SHA256=$expected_checkpoint_sha" \
    "DERIVED_FULL_BASE_SHA256=$expected_base_xdc_sha" \
    "BS3_CANDIDATE_XDC_SHA256=$expected_bs3_candidate_xdc_sha" \
    "G13A_CANDIDATE_XDC_SHA256=$expected_g13_candidate_xdc_sha" \
    "ACTIVE_XDC_SHA256=$expected_active_xdc_sha" \
    "PRESERVED_RESULTS_SHA256=$preserved_sha" \
    "GROUP13_GATE_SHA256=$expected_group13_gate_sha" \
    "GROUP13_RESULTS_CSV_SHA256=$group13_csv_sha" \
    "GROUPS14_17_GATE_SHA256=$expected_groups14_17_gate_sha" \
    "GROUPS14_17_RESULTS_CSV_SHA256=$groups14_csv_sha" \
    "OFFLINE_PROTECTION_GATE_SHA256=$expected_offline_gate_sha" \
    "VIVADO_VERSION=$vivado_version" \
    "VIVADO_SW_BUILD=$vivado_sw_build" \
    {APPLIED_BUS_SKEW_CONSTRAINTS=15} \
    {APPLIED_MAX_DELAY_CONSTRAINTS=14} \
    {GLOBAL_GROUP9_REPORT_BUS_SKEW_EXECUTED=NO} \
    {GLOBAL_GROUP13_REPORT_BUS_SKEW_EXECUTED=NO} \
    {GROUP9_TIMING_RERUN=NO} \
    {GROUPS10_12_RERUN=NO} \
    {INVALID_CONSTRAINT_COLLECTIONS=0} \
    {HARDWARE_ACCESSED=NO}]
  ...
  begin_phase PRE_BIT_GATE 900
  revalidate_recovery2_inputs $preserved_sha $group13_csv_sha $groups14_csv_sha \
    $timing $drc $cdc $clock_resource
  set pre_bit_path [file join $evidence_root G2B_PRE_BITSTREAM_HARD_GATE_RECOVERY2.txt]
  write_lines_atomic $pre_bit_path [list \
    {G2B_PRE_BITSTREAM_HARD_GATE_RECOVERY2=PASS} \
    {SSOT_REV5_COMPATIBILITY=PASS} \
    {META5=VERIFIED} \
    {G13A_CANDIDATE=VERIFIED} \
    {SOURCE_IDENTITY=PASS} \
    {GROUP9=PRESERVED_PASS} \
    {GROUPS10_12=PRESERVED_PASS} \
    ...
    {NO_UNRESOLVED_BLACK_BOXES=PASS} \
    {NO_UNRESOLVED_CRITICAL_METHODOLOGY_BLOCKER=PASS} \
    {NO_SOURCE_DCP_IDENTITY_AMBIGUITY=PASS} \
    {NO_INVALID_CONSTRAINT_COLLECTION=PASS} \
    {GLOBAL_GROUP9_REPORT_BUS_SKEW_EXECUTED=NO} \
    {GLOBAL_GROUP13_REPORT_BUS_SKEW_EXECUTED=NO} \
    {GROUP9_TIMING_RERUN=NO} \
    {HARDWARE_ACCESSED=NO} \
  ...
  set identity_path [file join $evidence_root G2B_LUT1_PRODUCT_CANDIDATE_IDENTITY.json]
  set identity_text "{\n"
  append identity_text "  \"source_branch\": \"[json_value $expected_source_branch]\",\n"
  append identity_text "  \"source_parent\": \"$expected_source_parent\",\n"
  append identity_text "  \"source_commit\": \"$expected_source_commit\",\n"
  append identity_text "  \"source_tree\": \"$expected_source_tree\",\n"
  append identity_text "  \"project_state_rev\": 5,\n"
  append identity_text "  \"meta4_evidence_commit\": \"27dcf152e862c6db88a365328b92c0fd250f04c2\",\n"
  append identity_text "  \"meta5_evidence_commit\": \"bbdeb474ce9d7e5f0db3e8ca8afb5448eef8f314\",\n"
  append identity_text "  \"g13a_evidence_commit\": \"10c7c2898d162af8e2262b3f99861c7d560c4557\",\n"
  append identity_text "  \"active_constraint_authority\": \"META5_G13A_PLUS_META4_BS3\",\n"
  append identity_text "  \"active_xdc_sha256\": \"$expected_active_xdc_sha\",\n"
  append identity_text "  \"routed_dcp_sha256\": \"$expected_checkpoint_sha\",\n"
  append identity_text "  \"recovery_mode\": \"ROUTED_DCP_REUSE\",\n"
  append identity_text "  \"vivado_version\": \"[json_value $vivado_version]\",\n"
  append identity_text "  \"vivado_sw_build\": \"[json_value $vivado_sw_build]\",\n"
  append identity_text "  \"device\": \"xc7a35tcsg325-2\",\n"
  append identity_text "  \"product_profile\": \"PRODUCT\",\n"
  append identity_text "  \"transport_abi\": \"AHD_C2H_TRANSPORT_ABI_V1\",\n"
  append identity_text "  \"abi_version\": 1,\n"
  append identity_text "  \"mmio_range\": \"0x3800..0x3BFF\",\n"
  append identity_text "  \"effective_user_clock_mhz\": [dict get $clock_resource effective_frequency],\n"
  append identity_text "  \"effective_axi_clock_mhz\": [dict get $clock_resource effective_frequency],\n"
  append identity_text "  \"resources\": {\"lut_used\": [dict get $clock_resource lut_used], \"lut_total\": [dict get $clock_resource lut_available], \"lut_percent\": [dict get $clock_resource lut_percent], \"ff_used\": [dict get $clock_resource ff_used], \"ff_total\": [dict get $clock_resource ff_available], \"ff_percent\": [dict get $clock_resource ff_percent], \"bram_used\": [dict get $clock_resource bram_used], \"bram_total\": [dict get $clock_resource bram_available], \"bram_percent\": [dict get $clock_resource bram_percent]},\n"
  append identity_text "  \"bitstream_path\": \"[json_value $bit_path]\",\n"
  append identity_text "  \"bitstream_size\": [file size $bit_path],\n"
  append identity_text "  \"bitstream_sha256\": \"$bit_sha\",\n"
  append identity_text "  \"ltx_path\": null,\n"
  append identity_text "  \"ltx_sha256\": null,\n"
  append identity_text "  \"hardware_accessed\": false\n"
  append identity_text "}\n"
  write_text_atomic $identity_path $identity_text
  ...
  write_lines_atomic $final_result_path [list \
    ...
    {GROUPS10_12=PRESERVED_PASS} \
    {GROUP13_REPLACEMENT_SIGNOFF=PASS} \
    {GROUP13_FAMILY_1=RESET_ABANDONED_COUNT_STABLE_PAYLOAD_PASS} \
    {GROUP13_FAMILY_2=RESET_COMMIT_PHASE_COMPLETION_BARRIER_PASS} \
    {GROUPS14_17=PASS} \
    {ROUTED_TIMING=PASS} \
    "WNS=[dict get $timing wns]" \
    "TNS=[dict get $timing tns]" \
    ...
    {FIRST_BLOCKER=NONE}]
  begin_phase COMPLETE 30
  puts "G2B_LUT1_SIGNOFF_RECOVERY2=PASS BITSTREAM_SHA256=$bit_sha"
}

if {[llength $argv] != 14} {
  puts stderr "usage: finalizer.tcl EVIDENCE_ROOT BITSTREAM_PATH OFFLINE_GATE OFFLINE_SHA PRESERVED_GATE PRESERVED_SHA GROUP13_GATE GROUP13_SHA GROUPS14_17_GATE GROUPS14_17_SHA PHASE_MARKER RUNNER_PATH RUNNER_SHA FINALIZER_SHA"
  exit 2
}

lassign $argv evidence_root bit_path offline_gate expected_offline_gate_sha \
  preserved_gate expected_preserved_gate_sha \
  group13_gate expected_group13_gate_sha \
  groups14_17_gate expected_groups14_17_gate_sha \
  phase_marker runner_path expected_runner_sha expected_finalizer_sha
foreach variable {evidence_root bit_path offline_gate preserved_gate group13_gate groups14_17_gate phase_marker runner_path} {
...
set expected_preserved_gate_sha [require_hash_value \
  [string toupper $expected_preserved_gate_sha] EXPECTED_PRESERVED_GATE_SHA]
set expected_group13_gate_sha [require_hash_value \
  [string toupper $expected_group13_gate_sha] EXPECTED_GROUP13_GATE_SHA]
set expected_groups14_17_gate_sha [require_hash_value \
  [string toupper $expected_groups14_17_gate_sha] EXPECTED_GROUPS14_17_GATE_SHA]
...
set expected_source_parent 66cc8e3497579c2f7cb41d0b3639b3c2f00d6c49
set expected_source_commit 64feb60de5d07f400e6b92527bfe54838b3372ee
set expected_source_tree 26399ed456941e26d5ee4b1b2ca50392338fa24a
...
set expected_active_xdc_sha C12A371F7F21D350A28C6B310046D543C788D40E805160F12C49FB24C467674C
...
set g13a_root [file normalize {C:/FPGA/V41_G2B_EVIDENCE/v41-development-g2b-g13a-reset-return-signoff-audit}]
set base_xdc [file join $g13a_root raw timing G2B_G13A_FULL_BASE_WITHOUT_GROUP9_AND_GROUP13.xdc]
set expected_base_xdc_sha 3F7D8613AB3ECF579F3F1E7A09B1608602768D2B9C880CE3B755437081DF1F87
set bs3_candidate_xdc [file join $bs3_root G2B_BS3_CANDIDATE_OWNERSHIP_CONSTRAINTS.xdc]
set expected_bs3_candidate_xdc_sha AE4BD91C1A8C3B1AF2FB9B0EA9A9382E9F618FD8E223BACF98E4468C10EAD087
set g13_candidate_xdc [file join $g13a_root G2B_G13A_CANDIDATE_CONSTRAINTS.xdc]
set expected_g13_candidate_xdc_sha E941A6F4A8D435B7496892C189CAA4A67DC5A8B17FE3CC9EACB2B9F18091D312
...
set evidence_repo [file normalize {C:/FPGA/V41_G2B_EVIDENCE}]
set ssot_manifest [file join $evidence_repo project-current-state SHA256_MANIFEST.txt]
set meta5_manifest [file join $evidence_repo v41-meta-project-state-rev5-group13-reset-return-signoff META5_SHA256_MANIFEST.txt]
set g13a_manifest [file join $g13a_root G2B_G13A_SHA256_MANIFEST.txt]
...
set raw_root [file join $evidence_root raw final_signoff_recovery2]
set prior_evidence_root [file join $evidence_repo v41-development-g2b-lut1-signoff-recovery]
set group13_csv [file join $evidence_root G2B_LUT1_GROUP13_SIGNOFF_RESULTS.csv]
set groups14_17_csv [file join $evidence_root G2B_LUT1_GROUPS14_17_RESULTS.csv]
...
if {$code != 0} {
  ...
  puts stderr "G2B_LUT1_SIGNOFF_RECOVERY2=FAIL ERROR=[safe_value $message]"
  exit 1
}
exit 0
