...
from availability_classifier_r3r2 import (
    AVAILABILITY_CLASSES,
    CAPTURE_ELIGIBLE_CLASSES,
    firmware_advance_response,
    validate_complete_matrix,
)
from controller_nvp_capture_r3r2 import Mmio as ProductMmio
from controller_nvp_capture_r3r2 import hardware_quiescent
from controller_nvp_capture_r3r2 import runtime_identity as read_product_identity
from vbi_tail_contract_r3r2 import summarize_tail_deltas
...
CAPTURE_DETAIL_FIELDS = [
    ...
    "CleanupResult", "ActiveFrameAndTransportIntegrity",
    "BoundedRouteSpecificVBITail", "LegacyCH1ExactTailFingerprint",
    "RouteVBITailFingerprint", "CaptureSequenceDeltaValues",
    "VerticalTailIntervalValues",
]
PIXEL_DETAIL_FIELDS = [
...
FIRMWARE_ACK_FIELDS = [
    "TimestampNs", "SessionID", "HostCaptureResult", "ResponsePurpose",
    "FirmwareResponseRaw", "Accepted", "Result",
]
...
def corrected_capture_validation_pass(validation: dict[str, Any]) -> bool:
    """R3R2 capture gate: bounded VBI is hard; legacy exact-tail is reporting."""
    return bool(
        validation.get("result") == "PASS" and
        validation.get("active_frame_and_transport_integrity") == "PASS" and
        validation.get("bounded_route_specific_vbi_tail") == "PASS" and
        validation.get("frame_boundary_count", 0) >= 1 and
...
def capture_session(args: argparse.Namespace, session_id: int, round_number: int,
    ...
    require(corrected_capture_validation_pass(validation),
...
def history_updates(session_result: dict[str, Any], capture_result: str,
    ...
    capture_fields = {
        ...
        "ActiveFrameAndTransportIntegrity": validation.get(
            "active_frame_and_transport_integrity"),
        "BoundedRouteSpecificVBITail": validation.get(
            "bounded_route_specific_vbi_tail"),
        "LegacyCH1ExactTailFingerprint": validation.get(
            "legacy_ch1_exact_21_tail_fingerprint"),
        "RouteVBITailFingerprint": validation.get(
            "route_vbi_tail_fingerprint"),
        "CaptureSequenceDeltaValues": ";".join(
            str(value) for value in
            validation.get("capture_sequence_delta_values", [])),
        "VerticalTailIntervalValues": ";".join(
            str(value) for value in
            validation.get("vertical_tail_interval_values", [])),
...
def summarize_r3r2(sessions: list[dict[str, Any]],
    ...
    all_capture_structure = all(
        row["validation"].get("result") == "PASS" and
        row["validation"].get("active_frame_and_transport_integrity") == "PASS" and
        row["validation"].get("bounded_route_specific_vbi_tail") == "PASS" and
        row["validation"].get("frame_boundary_count", 0) >= 1 and
        row["validation"].get("record_integrity_failures") == 0 and
        row["validation"].get("global_sequence_gaps") == 0 and
        row["validation"].get("attempt_sequence_gaps") == 0 and
        row["validation"].get("overflow_flag_records") == 0 and
    ...
    for channel in range(1, 5):
        ...
        tail_deltas = [
            int(value)
            for row in channel_captures
            for value in row["validation"].get(
                "capture_sequence_delta_values", [])
        ]
        tail_summary = summarize_tail_deltas(tail_deltas)
        if not channel_captures:
            tail_summary["fingerprint_classification"] = (
                "NOT_CAPTURED_NO_BT656")
        channel_summary[f"CH{channel}"] = {
    ...
    all_channels_multicolor = (
        len(multi_color_channels) == 4 and len(eligible) == 16 and
        len(captures) == 16)
    multi_channel_multicolor = len(multi_color_channels) >= 2
    if logical_camera is not None:
        ...
        if spatial and camera_temporal == "PASS":
            ...
            overall = "PASS_LOGICAL_CAMERA_CHANNEL_IDENTIFIED_PHYSICAL_CONNECTOR_OPEN"
    elif all_channels_multicolor:
        overall = "PASS_ALL_FOUR_CHANNELS_BT656_AND_BGDCOL_QUALIFIED"
    elif multi_channel_multicolor:
        overall = "PASS_MULTI_CHANNEL_MULTI_COLOR_PATH_PROVEN_MATRIX_COMPLETE"
    ...
    elif set(captured_channels) == {1, 3}:
        overall = "PASS_CH1_AND_CH3_BT656_AVAILABLE_MATRIX_COMPLETE"
        ...
        overall = (
            "PASS_PARTIAL_BT656_AVAILABILITY_ROUTE_SPECIFIC_VBI_MATRIX_COMPLETE")
    ...
    return {
        ...
        "out_of_range_vbi_captures": sum(
            row["validation"].get("route_vbi_tail_fingerprint") ==
            "OUT_OF_BOUNDS_ROUTE_TAIL" for row in captures),
        "variable_but_bounded_vbi_captures": sum(
            row["validation"].get("route_vbi_tail_fingerprint") ==
            "VARIABLE_BUT_BOUNDED_ROUTE_TAIL" for row in captures),
        "all_attempted_captures_active_frame_integrity": all(
            row["validation"].get("active_frame_and_transport_integrity") ==
            "PASS" for row in captures),
        "all_attempted_captures_bounded_vbi_tail": all(
            row["validation"].get("bounded_route_specific_vbi_tail") ==
            "PASS" for row in captures),
...
def persist_r3r2_availability(
        root: Path, sessions: list[dict[str, Any]],
        firmware_ack_ledger: list[dict[str, Any]]) -> None:
    ...
    acknowledgements = {
        int(row["SessionID"]): row for row in firmware_ack_ledger
        if row.get("Accepted") is True and row.get("Result") == "PASS"
    }
    for session in sessions:
        controller = session["controller"]
        availability = controller["source_availability"]
        acknowledgement = acknowledgements.get(int(session["session_id"]))
        require(acknowledgement is not None,
                "NVP_DIAG1_R3R2_PUBLIC_PROJECTION_WITHOUT_ACCEPTED_ACK")
        for phase, key in (
        ...
        matrix_rows.append({
            ...
            "FirmwareAdvanceAck": "PASS_AND_QUIESCENT",
        })
        reset_rows.append({
        ...
        outcome_rows.append({
            ...
            "FirmwareAdvanceAck": "PASS_AND_QUIESCENT",
            "FirmwareResponseRaw": acknowledgement["FirmwareResponseRaw"],
        })
...
def main() -> int:
    ...
    firmware_ack_ledger: list[dict[str, Any]] = []
    ...
    firmware_ack_csv = args.logs_root / "firmware-ack-ledger.csv"
    ...
    write_csv_atomic(firmware_ack_csv, FIRMWARE_ACK_FIELDS,
                     firmware_ack_ledger)
    try:
        ...
        for expected_session in range(1, 17):
            ...
            if capture_error is not None:
                ...
                firmware_ack_ledger.append({
                    "TimestampNs": time.time_ns(), "SessionID": session_id,
                    "HostCaptureResult": "FAIL",
                    "ResponsePurpose": "CAPTURE_FAILURE_SAFE_RESTORE",
                    "FirmwareResponseRaw": f"0x{((session_id << 16) | 0xA):08X}",
                    "Accepted": True, "Result": "FAIL",
                })
                write_csv_atomic(firmware_ack_csv, FIRMWARE_ACK_FIELDS,
                                 firmware_ack_ledger)
            ...
            firmware_ack_ledger.append({
                "TimestampNs": time.time_ns(), "SessionID": session_id,
                "HostCaptureResult": capture_outcome,
                "ResponsePurpose": (
                    "ADVANCE_AFTER_SUCCESSFUL_CAPTURE" if
                    capture_outcome == "PASS" else
                    "ADVANCE_AFTER_SAFE_NO_CAPTURE_OBSERVATION"),
                "FirmwareResponseRaw": response_hex,
                "Accepted": True, "Result": "PASS",
            })
            write_csv_atomic(firmware_ack_csv, FIRMWARE_ACK_FIELDS,
                             firmware_ack_ledger)
            session_result["controller"]["firmware_response_raw"] = response_hex
            session_result["controller"]["firmware_response_accepted"] = True
        ...
        persist_r3r2_availability(args.logs_root, sessions,
                                  firmware_ack_ledger)
        summary = summarize_r3r2(sessions, firmware_rows)
        result = {
