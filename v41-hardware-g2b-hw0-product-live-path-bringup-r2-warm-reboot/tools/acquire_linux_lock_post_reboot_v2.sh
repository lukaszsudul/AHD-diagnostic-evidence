...
[ "$node_count" -eq 0 ]
other_task_lock_count="$(find /tmp /run/lock /var/lock -mindepth 1 -maxdepth 1 -type d \( -iname 'ahd*.lock' -o -iname 'g2b*.lock' -o -iname 'xdma*.lock' -o -iname 'fpga*.lock' \) -print | wc -l)"
...
echo "XDMA_DEVICE_NODE_COUNT_BEFORE_LOCK=$node_count"
