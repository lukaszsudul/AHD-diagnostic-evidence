...
$taskRoot='C:\FPGA\V41_NVP_R1E_JTAG_RECOVERED_PAIRED_AB_R5';$helperPath=Join-Path $taskRoot 'scripts\Invoke-ContextualPlink.ps1';$readerPath=Join-Path $taskRoot 'scripts\read_nvp_r1e.py'
...
$roleDirectory=if($Role-eq'ArmA'){Join-Path $taskRoot '06_ARM_A_R1E'}else{Join-Path $taskRoot '07_ARM_B_FORMAL'};$fullEvidence=[IO.Path]::GetFullPath($EvidencePath);$roleFull=[IO.Path]::GetFullPath($roleDirectory)
if(-not$fullEvidence.StartsWith($roleFull+[IO.Path]::DirectorySeparatorChar,[StringComparison]::OrdinalIgnoreCase)){throw 'telemetry evidence must stay inside the selected R5 phase directory'};if(Test-Path -LiteralPath $fullEvidence){throw 'refusing to overwrite telemetry evidence'};if(-not(Test-Path -LiteralPath (Split-Path -Parent $fullEvidence)-PathType Container)){throw 'telemetry evidence parent directory does not exist'}
...
&$helperPath -PlinkPath $PlinkPath -HostKey $hostKey -RemoteCommand $remoteCommand -EvidencePath $fullEvidence -ExpectedIp '10.132.1.111' -ExpectedUser 'vcdeagent1' -EvidenceKind("R5_FULL_TELEMETRY_{0}"-f$Role.ToUpperInvariant()) -SendPasswordToStdin -SudoPasswordCopies 1 -TimeoutSeconds 120
exit $LASTEXITCODE
