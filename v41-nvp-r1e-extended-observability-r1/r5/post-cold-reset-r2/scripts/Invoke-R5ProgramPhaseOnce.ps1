...
$r1eBitPath = 'C:\FPGA\V41_NVP_R1E_SEMANTIC_DRC_CONTINUATION_R3\05_BITSTREAM\ahd_capture_v41_i2c_25khz_r1e_observability.bit'
...
$phaseSpec = switch ($Phase) {
    'FormalBootstrap' {
        [pscustomobject]@{
            EvidenceDirectory = Join-Path $taskRoot '05_FORMAL_BOOTSTRAP'
    ...
    'ArmA' {
        [pscustomobject]@{
            EvidenceDirectory = Join-Path $taskRoot '06_ARM_A_R1E'
            TclRole = 'ARM_A_25KHZ'
            BitPath = $r1eBitPath
    ...
    'ArmB' {
        [pscustomobject]@{
            EvidenceDirectory = Join-Path $taskRoot '07_ARM_B_FORMAL'
            TclRole = 'ARM_B_FORMAL_50KHZ'
            BitPath = $formalBitPath
            Filename = $formalFilename
            Size = $formalSize
