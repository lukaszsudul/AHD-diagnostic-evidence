...
$taskRoot='C:\FPGA\V41_NVP_R1E_JTAG_RECOVERED_PAIRED_AB_R5';$expectedIp='10.132.1.111'
$roleDirectory=switch($Role){'FormalBootstrap'{Join-Path $taskRoot '05_FORMAL_BOOTSTRAP'}'ArmA'{Join-Path $taskRoot '06_ARM_A_R1E'}'ArmB'{Join-Path $taskRoot '07_ARM_B_FORMAL'}}
$fullEvidence=[IO.Path]::GetFullPath($EvidencePath);$roleFull=[IO.Path]::GetFullPath($roleDirectory)
if(-not$fullEvidence.StartsWith($roleFull+[IO.Path]::DirectorySeparatorChar,[StringComparison]::OrdinalIgnoreCase)){throw 'host-cycle evidence must stay inside the selected R5 phase directory'};if(Test-Path -LiteralPath $fullEvidence){throw 'refusing to overwrite host-cycle evidence'};if(-not(Test-Path -LiteralPath(Split-Path -Parent $fullEvidence)-PathType Container)){throw 'host-cycle evidence parent directory does not exist'}
