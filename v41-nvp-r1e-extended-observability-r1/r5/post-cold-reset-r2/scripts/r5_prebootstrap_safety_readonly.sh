#!/usr/bin/env bash
# R5 POST_COLD_RESET_R2 pre-bootstrap safety discovery. Read-only; accepts
# either zero or one exact expected FPGA endpoint and absent XDMA state.

set -euo pipefail

baseline_boot_id=${1:?post-cold-reset boot ID baseline required}
...
printf 'POST_COLD_RESET_BOOT_ID_BASELINE=%s\n' "$baseline_boot_id"
...
for evidence_dir in \
  /home/vcdeagent1/FPGA_AHD_HOST/v41_nvp_r1e_r5/bootstrap_driver \
  /home/vcdeagent1/FPGA_AHD_HOST/v41_nvp_r1e_r5/arm_a_driver \
  /home/vcdeagent1/FPGA_AHD_HOST/v41_nvp_r1e_r5/arm_b_driver; do
...
if [[ ${#expected_endpoints[@]} -eq 1 ]]; then
  ...
  printf 'ENDPOINT_BDF=ABSENT_AFTER_COLD_RESET\n'
...
if [[ $endpoint_present == NO ]]; then
  printf 'PRE_BOOTSTRAP_RUNTIME_IDENTITY=NOT_AVAILABLE_ENDPOINT_ABSENT_AFTER_COLD_RESET\n'
elif [[ $endpoint_driver != xdma || ! -e /dev/xdma0_user ]]; then
  printf 'PRE_BOOTSTRAP_RUNTIME_IDENTITY=NOT_AVAILABLE_DRIVER_OR_NODE_ABSENT_AFTER_COLD_RESET\n'
else
  set +e
...
printf 'POST_COLD_RESET_BOOT_ID_CONTINUITY=PASS_BASELINE_UNCHANGED\n'
