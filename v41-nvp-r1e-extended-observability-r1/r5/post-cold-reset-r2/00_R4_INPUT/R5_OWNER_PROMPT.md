...
v41 NVP R1e — post-cold-reset JTAG-recovered formal bootstrap and complete paired A/B campaign R5

Revised for owner-performed pre-task cold reset; no in-task cold start
...
PRIMARY_PURPOSE:
    ...
    connection recovery and then performed a pre-task cold reset by removing
    Ubuntu-host power, establish a new known exact formal Phase-2 start state
    and obtain the complete R1e Arm-A and exact formal Arm-B scientific samples.
...
R5_SEQUENCE:
    record the owner-performed pre-task cold reset;
    establish a stable post-cold-reset Ubuntu baseline;
...
1. PRE-TASK JTAG RECOVERY, COLD RESET, AND REPORTING RULE
=====================================================================
...
OWNER_PRETASK_UBUNTU_POWER_REMOVAL=
    YES

OWNER_PRETASK_COLD_RESET=
    YES

PRETASK_COLD_RESET_OCCURRED_BEFORE_R5=
    YES

FPGA_CARD_POWER_RETENTION_DURING_HOST_POWER_REMOVAL=
    NOT_ASSUMED
```

Fresh R5 evidence controls every live gate.

The exact card-power retention scope is not inferred from the owner’s phrase
“Ubuntu power was disconnected.”

Therefore R5 must not assume any of the following survived:
...
the PCIe endpoint;
BAR accessibility;
the XDMA driver binding;
the XDMA node set;
runtime identity;
diagnostic magic;
the R4 boot ID;
or any historical boot continuity.
```

Expected and acceptable post-cold-reset discovery states include:

```text
JTAG target present with stable DONE=0;

JTAG target present with stable DONE=1;

expected PCIe endpoint absent;

expected endpoint present but application identity unproven;

pinned driver absent;

XDMA nodes absent.
```

These states are not scientific failures before the mandatory formal
bootstrap, provided the safety gates in Sections 9 and 10 pass.

The mandatory exact formal bootstrap is the first operation that establishes
an authoritative R5 FPGA/application start state.

────────

1.1 Boot-ID and operation-accounting rule

Record the first stable current Ubuntu boot ID as:

```text
POST_COLD_RESET_BOOT_ID_BASELINE=
```

Do not compare it with R4 as a continuity gate.
...
The owner-performed cold reset occurred before Codex receives this task.

It is recorded separately and is not counted as an in-task operation:
...
No additional cold start or power cycle is authorized during R5.

Every bootstrap/Arm-A/Arm-B reboot authorized below is a warm reboot only.

────────

1.2 Scientific-reporting rule

The pre-task cold reset is scientifically relevant because it intentionally
breaks prior runtime/boot continuity.

The single final report must record:
...
Do not claim that the formal image or any prior SRAM image was preserved across
the cold reset.

The cause or mechanical details of the earlier JTAG-connection handling remain
outside the scientific narrative because they did not alter the frozen source,
routed DCP or bitstream.

Do not delete or rewrite raw historical evidence.

The R5 final report records fresh R5 results:
...
If a fresh R5 gate fails, report that current failure normally.
...
Current DONE may be 0 or 1 after the owner-performed pre-task cold reset.

Neither value identifies an application image at this stage.
...
because the current SRAM image is unproven after the pre-task cold reset.
...
===================================================================== 10. P3 — POST-COLD-RESET HOST STABILITY AND PRE-BOOTSTRAP SAFETY

The current SRAM application identity is not a prerequisite.

Historical R4 boot continuity is intentionally not a prerequisite because the
owner performed a pre-task cold reset.

────────

10.1 Stable post-cold-reset Ubuntu baseline

Before interpreting PCIe state, run three independent read-only SSH sessions
over at least five seconds.
...
same boot ID in all three sessions;
monotonically increasing uptime;
no observed reboot/shutdown transition.
```
...
```text
BLOCKED_POST_COLD_RESET_HOST_NOT_STABLE
```
...
10.2 Pre-bootstrap host/PCIe safety discovery
...
link and BARs only when an expected endpoint exists;

loaded modules named xdma;
driver binding when present;
node set when present;
node owners;
task DMA count;
kernel/AER health.
```
...
endpoint absence is accepted before bootstrap;
no foreign/multiple FPGA endpoint;
no wrong same-name xdma module;
pinned-driver and node absence are accepted before bootstrap;
no XDMA node owner;
zero task DMA;
...
If the endpoint is absent, record:

```text
PRE_BOOTSTRAP_RUNTIME_IDENTITY=
    NOT_AVAILABLE_ENDPOINT_ABSENT_AFTER_COLD_RESET
```

If a reader returns 0xFFFFFFFF, record it as unproven.
...
host disappears;
host returns;
new boot ID relative to POST_COLD_RESET_BOOT_ID_BASELINE;
kernel=7.0.0-29-generic.
```
...
verify frozen R3/R4 host tools and fixtures
    ->
establish the stable post-cold-reset Ubuntu boot baseline
    ->
...
No program retry.
No additional cold start or power cycle during R5.
