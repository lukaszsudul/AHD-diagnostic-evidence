...
if (-not $fullEvidence.StartsWith(
    ...
    throw 'host-cycle evidence must stay inside the selected R5 phase directory'
}
if (Test-Path -LiteralPath $fullEvidence) {
    throw 'refusing to overwrite host-cycle evidence'
}
if (-not (Test-Path -LiteralPath (Split-Path -Parent $fullEvidence) -PathType Container)) {
    throw 'host-cycle evidence parent directory does not exist'
}

function Test-Tcp22 {
    $client = [Net.Sockets.TcpClient]::new()
    try {
        $task = $client.ConnectAsync($expectedIp, 22)
        if (-not $task.Wait($ConnectTimeoutMilliseconds)) { return $false }
...
$deadlineTicks = $startTicks + [long]($TimeoutSeconds * $frequency)
$records = [Collections.Generic.List[string]]::new()
$downSeen = $false
$upAfterDownSeen = $false
$sample = 0
while ([Diagnostics.Stopwatch]::GetTimestamp() -lt $deadlineTicks) {
    $sample++
    ...
    elseif ($downSeen -and $up) { $upAfterDownSeen = $true; break }
    Start-Sleep -Milliseconds $PollMilliseconds
}
$endTicks = [Diagnostics.Stopwatch]::GetTimestamp()
$gate = if ($downSeen -and $upAfterDownSeen) { 'PASS_HOST_DISAPPEARED_AND_RETURNED' } else { 'FAIL' }
$lines = [Collections.Generic.List[string]]::new()
$lines.Add("ROLE=$Role")
$lines.Add("UTC_START=$startUtc")
$lines.Add("UTC_END=$([DateTime]::UtcNow.ToString('o'))")
$lines.Add("STOPWATCH_FREQUENCY=$frequency")
$lines.Add("START_TICKS=$startTicks")
$lines.Add("END_TICKS=$endTicks")
$lines.Add("HOST_DOWN_OBSERVED=$(if ($downSeen) {'YES'} else {'NO'})")
$lines.Add("HOST_UP_AFTER_DOWN_OBSERVED=$(if ($upAfterDownSeen) {'YES'} else {'NO'})")
$lines.Add("HOST_CYCLE_GATE=$gate")
foreach ($record in $records) { $lines.Add($record) }
[IO.File]::WriteAllLines($fullEvidence, $lines, [Text.UTF8Encoding]::new($false))
