...
$readerPath = Join-Path $taskRoot 'scripts\read_nvp_r1e.py'
...
Assert-ExactFileHash $readerPath $expectedReaderSha

$roleDirectory = if ($Role -eq 'ArmA') {
    Join-Path $taskRoot '06_ARM_A_R1E'
} else {
    Join-Path $taskRoot '07_ARM_B_FORMAL'
}
...
if (-not $fullEvidence.StartsWith(
    ...
    throw 'telemetry evidence must stay inside the selected R5 phase directory'
}
if (Test-Path -LiteralPath $fullEvidence) {
    throw 'refusing to overwrite telemetry evidence'
}
if (-not (Test-Path -LiteralPath (Split-Path -Parent $fullEvidence) -PathType Container)) {
    throw 'telemetry evidence parent directory does not exist'
}

$expect = if ($Role -eq 'ArmA') { 'r1e' } else { 'formal' }
$readerPayload = ConvertTo-GzipBase64 ([IO.File]::ReadAllBytes($readerPath))
$remoteTemplate = 'sudo -S -k -p '''' /usr/bin/bash -c ''printf %s "$1" | /usr/bin/base64 -d | /usr/bin/gzip -dc | /usr/bin/python3 - --node /dev/xdma0_user --expect "$2" --twice --delay 1.0'' _ ''{0}'' ''{1}'''
$remoteCommand = $remoteTemplate -f $readerPayload, $expect
...
& $helperPath `
    ...
    -EvidenceKind ("R5_FULL_TELEMETRY_{0}" -f $Role.ToUpperInvariant()) `
    -SendPasswordToStdin `
