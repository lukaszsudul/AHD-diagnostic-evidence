if {$argc != 5 && $argc != 6} {
  puts stderr "usage: r1e_build.tcl REPOSITORY_ROOT BUILD_ROOT EVIDENCE_ROOT SOURCE_GIT_COMMIT BIT_FILENAME ?PROVENANCE_ONLY?"
  exit 2
}
...
if {$argc == 6} {
  ...
  if {$execution_mode ne "PROVENANCE_ONLY"} {
    error "optional execution mode must be PROVENANCE_ONLY"
  }
}
...
write_lines [file join $evidence_root EXPECTED_RUNTIME_PROVENANCE.txt] [list \
...
puts "PROVENANCE_ROUND_TRIP=PASS BIT_SOURCE_COMMIT=$source_commit"
...
set project_name v41_r1e_extended_observability
...
set sv_files [list \
  ...
  [file join $repo_root rtl v41 r1e_measurement_regs.sv] \
...
report_timing -delay_type max -to $nvp_io_ports -max_paths 32 -nworst 16 \
  -file [file join $evidence_root R1E_nvp_scl_sda_output_paths.rpt]
report_timing -delay_type max -from $nvp_io_ports -max_paths 32 -nworst 16 \
  -file [file join $evidence_root R1E_nvp_scl_sda_input_paths.rpt]
set nvp_iobuf_cells [get_cells -quiet -hier -filter \
  {NAME =~ *NVP_SCL_IOBUF || NAME =~ *NVP_SDA_IOBUF}]
report_property -file [file join $evidence_root R1E_nvp_iobuf_properties.txt] \
  $nvp_iobuf_cells
...
write_lines [file join $evidence_root R1E_nvp_output_fanin.txt] \
  [lsort [get_property NAME [all_fanin -flat -to $nvp_t_pins]]]
write_lines [file join $evidence_root R1E_nvp_input_fanout.txt] \
...
write_lines [file join $evidence_root R1E_probe_cells.txt] \
  [lsort [get_property NAME $probe_cells]]
write_lines [file join $evidence_root R1E_lifecycle_cells.txt] \
  [lsort [get_property NAME $lifecycle_cells]]
