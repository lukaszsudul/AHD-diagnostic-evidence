...
$taskRoot='C:\FPGA\V41_NVP_R1E_MODE_AWARE_DONE0_PAIRED_AB_R7';$expectedIp='10.132.1.111'
...
if(-not$fullEvidence.StartsWith($roleFull+[IO.Path]::DirectorySeparatorChar,[StringComparison]::OrdinalIgnoreCase)){throw 'host-cycle evidence must stay inside the selected R7 phase directory'};if(Test-Path -LiteralPath $fullEvidence){throw 'refusing to overwrite host-cycle evidence'};if(-not(Test-Path -LiteralPath(Split-Path -Parent $fullEvidence)-PathType Container)){throw 'host-cycle evidence parent directory does not exist'}
