...
if (-not $fullEvidence.StartsWith(
    ...
    throw 'evidence path must stay inside the R7 task root'
}
...
if (-not $fullEvidence.StartsWith(
    ...
    throw 'loader evidence must stay inside the selected R7 phase directory'
}
...
$remoteTemplate = @'
set -eu; module="$HOME/FPGA_AHD_HOST/dma_ip_drivers/XDMA/linux-kernel/xdma/xdma.ko"; loader="$HOME/FPGA_AHD_HOST/phase2_precheck_accepted/phase2_load_xdma_driver.sh"; out="$HOME/FPGA_AHD_HOST/v41_nvp_r1e_r7/__DRIVER_DIRECTORY__"; test -f "$module"; test ! -L "$module"; test -f "$loader"; test ! -L "$loader"; test "$(sha256sum "$module" | awk '{print toupper($1)}')" = __MODULE_SHA__; test "$(sha256sum "$loader" | awk '{print toupper($1)}')" = __LOADER_SHA__; test "$(stat -Lc '%a' "$loader")" = 644; test ! -e "$out"; exec sudo -S -k -p '' /usr/bin/bash "$loader" "$module" __MODULE_SHA__ "$out"
'@
...
& $helperPath `
    ...
    -EvidenceKind ("R7_EXACT_PINNED_LOADER_{0}" -f $Role.ToUpperInvariant()) `
    -SendPasswordToStdin `
