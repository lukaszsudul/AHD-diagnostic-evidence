...
proc r1e_r3::write_text {path value} {
  file mkdir [file dirname $path]
  set fh [open $path w]
  fconfigure $fh -translation lf
...
proc r1e_r3::utilization_row {text wanted} {
...
proc r1e_r3::gate_open_design {routed_dcp output_root session_role} {
...
proc r1e_r3::run_complete_report_tail {output_root gate_info} {
  ...
  set reqp_result [r1e_semantic::gate [file join $output_root SEMANTIC_REQP_1839]]
  set reqp_1839_semantic_count [dict get $reqp_result semantic_count]
  set reqp_1839_raw_count [dict get $reqp_result raw_count]
  set route_errors -1
  ...
  if {$route_errors != 0 || $drc_errors != 0 || $drc_critical_warnings != 0 || $reqp_1839_semantic_count != 4 || $bus_skew_violations != 0 || $cdc_critical_types != 0 || $cdc_unknown_types != 0 || $wns < 0.617 || $whs < 0.021 || $vdo_wns < 0.0 || $vdo_whs <= 0.0 || $lut_free_pct < 10.0 || $ff_free_pct < 10.0 || $bram_free_pct < 10.0} {
  ...
  write_lines [file join $output_root READ_ONLY_REPORT_TAIL_GATE.txt] [list \
    ...
    "REQP_1839_SEMANTIC_VIOLATION_COUNT=$reqp_1839_semantic_count" \
    "REQP_1839_RAW_TEXT_OCCURRENCES=$reqp_1839_raw_count" \
    "RAW_TEXT_OCCURRENCES_USED_AS_GATE=NO" "BUS_SKEW_VIOLATIONS=$bus_skew_violations" \
