...
#define SHUTDOWN_GUARD_RECORDS 128U
#define SHUTDOWN_GUARD_BYTES ((size_t)SHUTDOWN_GUARD_RECORDS * (size_t)RECORD_BYTES)
#define TOTAL_LOGICAL_REQUESTS (PRIMARY_RECORDS + SHUTDOWN_GUARD_RECORDS)
#define TOTAL_ALLOCATED_BYTES (PRIMARY_BYTES + SHUTDOWN_GUARD_BYTES)
...
#define GUARD_CLEANUP_TIMEOUT_NS UINT64_C(12000000000)
...
static int is_progress_milestone(size_t completed) {
    if (completed == PRIMARY_RECORDS) return 1;
...
static void emit_primary_complete(Metrics *m) {
    ...
    m->guard_completed_at_primary = m->guard_completed_exact;
    size_t guard_submitted = m->submitted > PRIMARY_RECORDS
        ? m->submitted - PRIMARY_RECORDS : 0;
    size_t guard_pending = guard_submitted - m->guard_completed_exact - m->guard_canceled;
    printf("{\"type\":\"PRIMARY_WINDOW_COMPLETE\","
           "\"primary_exact_completions\":%zu,\"primary_short_completions\":%zu,"
           "\"primary_failed_completions\":%zu,\"primary_pending\":%zu,"
           "\"primary_bytes\":%zu,\"guard_completed\":%zu,"
           "\"guard_pending\":%zu,\"current_total_outstanding\":%zu,"
           ...
           PRIMARY_RECORDS - m->primary_completed, PRIMARY_BYTES,
           m->guard_completed_exact, guard_pending, m->outstanding,
...
static int process_event(const struct io_event *event, Request *requests,
    ...
    if (id64 >= TOTAL_LOGICAL_REQUESTS) {
        *blocker = "R3R4R6R2R4_COMPLETION_ID_OUT_OF_RANGE";
        return -1;
    }
    size_t id = (size_t)id64;
    Request *request = &requests[id];
    if (!request->accepted || request->completed) {
        *blocker = "R3R4R6R2R4_COMPLETION_ID_DUPLICATE_OR_UNSUBMITTED";
        return -1;
    }
    request->completed = 1;
    ...
    if (id < PRIMARY_RECORDS) {
        ...
        if (request->result == RECORD_BYTES && request->result2 == 0) {
            ...
            ++m->primary_short;
            *blocker = "R3R4R6R2R4_PRIMARY_SHORT_COMPLETION";
            return -1;
        } else {
            ++m->primary_failed;
            *blocker = "R3R4R6R2R4_PRIMARY_FAILED_COMPLETION";
            return -1;
        }
        ...
        if (request->result == RECORD_BYTES && request->result2 == 0) {
            ...
            ++m->guard_short;
            *blocker = "R3R4R6R2R4_GUARD_POSITIVE_SHORT_COMPLETION";
            return -1;
        } else {
            ++m->guard_failed;
            *blocker = "R3R4R6R2R4_GUARD_FAILED_COMPLETION";
            return -1;
        }
    }
    return 0;
...
static int persist_tables(const char *directory, const Request *requests,
                          ...
                          const unsigned char *guard, int success,
                          const char *blocker) {
    ...
    fd = open_exclusive(directory, "guard-cancellations.csv");
    if (fd < 0) return -1;
    file = fdopen(fd, "w");
    if (file == NULL) { close(fd); return -1; }
    fprintf(file, "LogicalIndex,CancelAttempted,Canceled,Completed,ResultBytes,Result2\n");
    for (size_t i = PRIMARY_RECORDS; i < TOTAL_LOGICAL_REQUESTS; ++i) {
        const Request *r = &requests[i];
        fprintf(file, "%zu,%s,%s,%s,%" PRId64 ",%" PRId64 "\n",
                i, r->cancel_attempted ? "YES" : "NO", r->canceled ? "YES" : "NO",
                r->completed ? "YES" : "NO", r->result, r->result2);
    }
    if (fflush(file) != 0 || fsync(fileno(file)) != 0 || fclose(file) != 0) return -1;
    ...
    fd = open_exclusive(directory, "guard-by-index.bin");
    if (fd < 0) return -1;
    if (ftruncate(fd, (off_t)SHUTDOWN_GUARD_BYTES) != 0) { close(fd); return -1; }
    for (size_t i = PRIMARY_RECORDS; i < TOTAL_LOGICAL_REQUESTS; ++i) {
        if (requests[i].result != RECORD_BYTES || requests[i].result2 != 0) continue;
        size_t offset = (i - PRIMARY_RECORDS) * (size_t)RECORD_BYTES;
        if (write_all_at(fd, guard + offset, RECORD_BYTES, (off_t)offset) != 0) {
            close(fd); return -1;
        }
    }
    if (fsync(fd) != 0 || close(fd) != 0) return -1;
    ...
    dprintf(fd,
        "{\n  \"schema\": \"R3R4R6R2R4_ROLLING_HELPER_RESULT_V1\",\n"
        "  \"result\": \"%s\",\n  \"blocker\": \"%s\",\n"
        "  \"record_bytes\": %u,\n  \"primary_records\": %u,\n"
...
static int synthetic_self_test(void) {
    ...
    size_t guard_completed_when_primary_emitted = 0;
    int persistence_started_before_disable_issued = 0;
    int persistence_started_after_disable_issued = 0;
    int persistence_started_before_guard_complete = 0;
    int guard_cancel_after_parent_quiescent = 0;
    unsigned char cancel_target[TOTAL_LOGICAL_REQUESTS];
    memset(cancel_target, 0, sizeof(cancel_target));
    ...
    while (total_completed < TOTAL_LOGICAL_REQUESTS) {
        ...
        size_t taken = 0;
        for (size_t pass = 0; pass < 2 && taken < batch; ++pass) {
            size_t begin = pass == 0 ? 0 : PRIMARY_RECORDS;
            size_t end = pass == 0 ? PRIMARY_RECORDS : TOTAL_LOGICAL_REQUESTS;
            for (size_t i = begin; i < end && taken < batch; ++i) {
                if (!active[i] || completed[i]) continue;
                completed[i] = 1;
                active[i] = 0;
                ++taken; ++total_completed; --outstanding;
                if (i < PRIMARY_RECORDS) ++primary_completed;
                if (primary_completed == PRIMARY_RECORDS && !primary_event) {
                    primary_event = 1;
                    primary_completed_when_event_emitted = primary_completed;
                    guard_completed_when_primary_emitted =
                        total_completed - primary_completed;
                }
            }
        }
    ...
    if (disable_issued) {
        ...
        persistence_started_before_guard_complete =
            guard_completed_when_primary_emitted < SHUTDOWN_GUARD_RECORDS;
    } else {
        persistence_started_before_disable_issued = 1;
    }
    int parent_quiescent = 1;
    if (parent_quiescent) guard_cancel_after_parent_quiescent = 1;
    int guard_cancel_only = 1;
    for (size_t i = 0; i < TOTAL_LOGICAL_REQUESTS; ++i) {
        if ((i < PRIMARY_RECORDS && cancel_target[i]) ||
            (i >= PRIMARY_RECORDS && !cancel_target[i]))
            guard_cancel_only = 0;
    }
    ...
    int pass = submitted == TOTAL_LOGICAL_REQUESTS &&
               ...
               primary_completed_when_event_emitted == PRIMARY_RECORDS && placement_ok &&
               !persistence_started_before_disable_issued &&
               persistence_started_after_disable_issued &&
               persistence_started_before_guard_complete && guard_cancel_only &&
               guard_cancel_after_parent_quiescent &&
    ...
    printf("{\"type\":\"SELF_TEST_RESULT\",\"result\":\"%s\","
           ...
           "\"persistence_started_before_guard_completion\":%s,"
           "\"guard_cancel_targets_only\":%s,"
           "\"guard_cancel_after_parent_quiescent\":%s,"
           ...
           persistence_started_before_guard_complete ? "true" : "false",
           guard_cancel_only ? "true" : "false",
           guard_cancel_after_parent_quiescent ? "true" : "false",
...
int main(int argc, char **argv) {
    ...
    if (posix_memalign((void **)&primary, RECORD_BYTES, PRIMARY_BYTES) != 0 ||
        posix_memalign((void **)&guard, RECORD_BYTES, SHUTDOWN_GUARD_BYTES) != 0) {
        emit_error("R3R4R6R2R4_BUFFER_ALLOCATION_FAILED", 0, -ENOMEM, 0, 0);
        free(primary); free(guard); return 70;
    }
    memset(primary, 0, PRIMARY_BYTES);
    memset(guard, 0, SHUTDOWN_GUARD_BYTES);
    ...
    if (requests == NULL || pointers == NULL) {
        emit_error("R3R4R6R2R4_CONTROL_ALLOCATION_FAILED", 0, -ENOMEM, 0, 0);
        free(requests); free(pointers); free(primary); free(guard); return 70;
    }
    ...
    if (c2h < 0) {
        emit_error("R3R4R6R2R4_C2H_OPEN_FAILED", 0, -errno, 0, 0);
        free(requests); free(pointers); free(primary); free(guard); return 71;
    }
    for (size_t i = 0; i < TOTAL_LOGICAL_REQUESTS; ++i) {
        unsigned char *buffer = i < PRIMARY_RECORDS
            ? primary + i * (size_t)RECORD_BYTES
            : guard + (i - PRIMARY_RECORDS) * (size_t)RECORD_BYTES;
    ...
    if (syscall(__NR_io_setup, AIO_CONTEXT_CAPACITY, &context) < 0) {
        emit_error("R3R4R6R2R4_IO_SETUP_FAILED", 0, -errno, 0, 0);
        close(c2h); free(requests); free(pointers); free(primary); free(guard); return 72;
    }
    ...
    if (submit_rc != 0 || m.submitted != MAX_OUTSTANDING_IOCBS ||
        ...
        blocker = submit_rc == -2 ? "R3R4R6R2R4_DESCRIPTOR_WINDOW_VIOLATION" :
                                   "R3R4R6R2R4_INITIAL_PREQUEUE_FAILED";
        failure = 1;
    }
    ...
    if (!failure && early != 0) {
        blocker = "R3R4R6R2R4_COMPLETION_BEFORE_ENABLE";
        failure = 1;
    }
    if (failure) {
        ...
        printf("{\"type\":\"ROLLING_PREQUEUE_READY\","
               ...
               "\"primary_alignment_remainder\":%zu,\"guard_alignment_remainder\":%zu,"
               ...
               (size_t)((uintptr_t)primary % RECORD_BYTES),
               (size_t)((uintptr_t)guard % RECORD_BYTES), monotonic_ns());
        fflush(stdout);
    }
    ...
    while (!failure && !m.primary_event_emitted) {
        ...
        if (now - m.capture_start_ns >= TOTAL_CAPTURE_TIMEOUT_NS) {
            blocker = "R3R4R6R2R4_TOTAL_CAPTURE_TIMEOUT";
            failure = 1;
            break;
        }
        if (now - m.last_completion_ns >= NO_PROGRESS_TIMEOUT_NS) {
            blocker = "R3R4R6R2R4_NO_PROGRESS_TIMEOUT";
            failure = 1;
            break;
        }
        ...
        if (count < 0) {
            ...
            blocker = "R3R4R6R2R4_IO_GETEVENTS_FAILED";
            failure = 1;
            break;
        }
        if (count == 0) continue;
        ...
        if (m.next_submit < TOTAL_LOGICAL_REQUESTS) {
            ...
            if (rc != 0) {
                blocker = rc == -2 ? "R3R4R6R2R4_DESCRIPTOR_WINDOW_VIOLATION" :
                                     "R3R4R6R2R4_ROLLING_REFILL_FAILED";
                failure = 1;
                break;
            }
        }
    ...
    if (!failure && (!m.primary_event_emitted || m.primary_exact != PRIMARY_RECORDS)) {
        blocker = "R3R4R6R2R4_PRIMARY_WINDOW_INCOMPLETE";
        failure = 1;
    ...
    while (!parent_quiescent) {
        if (read_parent_command(&disable_issued, &parent_quiescent) < 0) {
            if (!failure) {
                blocker = "R3R4R6R2R4_PARENT_COMMAND_INVALID";
                failure = 1;
        ...
        if (!failure && m.primary_event_emitted && parent_quiescent &&
            !disable_issued) {
            blocker = "R3R4R6R2R4_PARENT_QUIESCENT_BEFORE_DISABLE_ISSUED";
            failure = 1;
        ...
        if (!failure && disable_issued && !m.primary_durable) {
            if (persist_primary_full(argv[2], primary, &m) != 0) {
                blocker = "R3R4R6R2R4_PRIMARY_PERSISTENCE_FAILED";
                failure = 1;
        ...
        if (count < 0 && errno != EINTR) {
            if (!failure) {
                blocker = "R3R4R6R2R4_IO_GETEVENTS_DRAIN_FAILED";
                failure = 1;
    ...
    uint64_t cleanup_deadline = monotonic_ns() + GUARD_CLEANUP_TIMEOUT_NS;
    ...
    if (cleanup_unresolved) {
        blocker = failure ? "R3R4R6R2R4_FAILURE_AIO_CLEANUP_UNRESOLVED" :
                            "R3R4R6R2R4_GUARD_AIO_CLEANUP_UNRESOLVED";
        failure = 1;
    }
    if (persist_tables(argv[2], requests, &m, primary, guard,
                       !failure && m.primary_durable, blocker) != 0) {
        blocker = "R3R4R6R2R4_METADATA_PERSISTENCE_FAILED";
        failure = 1;
    }
    if (cleanup_unresolved) {
    ...
    double average_refill_us = m.refill_latency_samples == 0 ? 0.0 :
        (double)m.total_refill_latency_ns / (double)m.refill_latency_samples / 1000.0;
    printf("{\"type\":\"GUARD_CLEANUP_COMPLETE\","
           ...
           average_refill_us, monotonic_ns());
    fflush(stdout);

    int destroy_failed = syscall(__NR_io_destroy, context) < 0;
    close(c2h);
    free(requests); free(pointers); free(primary); free(guard);
    if (destroy_failed) {
        emit_error("R3R4R6R2R4_IO_DESTROY_FAILED", 0, -errno, 0, 0);
        return 75;
    }
