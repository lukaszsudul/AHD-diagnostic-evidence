#!/usr/bin/env python3
"""Parent-owned MMIO controller for the one-shot R3R4R6R2R4 4-KiB capture."""
...
def source_readiness(mmio: Mmio) -> dict[str, Any]:
    require(mmio.read(0x3800, "READINESS") == 0x43324831,
            "R3R4R6R2R4_G2B_C2H_MAGIC_ABSENT")
    require(mmio.read(0x3804, "READINESS") == 0x00010000,
            "R3R4R6R2R4_TRANSPORT_ABI_VERSION_NOT_1")
    ...
    while time.monotonic() < deadline:
        ...
        before = after
    raise GateError("R3R4R6R2R4_LIVE_SOURCE_NOT_READY:" +
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    result: dict[str, Any] = {
        "schema": "R3R4R6R2R4_CONTROLLER_RESULT_V1", "task": TASK,
    ...
    try:
        ...
        require(Path(args.native_helper).is_file(),
                "R3R4R6R2R4_NATIVE_HELPER_MISSING")
        ...
        require(hardware_quiescent(control, status),
                "R3R4R6R2R4_S0_NOT_PHYSICALLY_QUIESCENT")
        ...
        while True:
            ...
            require(time.monotonic() < reset_deadline,
                    "R3R4R6R2R4_RESET_STREAM_STATE_TIMEOUT")
            time.sleep(0.001)
