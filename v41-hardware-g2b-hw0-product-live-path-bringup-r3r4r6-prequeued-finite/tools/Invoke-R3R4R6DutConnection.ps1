...
$run = 'C:\FPGA\G2B_HW0_PRODUCT_R3R4R6_20260907T174249Z'
$linuxRun = '/home/vcdeagent1/vcde_artifacts/g2b_hw0_product_r3r4r6/20260907T174249Z'
...
function Assert-PrivateAcl([string]$Path) {
  ...
  if (-not $acl.AreAccessRulesProtected -or $rules.Count -ne 1 -or
    ...
    throw 'R3R4R6_PRIVATE_ACL_INVALID'
  }
}

if ([IO.Path]::GetFullPath($PSScriptRoot) -cne (Join-Path $run 'scripts')) {
  throw 'R3R4R6_HELPER_LOCATION_INVALID'
}
if (Test-Path -LiteralPath $receipt) { throw 'R3R4R6_RECEIPT_EXISTS' }
...
if (@(Get-ChildItem -LiteralPath $credentialTemp -Force).Count) {
  throw 'R3R4R6_CREDENTIAL_TEMP_NOT_EMPTY'
}
if ((Get-FileHash -LiteralPath $plink).Hash -cne $plinkHash) {
  throw 'R3R4R6_PLINK_HASH_MISMATCH'
}
...
try {
  ...
  foreach ($line in [IO.File]::ReadAllLines('C:\FPGA\VCDE-DUT-1.txt')) {
    ...
    if ($line -notmatch '^\s*([^:=]+?)\s*[:=]\s*(.*?)\s*$') {
      throw 'R3R4R6_CREDENTIAL_FORMAT'
    }
    ...
    $key = switch ($label) {
      ...
      default { throw 'R3R4R6_CREDENTIAL_FIELD' }
    }
    if ($fields.ContainsKey($key)) { throw 'R3R4R6_DUPLICATE_CREDENTIAL_FIELD' }
    $fields[$key] = $val
  }
  if ($fields.Count -ne 3 -or $fields.ip -cne $ip -or $fields.user -cne $login) {
    throw 'R3R4R6_CREDENTIAL_IDENTITY'
  }
  $pw = [string]$fields.password
  $fields.Clear()
  $val = $null
  $line = $null
  if ([string]::IsNullOrWhiteSpace($pw)) { throw 'R3R4R6_EMPTY_CREDENTIAL' }
  ...
  for ($i=0; $i -lt $argsList.Count; $i++) {
    if ($argsList[$i] -ceq '-pw') { throw 'R3R4R6_SECRET_ARGUMENT' }
    if ($i -ne 6 -and $argsList[$i].Contains($pw,[StringComparison]::Ordinal)) {
      throw 'R3R4R6_SECRET_ARGUMENT'
    }
  ...
  $proc.StartInfo = $psi
  $started = $proc.Start()
  if (-not $started) { throw 'R3R4R6_PROCESS_START' }
  ...
  if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    $proc.WaitForExit()
    $exitCode = 124
    throw 'R3R4R6_CONNECTION_TIMEOUT'
  }
  $stdout = $outTask.GetAwaiter().GetResult()
  $stderr = $errTask.GetAwaiter().GetResult()
  $exitCode = $proc.ExitCode
} catch {
  $problem = if ($_.Exception.Message -like 'R3R4R6_*') {
    $_.Exception.Message
  } else {
    'R3R4R6_SANITIZED_LOCAL_EXCEPTION'
  }
} finally {
  if ($proc) {
...
if (-not $deleted) { $exitCode=126; $problem='R3R4R6_CREDENTIAL_DELETE_FAILED' }
$remaining = @(Get-ChildItem -LiteralPath $credentialTemp -Force).Count
if ($remaining -ne 0) { $exitCode=127; $problem='R3R4R6_CREDENTIAL_REMNANT' }
$result = [ordered]@{
  ...
  temporary_path_class='R3R4R6_PRIVATE_CREDENTIAL_TEMP'
...
$pw = $null
Write-Output $json
if ($exitCode -ne 0 -or $problem) { throw 'R3R4R6_CONNECTION_FAILED_SEE_RECEIPT' }
