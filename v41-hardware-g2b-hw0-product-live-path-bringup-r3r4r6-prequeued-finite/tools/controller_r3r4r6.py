#!/usr/bin/env python3
"""Parent-owned MMIO controller for one R3R4R6 prequeued finite capture."""
...
def json_safe_event(event: dict[str, Any]) -> None:
    ...
    require(all(isinstance(value, allowed) for value in event.values()),
            "R3R4R6_NATIVE_EVENT_NOT_METADATA_ONLY")
...
class Mmio:
    ...
    def read(self, offset: int, label: str) -> int:
        require(offset % 4 == 0 and any(lo <= offset <= hi
                ...
                "R3R4R6_MMIO_READ_ALLOWLIST_VIOLATION")
        stamp = time.time_ns()
        data = os.pread(self.fd, 4, offset)
        require(len(data) == 4, "R3R4R6_SHORT_MMIO_READ")
    ...
    def write(self, offset: int, value: int, purpose: str,
        ...
        require(count == 4, "R3R4R6_SHORT_MMIO_WRITE")
    ...
    def snapshot(self, label: str) -> dict[str, Any]:
        ...
        require((self.read(0x3810, label) & 0x300) == 0,
                "R3R4R6_SNAPSHOT_OR_RESET_BUSY")
        self.write(0x3844, 1, "COHERENT_SNAPSHOT", label)
        deadline = time.monotonic() + 2.0
        while (self.read(0x3848, label) & 3) != 2:
            require(time.monotonic() < deadline,
                    "R3R4R6_COHERENT_SNAPSHOT_TIMEOUT")
            time.sleep(0.001)
        counters = self.counters(label)
        require(self.read(0x3838, label) == epoch,
                "R3R4R6_SNAPSHOT_EPOCH_CHANGED")
        expected_generation = (generation + 1) & 0xFFFFFFFF
        require(self.read(0x384C, label) == expected_generation,
                "R3R4R6_SNAPSHOT_GENERATION_INVALID")
        return {
...
def read_native_event(process: subprocess.Popen[str], selector: selectors.BaseSelector,
    ...
    while time.monotonic() < deadline:
        ...
        if ready:
            ...
            require(bool(line), "R3R4R6_NATIVE_HELPER_STDOUT_CLOSED")
            event = json.loads(line)
            require(isinstance(event, dict) and isinstance(event.get("type"), str),
                    "R3R4R6_NATIVE_EVENT_SCHEMA_INVALID")
            json_safe_event(event)
            return event
        if process.poll() is not None:
            raise GateError("R3R4R6_NATIVE_HELPER_EXITED_BEFORE_REQUIRED_EVENT")
    raise GateError("R3R4R6_NATIVE_HELPER_EVENT_TIMEOUT")
    ...
    while True:
        ...
        if event["type"] == "ERROR":
            raise GateError(str(event.get("blocker") or
                                "R3R4R6_NATIVE_HELPER_UNSPECIFIED_ERROR"))
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    result: dict[str, Any] = {
        "schema": "R3R4R6_CONTROLLER_RESULT_V1", "task": TASK,
    ...
    try:
        require(Path(args.linux_lock).is_dir(),
                "R3R4R6_LINUX_TASK_LOCK_NOT_HELD")
        ...
        timeline.append(s0)
        result["S0"] = s0
        require((s0["Epoch"], s0["ErrorStatus"], s0["LastErrorCause"]) ==
                (3, 0x00000007, 0x00000003),
                "OWNER_ATTESTED_RUNTIME_STATE_CONTRADICTED_BY_S0")
        ...
        while True:
            ...
            require(time.monotonic() < reset_deadline,
                    "R3R4R6_RESET_STREAM_STATE_TIMEOUT")
            time.sleep(0.001)
        s1 = mmio.checkpoint("S1")
        timeline.append(s1)
        result["S1"] = s1
        require(s1["Epoch"] == 4,
                "R3R4R6_RESET_EPOCH_3_TO_4_FAILED")
        ...
        require((post_normalization & 0x3F) == 0,
                "R3R4R6_SESSION_NORMALIZATION_DID_NOT_CLEAR")
        ...
        require(process.stdout is not None, "R3R4R6_NATIVE_STDOUT_PIPE_MISSING")
        ...
        require(prequeue.get("primary_bytes") == PRIMARY_BYTES and
                ...
                "R3R4R6_PREQUEUE_READY_CONTRACT_FAILED")
        ...
        require(parent_quiescent,
                "R3R4R6_PARENT_HARDWARE_QUIESCENCE_NOT_PROVEN")
        ...
        require(process.stdin is not None, "R3R4R6_NATIVE_STDIN_PIPE_MISSING")
        process.stdin.write("PARENT_QUIESCENT\n")
        process.stdin.flush()
        process.stdin.close()

        native_deadline = time.monotonic() + 60.0
        ...
        require(not stderr.strip(), "R3R4R6_NATIVE_HELPER_STDERR_NONEMPTY")
        require(process.returncode == 0,
                "R3R4R6_NATIVE_HELPER_EXIT_FAILED")
