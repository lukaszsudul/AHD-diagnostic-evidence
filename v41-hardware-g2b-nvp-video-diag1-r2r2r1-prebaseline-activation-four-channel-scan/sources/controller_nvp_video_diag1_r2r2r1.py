...
from controller_nvp_capture_r2r2r1 import Mmio as ProductMmio
from controller_nvp_capture_r2r2r1 import hardware_quiescent
from controller_nvp_capture_r2r2r1 import runtime_identity as read_product_identity
...
def require_exact_c2h_unowned(path: str) -> dict[str, Any]:
    ...
    require_unblocked(Path(executable).is_file(),
                      "NVP_DIAG1_R2R2R1_EXACT_C2H_HOLDER_TOOL_UNAVAILABLE")
    ...
    require_unblocked(completed.returncode in (0, 1),
                      "NVP_DIAG1_R2R2R1_EXACT_C2H_HOLDER_QUERY_FAILED")
    require_unblocked(completed.returncode == 1 and not holders,
                      "NVP_DIAG1_R2R2R1_PREBASELINE_C2H_HOLDER_PRESENT")
    return {
        "device": path,
...
def execute_prepare(mmio: DiagnosticMmio, baseline_root: Path,
    ...
    require_unblocked(observed_start_count == expected_start_count,
                      f"NVP_DIAG1_R2R2R1_PREPARE_{name}_START_COUNT_DRIFT")
    ...
    while time.monotonic() < deadline:
        ...
        if bits["error"]:
            ...
            raise BlockedError(
                f"NVP_DIAG1_R2R2R1_PREPARE_{name}_FIRMWARE_ERROR:"
                f"0x{code:08X}")
        ...
        raise BlockedError(
            f"NVP_DIAG1_R2R2R1_PREPARE_{name}_FRESH_EXECUTION_TIMEOUT:"
    ...
    require_unblocked(bits["prepared"] and bits["baseline_restored"],
                      f"NVP_DIAG1_R2R2R1_PREPARE_{name}_STATUS_INVALID")
    ...
    require_unblocked(first == second,
                      f"NVP_DIAG1_R2R2R1_PREPARE_{name}_HOST_DOUBLE_READ_MISMATCH")
    require_unblocked(first["diag_state"] == 0 and first["diag_error"] == 0 and
                      ...
                      f"NVP_DIAG1_R2R2R1_PREPARE_{name}_ERROR_STATE")
    ...
    require_unblocked(checked["prepared"] and checked["baseline_restored"] and
                      ...
                      f"NVP_DIAG1_R2R2R1_PREPARE_{name}_COMPLETION_STATE_INVALID")
...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            pre_control = product_mmio.read(0x380C, "R2R2R1_PREBASELINE")
            pre_status = product_mmio.read(0x3810, "R2R2R1_PREBASELINE")
            require_unblocked(hardware_quiescent(pre_control, pre_status),
                              "NVP_DIAG1_R2R2R1_PREBASELINE_NOT_QUIESCENT")
            ...
            require(not autoinit_identity["busy"] and
                    ...
                    "NVP_DIAG1_R2R2R1_AUTOINIT_IDENTITY_MISMATCH")
        finally:
            product_mmio.close()
        ...
        require_unblocked(c1 > c0 and c2 > c1,
                          "NVP_DIAG1_R2R2R1_PREPARE_TRANSACTION_COUNTER_NOT_"
                          "STRICTLY_INCREASING")
        delta_a = c1 - c0
        delta_b = c2 - c1
        require_unblocked(delta_a == delta_b,
                          "NVP_DIAG1_R2R2R1_PREPARE_TRANSACTION_SEQUENCE_MISMATCH")
        require_unblocked(
                        ...
                        if not response_withheld_for_unsafe_quiescence:
                            ...
                            require(restored["baseline_restored"] and
                                    ...
                                    (mmio.read(0x3C40,
                                        ...
                                        (baseline_b["bgcolor_79"] & 0xFF),
                                    "NVP_DIAG1_R2R2R1_PRODUCT_BASELINE_RESTORE_FAILED")
