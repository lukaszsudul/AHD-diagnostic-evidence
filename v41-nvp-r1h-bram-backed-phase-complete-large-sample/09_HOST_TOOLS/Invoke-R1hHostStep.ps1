...
switch ($Step) {
    ...
    'PreLoaderValidate' {
        ...
        $expectedAdaptedSha=switch($PhaseToken){'Bootstrap'{'E93581DBCA64A81AAF1E87A4E1CC9C71AF061D09F6F627EE259162BEC234DCA2'}'A1'{'06413244CBD88D86CDBD48DA5E1A72F4DA3B64D432867143108494E9BA1D67E9'}'B1'{'5E21E15DF6D63539A52BE2AA24F0FFEC3EFD2452EE0788CC9E811A4BBF26ED95'}'A2'{'4CD712CD3122294AA0E7CBAF93B032BC0B638B16A705471E74C2FE16E38E1C9F'}'B2'{'ED36B35BEB3699C407BDADBB5B18064C6969D694FC44F9F926ED415EAB69FB6A'}'A3'{'F9CE79E127618F67C665552FE2A465E84F8023B9E0CD117E82F3E2492287C902'}'B3'{'27E3D6E171C93BD41A7346E3D342CCAEE91214352DFA016351B129752778C62E'}}
    ...
    'ExactDriverLoad' {
        ...
        $remoteTemplate=@'
set -eu; module="$HOME/FPGA_AHD_HOST/dma_ip_drivers/XDMA/linux-kernel/xdma/xdma.ko"; loader="$HOME/FPGA_AHD_HOST/phase2_precheck_accepted/phase2_load_xdma_driver.sh"; out="$HOME/FPGA_AHD_HOST/v41_nvp_r1h/__DRIVER_DIRECTORY__"; test -f "$module"; test ! -L "$module"; test -f "$loader"; test ! -L "$loader"; test "$(sha256sum "$module" | awk '{print toupper($1)}')" = __MODULE_SHA__; test "$(sha256sum "$loader" | awk '{print toupper($1)}')" = __LOADER_SHA__; test "$(stat -Lc '%a' "$loader")" = 644; test ! -e "$out"; exec sudo -S -k -p '' /usr/bin/bash "$loader" "$module" __MODULE_SHA__ "$out"
'@
