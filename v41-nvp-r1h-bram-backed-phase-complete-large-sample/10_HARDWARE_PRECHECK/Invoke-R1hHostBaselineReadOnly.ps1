...
$payloadPath=$script:R1hAcceptedTools.HostBaselinePayload.Path
$payload=ConvertTo-GzipBase64([IO.File]::ReadAllBytes($payloadPath))
$hostKey='SHA256:yunI1fwP5I6WfGcSVkyaPxd0siCbdSiOOXVrP0wtEu8'
$paths=@(
    (Join-Path $script:R1hPrecheckRoot 'HOST_BASELINE_SESSION_1.log'),
    (Join-Path $script:R1hPrecheckRoot 'HOST_BASELINE_SESSION_2.log')
)
$matrixPath=Join-Path $script:R1hPrecheckRoot 'R1H_HOST_BASELINE_MATRIX.csv'
$gatePath=Join-Path $script:R1hPrecheckRoot 'R1H_HOST_BASELINE_GATE.txt'
...
for($session=1;$session-le2;$session++){
    ...
    &$helper -PlinkPath $script:R1hAcceptedTools.Plink084.Path -HostKey $hostKey -RemoteCommand $remoteCommand `
        -EvidencePath $paths[$session-1] -ExpectedIp '10.132.1.111' -ExpectedUser 'vcdeagent1' `
        -EvidenceKind("R1H_HOST_BASELINE_SESSION_{0}"-f$session) -SendPasswordToStdin -SudoPasswordCopies 1 -TimeoutSeconds 120
...
Write-R1hUtf8NoBom -Path $matrixPath -Lines $csv.ToArray()
$gate=if($failures.Count-eq0){'PASS_2_OF_2'}else{'FAIL'}
$lines=[Collections.Generic.List[string]]::new();foreach($line in @("R1H_HOST_BASELINE=$gate","R1H_BOOT_ID_BASELINE=$($a.BootId)",'READ_ONLY_SSH_SESSIONS=2',"NEXT_REBOOT_KERNEL_PROVEN=$($a.NextKernel)","REMOTE_UPTIME_SPAN_SECONDS=$remoteSpan","LOCAL_MONOTONIC_SPAN_SECONDS=$localSpan",'NO_OBSERVED_REBOOT_OR_SHUTDOWN='+$(if($gate-eq'PASS_2_OF_2'){'YES'}else{'NOT_PROVEN'}),"HOST_SAMPLE_PAYLOAD_SHA256=$($script:R1hAcceptedTools.HostBaselinePayload.Sha256)")){$lines.Add($line)};foreach($f in $failures){$lines.Add("FAILURE=$f")}
Write-R1hUtf8NoBom -Path $gatePath -Lines $lines.ToArray();$lines
