...
if {$phase ni {before after}} {error "invalid phase"}
...
if {[catch {
 ...
 if {[llength $ts] != 1} {error "target count"}
 set target [lindex $ts 0]
 if {![string match */Xilinx/80802026a98b01 $target]} {error "target identity"}
 ...
 puts $f {sample,utc,target,part,idcode,done}
 for {set i 0} {$i<5} {incr i} {
  ...
  puts $f "$i,[clock format [clock seconds] -format %Y-%m-%dT%H:%M:%SZ -gmt 1],$target,$part,$id,$done"
  flush $f
  if {$part ne "xc7a35t" || $id ne "0362D093" || $done ne "1"} {error "identity or DONE mismatch $part $id $done"}
  after 500
 }
 close $f
 puts "JTAG_GATE=PASS"
