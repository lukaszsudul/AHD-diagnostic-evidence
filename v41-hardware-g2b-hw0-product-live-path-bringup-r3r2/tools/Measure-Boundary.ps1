...
$roots+=@(Get-ChildItem C:\FPGA\V41_G2B_EVIDENCE -Directory | Where-Object {$_.Name -match '^(project-current-state|v41-)' -and $_.Name -ne 'v41-hardware-g2b-hw0-product-live-path-bringup-r3r2'} | Select-Object -ExpandProperty FullName)
$rows=@(foreach($r in $roots){foreach($f in @((Get-Item -LiteralPath $r)) + @(Get-ChildItem -LiteralPath $r -Force -Recurse)){
 [pscustomobject]@{path=$f.FullName;directory=$f.PSIsContainer;size=if($f.PSIsContainer){0}else{$f.Length};created=$f.CreationTimeUtc.ToString('o');modified=$f.LastWriteTimeUtc.ToString('o');sha256=if(-not $f.PSIsContainer -and $f.FullName -notmatch '[\\/]secret[\\/]|[\\/]private[\\/]' -and $f.Name -match '(manifest|receipt|index)' -and $f.Length -lt 1000000){(Get-FileHash -LiteralPath $f.FullName).Hash}else{'METADATA_ONLY'}}
}})
