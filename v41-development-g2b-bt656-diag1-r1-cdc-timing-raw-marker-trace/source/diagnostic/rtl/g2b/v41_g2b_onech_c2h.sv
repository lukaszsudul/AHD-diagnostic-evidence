...
module v41_g2b_onech_c2h (
  ...
  // DIAG1 is a strictly observational tap. These signals are not consumed
  // anywhere in the functional parser/transport feedback path.
  output logic         bt656_diag_event_valid,
  output logic         bt656_diag_trigger_pulse,
  output logic         bt656_diag_next_frame_line1_commit,
  output logic [511:0] bt656_diag_event_payload,
  ...
  // Keep the public diagnostic schema independent of implementation-time FSM
  // recoding. Vivado may choose any physical state-register encoding, while
  // the event trace must always publish the frozen RTL values 0 through 4.
  function automatic logic [2:0] bt656_diag_state_code(
    input source_state_t state_value);
    begin
      case (state_value)
        SRC_IDLE:     bt656_diag_state_code = 3'd0;
        SRC_CAPTURE:  bt656_diag_state_code = 3'd1;
        SRC_WAIT_EAV: bt656_diag_state_code = 3'd2;
        SRC_HEADER:   bt656_diag_state_code = 3'd3;
        SRC_COMMIT:   bt656_diag_state_code = 3'd4;
        default:      bt656_diag_state_code = 3'd7;
      endcase
    end
  endfunction
  ...
  logic marker_event_pipe = 1'b0;
  logic marker_xy_valid_pipe = 1'b0;
  logic marker_f_pipe = 1'b0;
  logic marker_h_pipe = 1'b0;
  logic marker_v_pipe = 1'b0;
  logic [31:0] marker_bytes_pipe = '0;
  ...
  // ----------------------------------------------------------------------
  // DIAG1 passive event projection. Every predicate below is derived from
  // the pre-edge functional state and exactly mirrors an existing parser or
  // admission decision. Nothing in SOURCE_DOMAIN depends on these signals.
  // ----------------------------------------------------------------------
  logic         bt656_diag_event_valid_d;
  logic         bt656_diag_trigger_pulse_d;
  logic         bt656_diag_next_frame_line1_commit_d;
  logic [511:0] bt656_diag_event_payload_d;

  always_comb begin : BT656_DIAG1_OBSERVATION
    source_state_t state_after;
    logic locked_after;
    logic malformed_increment;
    logic source_drop_increment;
    logic commit_pulse;
    logic [3:0] malformed_reason;
    logic [3:0] drop_reason;
    logic any_filling;
    logic successful_eav;
    logic successful_commit;
    integer diag_slot_index;

    state_after = source_state;
    locked_after = source_locked_source;
    malformed_increment = 1'b0;
    source_drop_increment = 1'b0;
    commit_pulse = 1'b0;
    malformed_reason = 4'd0;
    drop_reason = 4'd0;
    any_filling = 1'b0;
    for (diag_slot_index = 0; diag_slot_index < 4;
         diag_slot_index = diag_slot_index + 1)
      if (slot_state_source[diag_slot_index] == SLOT_FILLING)
        any_filling = 1'b1;

    successful_eav = source_state == SRC_WAIT_EAV && marker_valid_pipe &&
                     marker_h_pipe && post_payload_count == 3 &&
                     pending_line <= 32'd1079;
    successful_commit = source_state == SRC_COMMIT &&
                        slot_state_source[filling_slot] == SLOT_FILLING &&
                        slot_generation_source[filling_slot] ==
                            filling_generation &&
                        reset_epoch_source == pending_epoch;

    if (transport_req_sync2_source != transport_req_seen_source) begin
      state_after = SRC_IDLE;
      // The transport request toggle is source-synchronized.  Its AXI-held
      // hard/normal qualifier is intentionally absent from this diagnostic
      // cone: reset classification is not required by the frame-boundary
      // trace, and consuming the raw qualifier created an unsafe userclk1 to
      // source-clock trace dependency.  Any in-flight filling slot remains
      // observable as a source-local reset/readiness-abort diagnostic.
      if (any_filling) begin
        source_drop_increment = 1'b1;
        drop_reason = 4'd3;
      end
      if (source_reset)
        locked_after = 1'b0;
      ...
      case (source_state)
        SRC_IDLE: begin
          if (marker_valid_pipe && !marker_h_pipe && !marker_v_pipe) begin
            state_after = SRC_CAPTURE;
            if (enable_applied_source && source_ready_live_source &&
                source_locked_source && !fatal_sync2_source &&
                !source_formatter_fatal && !source_ownership_fatal &&
                !allocation_choice[2]) begin
              source_drop_increment = 1'b1;
              drop_reason = 4'd1;
            end
          end
        ...
        SRC_CAPTURE: begin
          if (marker_valid_pipe) begin
            state_after = SRC_IDLE;
            locked_after = 1'b0;
            malformed_increment = 1'b1;
            malformed_reason = 4'd1;
            if (monitor_has_attempt && !monitor_ring_drop) begin
              source_drop_increment = 1'b1;
              drop_reason = 4'd2;
            end
          end else if (payload_byte_phase == 7 &&
                       payload_byte_count == PAYLOAD_BYTES-1) begin
            state_after = SRC_WAIT_EAV;
          end
        end
        SRC_WAIT_EAV: begin
          if (successful_eav) begin
            locked_after = 1'b1;
            if (monitor_has_attempt && monitor_writes_slot)
              state_after = SRC_HEADER;
            else
              state_after = SRC_IDLE;
          end else if (marker_valid_pipe) begin
            state_after = SRC_IDLE;
            locked_after = 1'b0;
            malformed_increment = 1'b1;
            malformed_reason = 4'd2;
            if (monitor_has_attempt && !monitor_ring_drop) begin
              source_drop_increment = 1'b1;
              drop_reason = 4'd2;
            end
        ...
        SRC_HEADER: begin
          if (header_index == 7)
            state_after = SRC_COMMIT;
        end
        SRC_COMMIT: begin
          state_after = SRC_IDLE;
          if (successful_commit)
            commit_pulse = 1'b1;
          else if (monitor_has_attempt && !monitor_ring_drop) begin
            source_drop_increment = 1'b1;
            drop_reason = 4'd4;
          end
        end
    ...
    bt656_diag_trigger_pulse_d = successful_eav && pending_line == 32'd1079;
    bt656_diag_next_frame_line1_commit_d =
        commit_pulse && pending_line == 32'd1;
    bt656_diag_event_valid_d = marker_event_pipe || malformed_increment ||
                               source_drop_increment || commit_pulse ||
                               bt656_diag_trigger_pulse_d;

    bt656_diag_event_payload_d = '0;
    bt656_diag_event_payload_d[2*32 +: 32] = marker_bytes_pipe;
    bt656_diag_event_payload_d[3*32 + 0] = marker_event_pipe;
    bt656_diag_event_payload_d[3*32 + 1] = marker_valid_pipe;
    bt656_diag_event_payload_d[3*32 + 2] = marker_f_pipe;
    bt656_diag_event_payload_d[3*32 + 3] = marker_v_pipe;
    bt656_diag_event_payload_d[3*32 + 4] = marker_h_pipe;
    bt656_diag_event_payload_d[3*32 + 5] = marker_xy_valid_pipe;
    bt656_diag_event_payload_d[3*32 + 6] = malformed_increment;
    bt656_diag_event_payload_d[3*32 + 7] = source_drop_increment;
    bt656_diag_event_payload_d[3*32 + 8] = commit_pulse;
    bt656_diag_event_payload_d[3*32 + 9] = bt656_diag_trigger_pulse_d;
    bt656_diag_event_payload_d[3*32 + 10] = source_ready_live_source;
    bt656_diag_event_payload_d[3*32 + 11] = enable_applied_source;
    bt656_diag_event_payload_d[3*32 + 12] = previous_sav_v;
    bt656_diag_event_payload_d[3*32 + 13] = source_locked_source;
    bt656_diag_event_payload_d[3*32 + 14] = locked_after;
    bt656_diag_event_payload_d[3*32 + 15] = monitor_has_attempt;
    bt656_diag_event_payload_d[3*32 + 16] = allocation_choice[2];
    bt656_diag_event_payload_d[3*32 + 17] = monitor_writes_slot;
    bt656_diag_event_payload_d[3*32 + 18] = ring_full_source;
    bt656_diag_event_payload_d[3*32 + 19] = ring_empty_source;
    bt656_diag_event_payload_d[3*32 + 20] = source_formatter_fatal;
    bt656_diag_event_payload_d[3*32 + 21] = source_ownership_fatal;
    bt656_diag_event_payload_d[3*32 + 25 +: 3] =
        bt656_diag_state_code(source_state);
    bt656_diag_event_payload_d[3*32 + 28 +: 3] =
        bt656_diag_state_code(state_after);
    bt656_diag_event_payload_d[4*32 +: 32] = source_frame_sequence;
    bt656_diag_event_payload_d[5*32 +: 32] = source_line_sequence;
    bt656_diag_event_payload_d[6*32 +: 32] = next_source_line;
    bt656_diag_event_payload_d[7*32 +: 32] = source_capture_sequence;
    bt656_diag_event_payload_d[8*32 +: 32] = pending_frame;
    bt656_diag_event_payload_d[9*32 +: 32] = pending_line;
    bt656_diag_event_payload_d[10*32 +: 32] = pending_capture;
    bt656_diag_event_payload_d[11*32 +: 32] = pending_attempt;
    bt656_diag_event_payload_d[12*32 +: 32] = source_lifetime_malformed;
    bt656_diag_event_payload_d[13*32 +: 32] = source_lifetime_dropped;
    bt656_diag_event_payload_d[14*32 +: 32] = pending_flags;
    bt656_diag_event_payload_d[15*32 + 0 +: 13] = payload_byte_count;
    bt656_diag_event_payload_d[15*32 + 13 +: 3] = post_payload_count;
    bt656_diag_event_payload_d[15*32 + 16 +: 3] = {1'b0, marker_fill};
    bt656_diag_event_payload_d[15*32 + 19 +: 2] =
        allocation_choice[2] ? allocation_choice[1:0] : 2'b0;
    bt656_diag_event_payload_d[15*32 + 21 +: 4] = malformed_reason;
    bt656_diag_event_payload_d[15*32 + 25 +: 4] = drop_reason;
  end

  // The complete diagnostic event is captured on one source-clock edge.
  // This stage is observational only and is intentionally absent from every
  // functional parser/transport feedback path.
  always_ff @(posedge source_clk) begin : BT656_DIAG1_EVENT_REGISTER
    if (source_reset) begin
      bt656_diag_event_valid <= 1'b0;
      bt656_diag_trigger_pulse <= 1'b0;
      bt656_diag_next_frame_line1_commit <= 1'b0;
      bt656_diag_event_payload <= '0;
    end else begin
      bt656_diag_event_valid <= bt656_diag_event_valid_d;
      bt656_diag_trigger_pulse <= bt656_diag_trigger_pulse_d;
      bt656_diag_next_frame_line1_commit <=
          bt656_diag_next_frame_line1_commit_d;
      bt656_diag_event_payload <= bt656_diag_event_payload_d;
    end
  end
  ...
  always_ff @(posedge source_clk) begin : SOURCE_DOMAIN
    ...
    if (transport_req_sync2_source != transport_req_seen_source) begin
      ...
      if (source_reset) begin
        ...
        marker_event_pipe <= 1'b0;
        marker_xy_valid_pipe <= 1'b0;
        marker_f_pipe <= 1'b0;
        marker_h_pipe <= 1'b0;
        marker_v_pipe <= 1'b0;
        marker_bytes_pipe <= '0;
      ...
      marker_event_pipe <= source_ready && marker_fill == 3 &&
                           marker_p0 == 8'hff && marker_p1 == 8'h00 &&
                           marker_p2 == 8'h00;
      marker_xy_valid_pipe <= source_byte[7] && source_byte[3:0] == 4'b0000;
      marker_f_pipe <= source_byte[6];
      marker_h_pipe <= source_byte[4];
      marker_v_pipe <= source_byte[5];
      marker_bytes_pipe <= {source_byte, marker_p2, marker_p1, marker_p0};
