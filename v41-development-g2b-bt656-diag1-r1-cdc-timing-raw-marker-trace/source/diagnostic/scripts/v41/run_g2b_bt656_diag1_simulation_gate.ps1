...
$sourcePaths = @(
  ...
  (Join-Path $repoPath 'tests\g2b\tb_g2b_bt656_diag1_r1_event_register.sv'),
  (Join-Path $repoPath 'tests\g2b\tb_g2b_bt656_diag1_r1_metadata.sv'),
...
$r1EventLog = Invoke-SimulationCase -Name 'r1_event_register' -Top 'tb_g2b_bt656_diag1_r1_event_register' -Snapshot 'diag1_r1_event_xsim' -Sources @(
    (Join-Path $repoPath 'rtl\g2b\v41_g2b_onech_c2h.sv'),
    (Join-Path $repoPath 'tests\g2b\tb_g2b_bt656_diag1_r1_event_register.sv'))

$r1MetadataLog = Invoke-SimulationCase -Name 'r1_metadata' -Top 'tb_g2b_bt656_diag1_r1_metadata' -Snapshot 'diag1_r1_metadata_xsim' -Sources @(
    (Join-Path $repoPath 'rtl\diagnostic\g2b_bt656_boundary_trace.sv'),
    (Join-Path $repoPath 'tests\g2b\tb_g2b_bt656_diag1_r1_metadata.sv'))
...
$expected = [ordered]@{
  ...
  T11 = @{ Log = $r1EventLog; Pattern = 'BT656_DIAG1_R1_T11_PASS source_registered_aligned_adjacent_events' }
  T12 = @{ Log = $r1MetadataLog; Pattern = 'BT656_DIAG1_R1_T12_PASS frozen_metadata_before_done_stable_new_generation' }
}
...
$existingPassed = @($results | Where-Object { $_.Test -match '^T([1-9]|10)$' -and $_.Result -eq 'PASS' }).Count
$receipt = [ordered]@{
  Gate = 'BT656_DIAG1_R1_SIMULATION_GATE'
  ExistingPassed = $existingPassed
  ExistingTotal = 10
  Passed = $passed
  Total = 12
  Result = $(if ($passed -eq 12 -and $existingPassed -eq 10) { 'PASS' } else { 'FAIL' })
  FunctionalNoninterference = $results[8].Result
  SourceMutations = 0
  Tests = $results
}
$receipt | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath (Join-Path $outputPath 'G2B_BT656_DIAG1_R1_SIMULATION_GATE.json') -Encoding utf8NoBOM

$markdown = [System.Collections.Generic.List[string]]::new()
$markdown.Add('# G2B BT656 DIAG1-R1 simulation gate')
$markdown.Add('')
$markdown.Add("- Result: $($receipt.Result)")
$markdown.Add("- Existing DIAG1 passed: $existingPassed/10")
$markdown.Add("- Total passed: $passed/12")
...
$markdown | Set-Content -LiteralPath (Join-Path $outputPath 'G2B_BT656_DIAG1_R1_SIMULATION_GATE.md') -Encoding utf8NoBOM

if ($passed -ne 12 -or $existingPassed -ne 10) {
  throw "BT656_DIAG1_R1_SIMULATION_GATE_FAILED:$passed/12 existing=$existingPassed/10"
}
Write-Output 'EXISTING_DIAG1_SIMULATION_GATE=10/10 PASS'
Write-Output 'BT656_DIAG1_R1_SIMULATION_GATE=12/12 PASS'
