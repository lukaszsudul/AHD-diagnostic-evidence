# AHD v41 G2B_BT656_DIAG1-R1 nonincremental Vivado 2025.2 build.
...
if {$argc != 7} {
  puts stderr "usage: g2b_bt656_diag1_build.tcl REPO_ROOT BUILD_ROOT EVIDENCE_ROOT SOURCE_COMMIT SOURCE_TREE BIT_FILENAME MODE"
  exit 2
}
lassign $argv repo_root build_root evidence_root source_commit source_tree bit_filename run_mode
...
if {$run_mode ni {SYNTH_GATE FULL}} {
  puts stderr "mode must be SYNTH_GATE or FULL"
  exit 2
}
...
set build_status [catch {
  ...
  if {$actual_commit ne $source_commit || $actual_tree ne $source_tree ||
      ...
      $actual_merge_base ne $accepted_product_parent ||
      ($run_mode eq "FULL" && $actual_status ne "")} {
  ...
  set all_rel_files [concat $sv_rel_files $vhdl_rel_files $xdc_rel_files [list \
    ...
    scripts/v41/g2b_bt656_diag1_r1_synth_audit.tcl]]
  set all_files [list]
  ...
  set synth_cdc_report [file join $evidence_root DIAG1_R1_SYNTH_CDC.rpt]
  report_cdc -details -file $synth_cdc_report
  source [file join $repo_root scripts v41 \
    g2b_bt656_diag1_r1_synth_audit.tcl]
  set synth_audit [run_g2b_bt656_diag1_r1_synth_audit \
    $evidence_root $synth_cdc_report]
  write_lines [file join $evidence_root DIAG1_R1_SYNTH_GATE_RESULT.txt] [list \
    {DIAG1_SYNTH_CDC_GATE=PASS} \
    {DIAG1_SYNTH_INTERCLOCK_PATH_GATE=PASS} \
    "DIAG1_RAW_USERCLK1_TO_SOURCE_TRACE_ENDPOINTS=[dict get $synth_audit RAW_USERCLK]" \
    "DIAG1_RAW_SOURCE_METADATA_TO_AXI_ENDPOINTS=[dict get $synth_audit RAW_METADATA]" \
    "DIAG1_WIDE_BITWISE_CDC_SAMPLERS=[dict get $synth_audit WIDE_SAMPLERS]" \
    "DIAG1_NEW_UNRESOLVED_CRITICAL_CDC=[dict get $synth_audit NEW_CRITICAL]" \
    "DIAG1_NEW_UNRESOLVED_WARNING_CDC=[dict get $synth_audit NEW_WARNING]" \
    "DIAG1_SOURCE_LOCAL_SETUP_ESTIMATE=[dict get $synth_audit SOURCE_LOCAL_SETUP]" \
    {RESULT=PASS}]

  if {$run_mode eq "SYNTH_GATE"} {
    write_lines [file join $evidence_root DIAG1_BUILD_RESULT.txt] [list \
      {RESULT=PASS} {MODE=SYNTH_GATE} \
      {BUILD_CLASSIFICATION=BT656_DIAG1_R1_SYNTH_CDC_GATE} \
      "SOURCE_COMMIT=$source_commit" "SOURCE_TREE=$source_tree" \
      {BITSTREAM=NONE} {LTX_GENERATED=NO}]
    puts "BT656_DIAG1_R1_SYNTH_GATE_PASS"
  }
  ...
  set routed_cdc_audit [diag1_r1_audit_cdc_rows $cdc_report]
  set routed_diag1_critical [dict get $routed_cdc_audit CRITICAL]
  set routed_diag1_warning [dict get $routed_cdc_audit WARNING]
  if {$cdc_errors != 0 || $cdc_unknown != 0 ||
      [llength $routed_diag1_critical] != 0 ||
      [llength $routed_diag1_warning] != 0} {
    write_lines [file join $evidence_root DIAG1_R1_ROUTED_UNRESOLVED_CDC.txt] \
      [concat $routed_diag1_critical $routed_diag1_warning]
    error "DIAG1-R1 routed CDC gate failed errors=$cdc_errors unknown=$cdc_unknown new_critical=[llength $routed_diag1_critical] new_warning=[llength $routed_diag1_warning]"
  }
  write_lines [file join $evidence_root DIAG1_CDC_DISPOSITION.txt] [list \
    {CDC_GATE=PASS_DIAG1_R1_EXPLICIT_PROTOCOL} \
    "CDC_TOTAL_CRITICAL=$cdc_critical" \
    "CDC_TOTAL_WARNING=$cdc_warnings" \
    {DIAG1_NEW_UNRESOLVED_CRITICAL_CDC=0} \
    {DIAG1_NEW_UNRESOLVED_WARNING_CDC=0} \
    {DIAG1_WIDE_BITWISE_CDC_SAMPLERS=0} \
  ...
  # The post-synthesis hard gate above is the authoritative source-cone and
  # source-to-AXI structural proof.  At route, explicitly prove that the old
  # failing cross-domain endpoint pair has no timed path.
  set old_trace_start [get_cells -quiet -hier -regexp \
    {.*G2B_ONECH_C2H/transport_hard_hold_axi_reg}]
  set old_trace_end [get_cells -quiet -hier -regexp \
    {.*BT656_BOUNDARY_TRACE/post_count_source_reg.*}]
  set old_trace_paths [get_timing_paths -quiet -delay_type max \
    -from $old_trace_start -to $old_trace_end -max_paths 1 -nworst 1]
  if {[llength $old_trace_paths] != 0} {
    error "removed transport_hard_hold_axi to post_count_source path still exists"
  }
  write_lines [file join $evidence_root DIAG1_R1_PREVIOUS_WORST_PATH_PROOF.txt] [list \
    {STARTPOINT=transport_hard_hold_axi_reg} \
    {ENDPOINT=post_count_source_reg[0]} \
    {TIMED_OR_CDC_PATH_COUNT=0} \
    {RESULT=PASS}]
  ...
  write_lines [file join $evidence_root DIAG1_BUILD_RESULT.txt] [list \
    {RESULT=PASS} {MODE=FULL} \
    {BUILD_CLASSIFICATION=BT656_DIAG1_R1_RAW_MARKER_TRACE_CANDIDATE} \
    "SOURCE_COMMIT=$source_commit" "SOURCE_TREE=$source_tree" \
    "BITSTREAM=$bit_path" "BITSTREAM_SHA256=$bit_sha" \
    "ROUTED_DCP_SHA256=$routed_sha" {LTX_GENERATED=NO}]
  puts "BT656_DIAG1_R1_BUILD_PASS BITSTREAM=$bit_path SHA256=$bit_sha"
  }
...
if {$build_status != 0} {
  ...
  puts stderr "BT656_DIAG1_R1_BUILD_FAIL STAGE=$stage ERROR=$build_error"
  catch {close_project}
  exit 1
}
catch {close_project}
