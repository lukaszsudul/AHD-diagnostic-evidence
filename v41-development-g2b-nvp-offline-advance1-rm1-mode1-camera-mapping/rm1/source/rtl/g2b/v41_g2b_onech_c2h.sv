...
module v41_g2b_onech_c2h (
  ...
  // Diagnostic-only observation taps. They expose existing AXI-domain state
  // and have no fanout into transport behavior or the frozen MMIO/record ABI.
  output logic        diag_stored_enable,
  output logic        diag_c2h_active,
  output logic        diag_ring_empty,
  output logic        diag_ring_full,

  // DIAG2-RM1 fanout-only observations.  These are registered from the
  // existing marker_p0/p1/p2 + source_byte detector; they neither replace
  // the physical sampling path nor feed any functional parser decision.
  output logic        diag_rm1_sample_valid,
  output logic [7:0]  diag_rm1_sample_byte,
  output logic [1:0]  diag_rm1_prefix_state,
  output logic        diag_rm1_candidate,
  output logic        diag_rm1_raw_parity_valid,
  output logic        diag_rm1_f,
  output logic        diag_rm1_v,
  output logic        diag_rm1_h,
  output logic        diag_rm1_parser_qualified,
  output logic [2:0]  diag_rm1_parser_state
);
  ...
  wire rm1_candidate_now = source_ready && marker_fill == 3 &&
      marker_p0 == 8'hff && marker_p1 == 8'h00 && marker_p2 == 8'h00;
  wire rm1_raw_parity_now = source_byte[7] &&
      source_byte[3] == (source_byte[5] ^ source_byte[4]) &&
      source_byte[2] == (source_byte[6] ^ source_byte[4]) &&
      source_byte[1] == (source_byte[6] ^ source_byte[5]) &&
      source_byte[0] == (source_byte[6] ^ source_byte[5] ^ source_byte[4]);
  wire rm1_parser_qualified_now = rm1_candidate_now && source_byte[7] &&
      source_byte[3:0] == 4'b0000;
  wire [1:0] rm1_prefix_state_now = rm1_candidate_now ? 2'd3 :
      ((marker_fill >= 2 && marker_p1 == 8'hff &&
        marker_p2 == 8'h00 && source_byte == 8'h00) ? 2'd2 :
       ((marker_fill >= 1 && marker_p2 == 8'hff &&
         source_byte == 8'h00) ? 2'd1 : 2'd0));
  ...
  always_ff @(posedge source_clk) begin : SOURCE_DOMAIN
    ...
    diag_rm1_sample_valid <= 1'b0;
    diag_rm1_candidate <= 1'b0;
    diag_rm1_raw_parity_valid <= 1'b0;
    diag_rm1_parser_qualified <= 1'b0;
    ...
    if (transport_req_sync2_source != transport_req_seen_source) begin
      ...
      // Register an observation-only copy of the exact detector inputs and
      // decisions used on this edge.  Full BT.656 protection-bit validation
      // is deliberately separate from the legacy parser's literal low-nibble
      // predicate so RM1 can distinguish wire legality from parser policy.
      diag_rm1_sample_valid <= source_ready;
      diag_rm1_sample_byte <= source_byte;
      diag_rm1_prefix_state <= rm1_prefix_state_now;
      diag_rm1_candidate <= rm1_candidate_now;
      diag_rm1_raw_parity_valid <= rm1_candidate_now && rm1_raw_parity_now;
      diag_rm1_f <= source_byte[6];
      diag_rm1_v <= source_byte[5];
      diag_rm1_h <= source_byte[4];
      diag_rm1_parser_qualified <= rm1_parser_qualified_now;
      diag_rm1_parser_state <= source_state;
