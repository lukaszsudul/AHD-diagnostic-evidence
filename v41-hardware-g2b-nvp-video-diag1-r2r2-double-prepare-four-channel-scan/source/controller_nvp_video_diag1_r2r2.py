...
from controller_nvp_capture import Mmio as ProductMmio
from controller_nvp_capture import runtime_identity as read_product_identity


TASK = "G2B-NVP-VIDEO-DIAG1-R2R2"
...
def verify_persisted_host_histories(status_csv: Path, status_jsonl: Path,
    ...
    require(all(row["CaptureResult"] == "PASS" and
                row["CleanupResult"] == "PASS" and
                row["PixelClassification"] in FRAME_CLASSIFICATIONS -
                {"INVALID_OR_CORRUPT"} and
                isinstance(row["FrameSHA256"], str) and
                len(row["FrameSHA256"]) == 64
                for row in json_rows),
            "NVP_DIAG1_R1_HOST_HISTORY_PERSISTED_RESULT_INVALID")
    return {
...
BASELINE_FIELDS = [
    ...
    "diag_status", "diag_state", "diag_error",
]
...
def execute_prepare(mmio: DiagnosticMmio, baseline_root: Path,
    ...
    require(observed_start_count == expected_start_count,
            f"NVP_DIAG1_R2R2_PREPARE_{name}_START_COUNT_DRIFT")
    ...
    while time.monotonic() < deadline:
        ...
        if bits["error"]:
            ...
            raise ScanError(
                f"NVP_DIAG1_R2R2_PREPARE_{name}_FIRMWARE_ERROR:"
                f"0x{code:08X}")
        ...
        raise ScanError(
            f"NVP_DIAG1_R2R2_PREPARE_{name}_FRESH_EXECUTION_TIMEOUT:"
            f"STATUS=0x{last_status:08X}:STATE={last_state}:"
            f"START_COUNT={observed_start_count}:LAST_COUNT={last_count}")
    require(bits["prepared"] and bits["baseline_restored"],
            f"NVP_DIAG1_R2R2_PREPARE_{name}_STATUS_INVALID")
    ...
    require(first == second,
            f"NVP_DIAG1_R2R2_PREPARE_{name}_HOST_DOUBLE_READ_MISMATCH")
    require(first["diag_state"] == 0 and first["diag_error"] == 0 and
            ...
            f"NVP_DIAG1_R2R2_PREPARE_{name}_ERROR_STATE")
    checked = status_bits(first["diag_status"])
    require(checked["prepared"] and checked["baseline_restored"] and
            ...
            f"NVP_DIAG1_R2R2_PREPARE_{name}_COMPLETION_STATE_INVALID")
...
def main() -> int:
    ...
    try:
        ...
        require(capabilities == 0x000003FF and
                ...
                "NVP_DIAG1_R2R2_CAPABILITIES_MISMATCH")
        runtime_parameters = {
        ...
        require(c1 > c0 and c2 > c1,
                "NVP_DIAG1_R2R2_PREPARE_TRANSACTION_COUNTER_NOT_STRICTLY_"
                "INCREASING")
        delta_a = c1 - c0
        delta_b = c2 - c1
        require(delta_a == delta_b,
                "NVP_DIAG1_R2R2_PREPARE_TRANSACTION_SEQUENCE_MISMATCH")
        require(baseline_a["bgcolor_78"] == baseline_b["bgcolor_78"] and
                baseline_a["bgcolor_79"] == baseline_b["bgcolor_79"] and
                baseline_a["route"] == baseline_b["route"],
                "NVP_DIAG1_R2R2_FUNCTIONAL_BASELINE_MISMATCH")
        require((baseline_b["bgcolor_78"] & 0xFF) == 0x88 and
                (baseline_b["bgcolor_79"] & 0xFF) == 0x88 and
                (baseline_b["route"] & 0x0F) == 0,
                "NVP_DIAG1_R2R2_BASELINE_CONTRADICTS_ACCEPTED_PRODUCT_STATE")
        baseline_comparison = {
            "result": "PASS",
            ...
            "original_bank_authority":
                "SECOND_PREPARE_FIRMWARE_PRIVATE_VALUE",
        ...
        for expected_session in range(1, 17):
            ...
            capture_outcome = "PASS" if capture_error is None else "FAIL"
            ...
            try:
                ...
                if not baseline_gate_passed:
                    ...
                    safe_restore_complete = (
                        ...
                        not state["i2c_owned"] and state["i2c_bus_idle"])
