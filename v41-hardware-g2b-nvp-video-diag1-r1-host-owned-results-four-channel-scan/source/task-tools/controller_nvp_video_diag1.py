...
TASK = "AHD-v41-G2B-NVP-VIDEO-DIAG1-R1"
        ...
        if mmio is not None:
            try:
                ...
                safe_restore_complete = (
                    state["baseline_restored"] and not state["i2c_owned"] and
                    state["i2c_bus_idle"])
                if not safe_restore_complete:
                    if response_withheld_for_unsafe_quiescence:
                        result["restore_failure"] = (
                            "NVP_DIAG1_FAILURE_RESPONSE_WITHHELD_TO_PREVENT_"
                            "ROUTE_CHANGE_WITHOUT_PHYSICAL_QUIESCENCE")
                    elif pending_response is not None and state["capture_ready"]:
                        mmio.write_capture_response(pending_response, "ABORT")
                        pending_response = None
                    elif not state["error"]:
                        mmio.write_control(8, "FAILURE_ABORT_AND_SAFE_RESTORE")
                    if not response_withheld_for_unsafe_quiescence:
                        wait_for_baseline_restore(mmio, 15.0,
                                                  "FAILURE_SAFE_RESTORE_WAIT")
