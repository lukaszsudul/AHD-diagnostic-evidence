# AHD v41 G2B NVP VIDEO DIAG1-R1: host-owned-result four-channel diagnostic.
#
...
#
# DIAG1-R1 has one permitted execution mode: a clean, exact committed source is
# synthesized, optimized, resource/architecture-gated, placed, routed, signed
# off, and written as a bitstream in one Vivado invocation.  Precommit and DCP-
# recovery modes are deliberately unreachable.  A commit-bound 19/19
# simulation receipt is a required explicit input.

if {$argc != 8} {
  puts stderr "usage: g2b_nvp_video_diag1_build.tcl REPO_ROOT BUILD_ROOT EVIDENCE_ROOT SOURCE_COMMIT SOURCE_TREE G2B_NVP_VIDEO_DIAG1_R1_FOUR_CHANNEL_SCAN.bit NVP_VIDEO_DIAGNOSTIC SIMULATION_RECEIPT"
  exit 2
}
...
set expected_r1_source_commit fcab95726761a0666a67e31c283dbdfb9e775074
set expected_r1_source_tree bbf1a5fee70a2eb68bb96305ed10934a1559ca6a
set expected_simulation_receipt_sha 7A72A2E95B677884630C00AE6DB299F33844ADC8DAE48F9AB73372FAD7B6B47E
...
set expected_task AHD_V41_G2B_NVP_VIDEO_DIAG1_R1
set expected_candidate_classification G2B_NVP_VIDEO_DIAG1_R1_HOST_OWNED_RESULTS_CANDIDATE
set expected_bit_filename G2B_NVP_VIDEO_DIAG1_R1_FOUR_CHANNEL_SCAN.bit
set expected_diag_version 0x00010001
set expected_diag_capabilities 0x000003FF
...
proc validate_committed_simulation_receipt {} {
  ...
  set expected_path [file normalize [file join $task_root build \
    simulation_gate_committed_final NVP_VIDEO_DIAG1_SIMULATION_RECEIPT.txt]]
  ...
  foreach {key expected} {
    ...
    INHERITED_SIMULATION_CASES_PASSED 16
    R1_SIMULATION_CASES_EXPECTED 19
    R1_SIMULATION_CASES_PASSED 19
    T17_SNAPSHOT_ATOMICITY PASS
    T18_HOST_OWNED_HISTORY PASS
    T19_RESOURCE_ARCHITECTURE_STATIC_CONTRACT PASS
    TIMING_BOUNDARY_RUNTIME_PROOF PASS
    TIMING_BOUNDARY_PRODUCTION_FORMULA_PROOF PASS
  ...
  require_receipt_value $values TESTED_GIT_BRANCH $expected_branch
  require_receipt_value $values TESTED_GIT_HEAD_AT_START $source_commit
  require_receipt_value $values TESTED_GIT_TREE_AT_START $source_tree
  foreach {key rel} {
    TESTED_CORE_SHA256 rtl/g2b/g2b_nvp_video_diag.sv
    TESTED_CORE_TB_SHA256 tests/nvp_video_diag/tb_g2b_nvp_video_diag.sv
    TESTED_I2C_MASTER_SHA256 rtl/v41/nvp_i2c_fixed_master.sv
    TESTED_I2C_TB_SHA256 tests/nvp_video_diag/tb_nvp_i2c_fixed_master.sv
    TESTED_TOP_SHA256 rtl/top/ahd_capture_top_xdma.sv
  } {
...
proc validate_r1_source_architecture {} {
  ...
  if {![regexp {DIAG_VERSION[ \t]*=[ \t]*32'h0001_0001} $text]} {
    error "R1 diagnostic source version is not 0x00010001"
  }
  if {$expected_diag_version ne "0x00010001" || $expected_diag_capabilities ne "0x000003FF"} {
    error "internal R1 diagnostic identity constants are inconsistent"
  }
...
if {![regexp {^G2B_NVP_VIDEO_DIAG1_R1_[0-9]{8}T[0-9]{6}Z$} [file tail $task_root]] ||
...
if {$actual_merge_base ne $failed_diag1_commit ||
    $integration_commit_count < 1 || $integration_commit_count > 2} {
  error "R1 source is not a bounded one-or-two-commit descendant of failed DIAG1"
}
...
set build_result [catch {
  ...
  create_project v41_g2b_nvp_video_diag1_r1 $project_dir -part $expected_part
  ...
  set synth_dcp_path [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R1_SYNTH.dcp]
  ...
  set post_opt_dcp_path [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R1_POST_OPT.dcp]
  ...
  set routed_dcp_path [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R1_ROUTED.dcp]
  ...
  set bit_size [file size $bit_path]
  set bit_sha [sha256_file $bit_path]
