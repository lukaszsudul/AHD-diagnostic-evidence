...
#define SHUTDOWN_GUARD_RECORDS 32U
...
static int process_event(const struct io_event *event, Request *requests,
    ...
    if (id64 >= TOTAL_LOGICAL_REQUESTS) {
        *blocker = "R3R4R6R2R3_COMPLETION_ID_OUT_OF_RANGE";
        return -1;
    }
    size_t id = (size_t)id64;
    Request *request = &requests[id];
    if (!request->accepted || request->completed) {
        *blocker = "R3R4R6R2R3_COMPLETION_ID_DUPLICATE_OR_UNSUBMITTED";
        return -1;
    }
    request->completed = 1;
    ...
    if (id < PRIMARY_RECORDS) {
        ...
        if (request->result == RECORD_BYTES && request->result2 == 0) {
            ...
            ++m->primary_short;
            *blocker = "R3R4R6R2R3_PRIMARY_SHORT_COMPLETION";
            return -1;
        } else {
            ++m->primary_failed;
            *blocker = "R3R4R6R2R3_PRIMARY_FAILED_COMPLETION";
            return -1;
        }
        ...
        if (request->result == RECORD_BYTES && request->result2 == 0) {
            ...
            ++m->guard_short;
            *blocker = "R3R4R6R2R3_GUARD_POSITIVE_SHORT_COMPLETION";
            return -1;
        } else {
            ++m->guard_failed;
            *blocker = "R3R4R6R2R3_GUARD_FAILED_COMPLETION";
            return -1;
        }
    }
    return 0;
...
static int read_parent_quiescent(int *parent_quiescent) {
...
static int persist_tables(const char *directory, const Request *requests,
    ...
    dprintf(fd,
        "{\n  \"schema\": \"R3R4R6R2R3_ROLLING_HELPER_RESULT_V1\",\n"
        "  \"result\": \"%s\",\n  \"blocker\": \"%s\",\n"
        "  \"record_bytes\": %u,\n  \"primary_records\": %u,\n"
...
static int synthetic_self_test(void) {
        ...
        for (size_t pass = 0; pass < 2 && taken < batch; ++pass) {
            ...
            for (size_t i = begin; i < end && taken < batch; ++i) {
                ...
                if (primary_completed == PRIMARY_RECORDS && !primary_event) {
                    ...
                    size_t guard_completed = total_completed - primary_completed;
                    persistence_started_before_guard_complete = guard_completed < SHUTDOWN_GUARD_RECORDS;
                }
            }
        }
    ...
    printf("{\"type\":\"SELF_TEST_RESULT\",\"result\":\"%s\","
           ...
           "\"guard_cancel_targets_only\":%s,\"completed_data_preserved\":%s}\n",
...
int main(int argc, char **argv) {
    ...
    if (posix_memalign((void **)&primary, RECORD_BYTES, PRIMARY_BYTES) != 0 ||
        ...
        emit_error("R3R4R6R2R3_BUFFER_ALLOCATION_FAILED", 0, -ENOMEM, 0, 0);
    ...
    if (requests == NULL || pointers == NULL) {
        emit_error("R3R4R6R2R3_CONTROL_ALLOCATION_FAILED", 0, -ENOMEM, 0, 0);
    ...
    if (c2h < 0) {
        emit_error("R3R4R6R2R3_C2H_OPEN_FAILED", 0, -errno, 0, 0);
    ...
    if (syscall(__NR_io_setup, AIO_CONTEXT_CAPACITY, &context) < 0) {
        emit_error("R3R4R6R2R3_IO_SETUP_FAILED", 0, -errno, 0, 0);
    ...
    if (submit_rc != 0 || m.submitted != MAX_OUTSTANDING_IOCBS ||
        ...
        blocker = submit_rc == -2 ? "R3R4R6R2R3_DESCRIPTOR_WINDOW_VIOLATION" :
                                   "R3R4R6R2R3_INITIAL_PREQUEUE_FAILED";
        failure = 1;
    }
    ...
    if (!failure && early != 0) {
        blocker = "R3R4R6R2R3_COMPLETION_BEFORE_ENABLE";
        failure = 1;
    }
    if (failure) {
    ...
    while (!failure && !m.primary_event_emitted) {
        ...
        if (now - m.capture_start_ns >= TOTAL_CAPTURE_TIMEOUT_NS) {
            blocker = "R3R4R6R2R3_TOTAL_CAPTURE_TIMEOUT";
            failure = 1;
            break;
        }
        if (now - m.last_completion_ns >= NO_PROGRESS_TIMEOUT_NS) {
            blocker = "R3R4R6R2R3_NO_PROGRESS_TIMEOUT";
            failure = 1;
            break;
        }
        ...
        if (count < 0) {
            ...
            blocker = "R3R4R6R2R3_IO_GETEVENTS_FAILED";
            failure = 1;
            break;
        }
        if (count == 0) continue;
        ...
        if (m.next_submit < TOTAL_LOGICAL_REQUESTS) {
            ...
            if (rc != 0) {
                blocker = rc == -2 ? "R3R4R6R2R3_DESCRIPTOR_WINDOW_VIOLATION" :
                                     "R3R4R6R2R3_ROLLING_REFILL_FAILED";
                failure = 1;
                break;
            }
        }
    ...
    if (!failure && (!m.primary_event_emitted || m.primary_exact != PRIMARY_RECORDS)) {
        blocker = "R3R4R6R2R3_PRIMARY_WINDOW_INCOMPLETE";
        failure = 1;
        emit_error(blocker, m.next_submit, -EIO, 0, m.outstanding);
    }
    if (!failure && persist_primary_full(argv[2], primary, &m) != 0) {
        blocker = "R3R4R6R2R3_PRIMARY_PERSISTENCE_FAILED";
        failure = 1;
    ...
    while (!parent_quiescent) {
        if (read_parent_quiescent(&parent_quiescent) < 0) {
            if (!failure) {
                blocker = "R3R4R6R2R3_PARENT_COMMAND_INVALID";
                failure = 1;
        ...
        if (count < 0 && errno != EINTR) {
            if (!failure) {
                blocker = "R3R4R6R2R3_IO_GETEVENTS_DRAIN_FAILED";
                failure = 1;
    ...
    if (cleanup_unresolved) {
        blocker = failure ? "R3R4R6R2R3_FAILURE_AIO_CLEANUP_UNRESOLVED" :
                            "R3R4R6R2R3_GUARD_AIO_CLEANUP_UNRESOLVED";
        failure = 1;
    }
    if (persist_tables(argv[2], requests, &m, primary, guard,
                       !failure && m.primary_durable, blocker) != 0) {
        blocker = "R3R4R6R2R3_METADATA_PERSISTENCE_FAILED";
        failure = 1;
    }
    if (cleanup_unresolved) {
    ...
    if (destroy_failed) {
        emit_error("R3R4R6R2R3_IO_DESTROY_FAILED", 0, -errno, 0, 0);
        return 75;
    }
