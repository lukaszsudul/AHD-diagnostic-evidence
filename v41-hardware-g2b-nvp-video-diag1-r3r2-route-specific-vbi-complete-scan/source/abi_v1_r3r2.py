    ...
    def accept(self, blob: bytes) -> RecordAssessment:
        ...
        if channel in self.last_source:
            ...
            if previous_line == self.contract.lines_per_frame - 1:
                ...
                expected_line = 0
                capture_delta = _u32(
                    record.source_capture_sequence - previous_capture)
                capture_ok = 1 <= capture_delta <= 22
            else:
                expected_frame = previous_frame
                expected_line = previous_line + 1
                capture_delta = _u32(
                    record.source_capture_sequence - previous_capture)
                capture_ok = capture_delta == 1
            observed_source = (
            ...
            if observed_source != expected_source or not capture_ok:
                reasons.append(
                    f"source progression {observed_source} expected {expected_source}; "
                    f"capture delta {capture_delta} "
                    f"expected {'1..22' if previous_line == self.contract.lines_per_frame - 1 else '1'}"
                )
