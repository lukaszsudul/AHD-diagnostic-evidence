...
from abi_v1_r3r2 import AbiContract, RecordValidationError, parse_record
from frame_reconstruct_nvp_capture import write_png_from_uyvy
from vbi_tail_contract_r3r2 import (
    LAST_ACTIVE_LINE,
    MAX_FRAME_BOUNDARY_CAPTURE_DELTA,
    MIN_FRAME_BOUNDARY_CAPTURE_DELTA,
    NON_FRAME_BOUNDARY_CAPTURE_DELTA,
    ...
    try:
        ...
        for index in range(RECORDS):
            ...
            if previous is not None:
                ...
                frame_boundary = (previous.source_line_sequence ==
                                  LAST_ACTIVE_LINE and
                                  expected_line == 0)
                capture_delta = delta32(record.source_capture_sequence,
                                        previous.source_capture_sequence)
                if frame_boundary:
                    frame_boundary_count += 1
                    capture_ok = (MIN_FRAME_BOUNDARY_CAPTURE_DELTA <=
                                  capture_delta <=
                                  MAX_FRAME_BOUNDARY_CAPTURE_DELTA)
                else:
                    capture_ok = capture_delta == NON_FRAME_BOUNDARY_CAPTURE_DELTA
                if not capture_ok:
        ...
        for index in range(1, RECORDS):
            ...
            if (previous_record is None or current_record is None or
                    previous_record.source_line_sequence != LAST_ACTIVE_LINE):
                continue
            assessment = validate_transition(
                previous_frame=previous_record.source_frame_sequence,
                previous_line=previous_record.source_line_sequence,
                previous_capture=previous_record.source_capture_sequence,
                previous_attempt=previous_record.channel_attempt_sequence,
                previous_global=previous_record.global_stream_sequence,
                previous_malformed=previous_record.value(
                    "source_malformed_count_snapshot"),
                previous_dropped=previous_record.value(
                    "source_dropped_count_snapshot"),
                current_frame=current_record.source_frame_sequence,
                current_line=current_record.source_line_sequence,
                current_capture=current_record.source_capture_sequence,
                current_attempt=current_record.channel_attempt_sequence,
                current_global=current_record.global_stream_sequence,
                current_malformed=current_record.value(
                    "source_malformed_count_snapshot"),
                current_dropped=current_record.value(
                    "source_dropped_count_snapshot"),
                current_flags=current_record.flags,
                next_frame=(next_record.source_frame_sequence
                            if next_record is not None else None),
                next_line=(next_record.source_line_sequence
                           if next_record is not None else None),
                next_flags=(next_record.flags if next_record is not None else None),
                valid_flag=contract.flags["VALID"],
                sof_flag=contract.flags["SOF"],
            )
            capture_delta = assessment["capture_sequence_delta"]
            boundary_rows.append({
                ...
                "MinimumLegalCaptureSequenceDelta":
                    MIN_FRAME_BOUNDARY_CAPTURE_DELTA,
                "MaximumLegalCaptureSequenceDelta":
                    MAX_FRAME_BOUNDARY_CAPTURE_DELTA,
                "TailIntervals": assessment["tail_intervals"],
        ...
        first_flags_allowed = bool(
            records[0] is not None and records[0].flags in (
                contract.flags["VALID"],
                contract.flags["VALID"] | contract.flags["DISCONTINUITY"],
            ))
        active_frame_integrity = (
            integrity_failures == 0 and global_gaps == 0 and
            attempt_gaps == 0 and source_breaks == 0 and
            records[0] is not None and records[-1] is not None and
            records[0].global_stream_sequence == 0 and
            records[-1].global_stream_sequence == 2499 and
            first_flags_allowed and not overflow_indices and
            not malformed_indices and not discont_after_first and
            malformed_delta == 0 and dropped_delta == 0)
        bounded_vbi_pass = (
            bool(boundary_rows) and unexpected_capture_gaps == 0 and
            all(row["Result"] == "PASS" for row in boundary_rows))
        primary_pass = active_frame_integrity and bounded_vbi_pass
        frame_pass = frame_start is not None
        stream_pass = active_frame_integrity and frame_pass
        tail_summary = summarize_tail_deltas(
            row["CaptureSequenceDelta"] for row in boundary_rows)
        ...
        result = {
            ...
            "frame_boundary_count": frame_boundary_count,
            "legal_vertical_tail_capture_sequence_jumps":
                sum(row["Result"] == "PASS" for row in boundary_rows),
            "unexpected_source_capture_sequence_gaps": unexpected_capture_gaps,
            "out_of_bounds_capture_gaps": unexpected_capture_gaps,
            ...
            "active_frame_and_transport_integrity":
                "PASS" if active_frame_integrity else "FAIL",
            "bounded_route_specific_vbi_tail":
                "PASS" if bounded_vbi_pass else "FAIL",
            "legacy_ch1_exact_21_tail_fingerprint":
                tail_summary["legacy_ch1_exact_21_tail_fingerprint"],
            "route_vbi_tail_fingerprint":
                tail_summary["fingerprint_classification"],
            "capture_sequence_delta_values":
                tail_summary["capture_sequence_delta_values"],
            "vertical_tail_interval_values":
                tail_summary["tail_interval_values"],
            "minimum_vertical_tail_intervals":
                tail_summary["minimum_tail_intervals"],
            "maximum_vertical_tail_intervals":
                tail_summary["maximum_tail_intervals"],
            "mode_vertical_tail_intervals":
                tail_summary["mode_tail_intervals"],
            "unique_vertical_tail_interval_count":
                tail_summary["unique_tail_interval_count"],
            "active_diagnostic_exact_22_structural_gates": 0,
        ...
        continuity_rows = [
            ...
            {"Metric": "LEGAL_VERTICAL_TAIL_CAPTURE_SEQUENCE_JUMPS",
             "Value": sum(row["Result"] == "PASS" for row in boundary_rows)},
        ...
        write_csv(args.logs_dir / "frame-boundary-analysis.csv",
                  ["PreviousRecordIndex", "Line0RecordIndex", "PreviousFrame",
                   ...
                   "Line1Flags", "CaptureSequenceDelta",
                    "MinimumLegalCaptureSequenceDelta",
                    "MaximumLegalCaptureSequenceDelta", "TailIntervals",
                    "AttemptSequenceDelta",
                    "GlobalSequenceDelta", "MalformedDelta", "DroppedDelta",
                    "Failures", "Result"], boundary_rows)
