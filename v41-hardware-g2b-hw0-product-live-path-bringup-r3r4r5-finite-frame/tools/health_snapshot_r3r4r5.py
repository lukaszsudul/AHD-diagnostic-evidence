"""Read-only R3R4R5 PCIe, AER, kernel, link, and driver health snapshot."""
import json
import os
import pathlib
import re
...
LABELS = {"after-bind", "before-capture", "after-capture", "before-unload",
          "after-unload"}
...
if len(sys.argv) != 2 or sys.argv[1] not in LABELS:
    raise SystemExit("USAGE: health_snapshot_r3r4r5.py " +
                     "|".join(sorted(LABELS)))
label = sys.argv[1]
result = {"task": TASK, "label": label, "result": "BLOCKED",
          "utc_ns": time.time_ns()}
try:
    ...
    module_loaded = P("/sys/module/xdma_ahd_pcie").exists()
    nodes = sorted(str(item) for item in P("/dev").glob("xdma*"))
    if label == "after-unload":
        require(driver is None and not module_loaded and not nodes,
                "POST_UNLOAD_DRIVER_OR_NODES_REMAIN")
    else:
        require(driver == "xdma_ahd_pcie" and module_loaded,
                "AHD_XDMA_RUNTIME_STATE_CHANGED")
        require(any(re.fullmatch(r"/dev/xdma\d+_user", item) for item in nodes) and
                any(re.fullmatch(r"/dev/xdma\d+_c2h_0", item) for item in nodes),
                "REQUIRED_XDMA_NODES_MISSING")
    require(not P("/sys/module/xdma").exists(), "PLATFORM_XDMA_MODULE_PRESENT")

    lspci = {bdf: subprocess.check_output(
        ["lspci", "-s", bdf, "-vvv"], text=True)
        for bdf in ("0000:01:00.0", "0000:00:01.1")}
    (ROOT / f"logs/kernel-{label}.txt").write_bytes(kernel_now)
    result.update(
        result="PASS", blocker="NONE", boot_id=boot,
        endpoint_link=endpoint_link, root_port_link=root_link, aer=current_aer,
        aer_delta_from_after_load="NONE", kernel_fatal_matches=fatal,
        kernel_bytes=len(kernel_now), kernel_delta_bytes=len(delta.encode()),
        taint=int(P("/proc/sys/kernel/tainted").read_text()), driver=driver,
        module_loaded=module_loaded, platform_xdma=False, nodes=nodes,
        open_xdma_fds=open_xdma_fds(), lspci=lspci)
except BaseException as exc:
    result["blocker"] = str(exc)

(ROOT / f"logs/health-{label}.json").write_text(
