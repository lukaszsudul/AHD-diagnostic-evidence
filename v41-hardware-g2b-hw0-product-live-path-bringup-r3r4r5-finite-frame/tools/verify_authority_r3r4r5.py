#!/usr/bin/env python3
"""Commit-pinned authority verification for the governed R3R4R5 run."""
...
PREDECESSORS = {
    ...
    "r3r4r4": {
        "commit": "4b4346e1def6dce0bbeffe227e2731f9c7cfd314",
        "directory": "v41-hardware-g2b-hw0-product-live-path-bringup-r3r4r4-finite-frame",
        "manifest": "G2B_HW0_PRODUCT_R3R4R4_SHA256_MANIFEST.txt",
        "state": "G2B_HW0_PRODUCT_R3R4R4_STATE.json",
    },
}
...
def main(output_path: pathlib.Path) -> int:
    ...
    require(checks, "EVIDENCE_HEAD_PINNED_R3R4R4", head == PREDECESSORS["r3r4r4"]["commit"])
    require(checks, "EVIDENCE_REMOTE_PINNED_R3R4R4", remote == head)
    ...
    r3r4r4 = states["r3r4r4"]
    r4_analysis = git_json(
        PREDECESSORS["r3r4r4"]["commit"],
        PREDECESSORS["r3r4r4"]["directory"],
        "raw/G2B_HW0_PRODUCT_R3R4R4_SELFTEST_FAILURE_ANALYSIS.json",
    )
    require(checks, "R3R4R4_EXPECTED_BLOCKER",
            r3r4r4.get("failed_selftest") == "PARENT_MULTI_SAMPLE_QUIESCENCE_PASS"
            and r3r4r4.get("existing_offline_selftests") == {"passed": 11, "total": 11}
            and r3r4r4.get("new_contract_selftests") == {"passed": 3, "total": 5}
            and r3r4r4.get("total_offline_selftests") == {"passed": 14, "total": 16}
            and not r3r4r4.get("hardware_accessed")
            and r3r4r4.get("dut_connections") == 0
            and r3r4r4.get("driver_load_attempts") == 0
            and r3r4r4.get("prior_immutable_artifact_new_writes") == 0)
    require(checks, "R3R4R4_FLOAT_BOUNDARY_DIAGNOSIS",
            r4_analysis.get("diagnosis") == "BINARY_FLOAT_BOUNDARY_1.4_MINUS_1.0_IS_BELOW_0.4"
            and r4_analysis.get("rows", [])[-1].get("span_repr") == "0.3999999999999999"
            and r4_analysis.get("rows", [])[-1].get("required_span_repr") == "0.4"
            and r4_analysis.get("capture_or_selftest_modified_after_suite") is False
            and r4_analysis.get("suite_rerun") is False)
    require(checks, "R3R4R4_RETAINED_CONTRACT_PROOFS",
            r3r4r4.get("ordering_proof_basis") == "EVENT_SEQUENCE_PLUS_EXPLICIT_DEPENDENCY"
            and r3r4r4.get("persisted_first_record_reread_proof") == "PASS"
            and r3r4r4.get("primary_durability_checkpoints") == "PASS"
            and r3r4r4.get("first_record_durable_before_primary_target") == "PASS")
    ...
    abi_hash = sha_file(EVIDENCE / PREDECESSORS["r3r4r4"]["directory"] / "tools" / "V41_C2H_TRANSPORT_ABI_V1.json")
    ...
    receipt = {
        "schema": "R3R4R5_AUTHORITY_VERIFICATION_V1",
        "result": result,
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        raise SystemExit("usage: verify_authority_r3r4r5.py OUTPUT.json")
