#!/usr/bin/env python3
"""Independent structural hard gate for the R3R4R5 capture contract."""
...
ROOT = Path(r"C:\FPGA\G2B_HW0_PRODUCT_R3R4R5_20260907T151342Z")
...
def main() -> int:
    ...
    selftest = json.loads((
        ARTIFACTS / "G2B_HW0_PRODUCT_R3R4R5_CAPTURE_TOOL_SELFTEST.json"
    ).read_text(encoding="utf-8"))
    delta = json.loads((
        ARTIFACTS / "G2B_HW0_PRODUCT_R3R4R5_CAPTURE_TOOL_DIFF.json"
    ).read_text(encoding="utf-8"))
    ...
    checks = {
        ...
        "RUNTIME_UNCHANGED_AFTER_SELFTEST":
            ...
            delta.get("working_hashes", {}).get("capture_r3r4.py") and
            delta.get("runtime_capture_semantics_changed") is False and
            delta.get("runtime_quiescence_implementation_changed") is False,
        "AUTHORIZED_DELTA_PASSED":
            delta.get("authorized_tool_delta") ==
            "QUIESCENCE_FIXTURE_1_400_TO_1_401_PLUS_RUN_IDENTITY_ONLY" and
            delta.get("result") == "PASS",
        "SYNTHETIC_FIXTURE_ONLY_1_401":
            delta.get("synthetic_final_observation_seconds") == 1.401 and
            delta.get("synthetic_evaluated_span_seconds", 0.0) > 0.4 and
            delta.get("checks", {}).get("SELFTEST_ONLY_EXACT_FIXTURE_DELTA") is True,
        ...
        "PRIMARY_CHECKPOINT_WORKER_ASYNCHRONOUS":
            ...
            "r3r4r5-primary-durability-worker" in
    ...
    blocker = ("NONE" if not failed else
               "R3R4R5_CAPTURE_TOOL_ARCHITECTURE_HARD_GATE_FAILED:" + reason)
    result = {
        "schema": "R3R4R5_CAPTURE_TOOL_ARCHITECTURE_HARD_GATE_V1",
    ...
    write_exclusive(
        ARTIFACTS /
        "G2B_HW0_PRODUCT_R3R4R5_CAPTURE_TOOL_ARCHITECTURE_HARD_GATE.json",
    ...
    text = "\n".join(lines) + "\n"
    write_exclusive(
        ARTIFACTS / "G2B_HW0_PRODUCT_R3R4R5_CAPTURE_TOOL_AUDIT.md", text)
    write_exclusive(
        ARTIFACTS /
        "G2B_HW0_PRODUCT_R3R4R5_CAPTURE_TOOL_ARCHITECTURE_HARD_GATE.md", text)
