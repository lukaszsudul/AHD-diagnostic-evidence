# AHD v41 G2B NVP CAMERA ACQ1-COMPAT0-R2: fresh routed implementation stage.
#
...
#
# ACQ1-COMPAT0-R2 creates one fresh nonincremental routed implementation and all raw
# physical reports.  The physical CDC rows are deliberately handed to the
# separate semantic reconciliation gate.  This stage must not write a
# bitstream.

if {$argc != 8} {
  puts stderr "usage: g2b_nvp_camera_acq1_compat0_r2_build.tcl REPO_ROOT BUILD_ROOT EVIDENCE_ROOT SOURCE_COMMIT SOURCE_TREE G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_CH1_SLICE.bit NVP_CAMERA_ACQ1_COMPAT0_R2 SIMULATION_RECEIPT"
  exit 2
}

lassign $argv repo_root build_root evidence_root source_commit source_tree bit_filename build_profile simulation_receipt
if {$build_profile ne "NVP_CAMERA_ACQ1_COMPAT0_R2"} {
  error "BUILD_PROFILE must be exactly NVP_CAMERA_ACQ1_COMPAT0_R2"
}
set enable_rtrack_diagnostics 0
set enable_nvp_video_diagnostic 0
set enable_nvp_camera_scan1 1
set enable_nvp_acq1_executor 1
set execution_mode POSTCOMMIT_FULL_BUILD
set precommit_manifest "DISABLED"
set precommit_mode 0
...
set simulation_receipt [file normalize $simulation_receipt]
set build_harness [file normalize [info script]]
set task_root [file normalize [file join $evidence_root .. ..]]
set artifact_root [file normalize [file join $task_root signoff]]
...
set accepted_hardened_product_commit 30b14d13b0b789b62b05ab513eb9578c7c43b11a
set accepted_hardened_product_tree bdbe39077a03f8945ebdd1ed9e52761fbe787696
set expected_parent_commit c7e16fa3da26545cef960a6c75427a3614c4b655
set expected_parent_tree 56526e17154f8e06f3eb4b95233934c18b6ee06e
set failed_diag1_commit $expected_parent_commit
set failed_diag1_tree $expected_parent_tree
set expected_r1_source_commit dae2aff60141ecdbc0afac08fc0df9a3166f66c6
set expected_r1_source_tree 21e33d481ef637667756caa8015e7fa1b1dd8ebf
set expected_simulation_receipt_sha 5ADE100B1C1EC4ED40CDD231E9CA8EA0D94CF8B11855F9B3E15F89A3EE8381FF
...
set expected_branch diag/v41-g2b-nvp-camera-acq1-compat0
...
set expected_task AHD_V41_G2B_NVP_CAMERA_ACQ1_COMPAT0_R2
set expected_candidate_classification ACQ1_COMPAT0_R2_OFFLINE_QUALIFIED_DIAGNOSTIC_CANDIDATE
set expected_bit_filename G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_CH1_SLICE.bit
set expected_diag_version 0x00010001
set expected_diag_capabilities 0x0000000F
set expected_acq_magic 0x4E564143
set expected_acq_version 0x00010000
set expected_acq_capabilities 0x000000FF
set previous_diag1_lut 22306
set previous_diag1_ff 23674
set previous_diag1_core_lut 3992
set previous_diag1_core_ff 4222
set post_opt_lut_hard_max 20384
set post_opt_lut_target_max 19136
set post_opt_lut_preferred_max 19136
set required_lut_reduction 0
...
proc canonical_build_input_hashes {} {
  global repo_root sv_rel_files vhdl_rel_files xdc_rel_files build_harness
  ...
  lappend result "TASK_LOCAL_BUILD_HARNESS|[sha256_file $build_harness]"
  return $result
}

proc validate_precommit_input_manifest {manifest_path expected_lines} {
  error "PRECOMMIT_FULL_BUILD is disabled for ACQ1-COMPAT0-R2"
...
proc source_seal_is_current {} {
  global source_commit source_tree expected_branch sealed_input_hashes
  if {[catch {
    ...
    # Task-local, untracked simulation/build products do not alter the committed
    # source identity.  Only tracked source changes are forbidden.
    set status_now [git_value status --porcelain --untracked-files=no]
    set hashes_now [canonical_build_input_hashes]
    set seal_ok [expr {$identity_ok && $status_now eq "" &&
                       $hashes_now eq $sealed_input_hashes}]
  }]} {
    return 0
  }
  return $seal_ok
...
proc evaluate_resource_gate {stage metrics} {
  global evidence_root previous_diag1_lut post_opt_lut_hard_max
  global post_opt_lut_target_max post_opt_lut_preferred_max required_lut_reduction
  set lut_used_raw [dict get $metrics LUT_USED]
  set lut_available_raw [dict get $metrics LUT_AVAILABLE]
  if {![string is double -strict $lut_used_raw] ||
      ![string is double -strict $lut_available_raw]} {
    error "$stage resource gate received non-numeric LUT values"
  }
  ...
  if {abs($lut_used_raw - $lut_used) > 0.000001 ||
    ...
    error "$stage LUT utilization is not an exact integer count"
  }
  set lut_reduction [expr {$previous_diag1_lut - $lut_used}]
  ...
  set gate PASS
  set reason NONE
  if {$lut_available != 20800} {
    set gate FAIL
    set reason LUT_AVAILABLE_NOT_20800
  } elseif {$lut_used > $post_opt_lut_hard_max} {
    set gate FAIL
    set reason LUT_USED_GT_20384
  } elseif {$lut_reduction < $required_lut_reduction} {
    set gate FAIL
    set reason LUT_REDUCTION_LT_1922
  } elseif {$ff_percent > 95.0} {
    set gate FAIL
    set reason FF_GT_95_PERCENT
  } elseif {$bram_percent > 90.0} {
    set gate FAIL
    set reason BRAM_GT_90_PERCENT
  } elseif {$dsp_percent > 90.0} {
    set gate FAIL
    set reason DSP_GT_90_PERCENT
  }
  write_lines [file join $evidence_root "${stage}_RESOURCE_GATE.txt"] [list \
    ...
    "PREVIOUS_DIAG1_LUT=$previous_diag1_lut" \
    "LUT_REDUCTION_VS_DIAG1=$lut_reduction" \
    "LUT_REDUCTION_REQUIRED=$required_lut_reduction" \
    "LUT_HARD_MAX=$post_opt_lut_hard_max" \
    "LUT_ENGINEERING_TARGET_MAX=$post_opt_lut_target_max" \
    "LUT_PREFERRED_MAX=$post_opt_lut_preferred_max" \
    "LUT_HARD_GATE=[expr {$lut_used <= $post_opt_lut_hard_max ? {PASS} : {FAIL}}]" \
    "LUT_REDUCTION_GATE=[expr {$lut_reduction >= $required_lut_reduction ? {PASS} : {FAIL}}]" \
    "LUT_ENGINEERING_TARGET=[expr {$lut_used <= $post_opt_lut_target_max ? {PASS} : {ADVISORY_MISS}}]" \
    "LUT_PREFERRED_TARGET=[expr {$lut_used <= $post_opt_lut_preferred_max ? {PASS} : {ADVISORY_MISS}}]" \
...
proc enforce_exact_cdc_disposition {cdc_path cdc_violations} {
  ...
  set expected_hashes [dict create \
    CDC-1 PROFILE_SPECIFIC_R3_PENDING \
  ...
  foreach id [list CDC-1 CDC-10 CDC-13] {
    ...
    if {$id ne "CDC-1" && $actual_hash ne $expected_hash} {
  ...
  set provisional_lines [list \
    {RESULT=PASS_RAW_PHYSICAL_ROWS_PENDING_SEMANTIC_V3} \
    {CDC_1_EXPECTED_COUNT=423} \
    "CDC_1_ACTUAL_SHA256=[sha256_file [file join $evidence_root G2B_CDC_CRITICAL_CDC_1_CANONICAL.txt]]" \
    "CRITICAL_TOTAL_SHA256=$total_hash"]
  foreach family [dict keys $expected_family_counts] {
    lappend provisional_lines "${family}_PREVIOUS_COUNT=[dict get $expected_family_counts $family]" \
      "${family}_R3_PHYSICAL_REPRESENTATIVE_COUNT=[dict get $family_counts $family]"
  }
  lappend provisional_lines \
    {FINAL_SEMANTIC_DISPOSITION=REQUIRED_BEFORE_BITSTREAM} \
    {BITSTREAM_AUTHORIZED_BY_THIS_RECEIPT=NO}
  write_lines [file join $evidence_root G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_CDC_PHYSICAL_HANDOFF.txt] $provisional_lines
...
proc require_receipt_value {values key expected} {
  if {![dict exists $values $key]} {
    error "committed simulation receipt is missing $key"
  }
  set actual [dict get $values $key]
  if {$actual ne $expected} {
    error "committed simulation receipt mismatch for $key: expected '$expected', got '$actual'"
  }
}

proc validate_committed_simulation_receipt {} {
  global simulation_receipt repo_root source_commit source_tree expected_branch
  global expected_simulation_receipt_sha task_root
  set expected_path [file normalize [file join $task_root tests \
    G2B_NVP_ACQ1_COMPAT0_R2_COMBINED_TEST_RECEIPT.txt]]
  if {[path_key $simulation_receipt] ne [path_key $expected_path]} {
    error "simulation receipt path mismatch: expected $expected_path, got $simulation_receipt"
  }
  if {![file isfile $simulation_receipt] || [file size $simulation_receipt] == 0} {
    error "committed simulation receipt is missing or empty: $simulation_receipt"
  }
  set receipt_sha [sha256_file $simulation_receipt]
  if {$receipt_sha ne $expected_simulation_receipt_sha} {
    error "committed simulation receipt SHA-256 mismatch"
  }
  set values [fix1_r1_read_key_values $simulation_receipt]
  foreach {key expected} {
    ...
    REQUIRED_PARENT c7e16fa3da26545cef960a6c75427a3614c4b655
    ACQ1_EXECUTOR_INTEGRATION_GATE {20/20 PASS}
    ACQ1_UNAUTHORIZED_FUNCTIONAL_WRITE_COUNT 0
    SCAN1_SIMULATION_AND_HOST_GATE {24/24 PASS}
    SCAN1_CLOSED_RUNTIME_BUNDLE_GATE PASS
    COMBINED_OFFLINE_GATE {44/44 PASS}
    HARDWARE_ACCESSED NO
  } {
    require_receipt_value $values $key $expected
  }
  return [list $receipt_sha $values]
}

proc validate_r1_source_architecture {} {
  global repo_root expected_diag_version expected_diag_capabilities
  global expected_acq_magic expected_acq_version expected_acq_capabilities
  set core_path [file join $repo_root rtl g2b g2b_nvp_camera_scan1.sv]
  set package_path [file join $repo_root rtl g2b g2b_nvp_camera_scan1_manifest_pkg.sv]
  set executor_path [file join $repo_root rtl g2b g2b_nvp_acq1_compat0_r2.sv]
  set wrapper_path [file join $repo_root rtl g2b g2b_nvp_camera_scan1_acq1_compat0_r2.sv]
  set top_path [file join $repo_root rtl top ahd_capture_top_xdma.sv]
  set core [read_text $core_path]
  set package [read_text $package_path]
  set executor [read_text $executor_path]
  set wrapper [read_text $wrapper_path]
  set top [read_text $top_path]
  foreach pair [list \
      [list {SCAN1_ENTRY_COUNT = 82} $package] \
      [list {SCAN1_GROUP_COUNT = 10} $package] \
      [list {SCAN1_CLEAN_TRANSACTION_COUNT = 105} $package] \
      [list {SCAN1_RAW_SEMANTICS_PARTIAL_COUNT = 40} $package] \
      [list {SCAN1_MAGIC = 32'h4E565343} $package] \
      [list {SCAN1_VERSION = 32'h00010001} $package] \
      [list {SCAN1_CAPABILITIES = 32'h0000000F} $package] \
      [list {parameter integer ENABLE_NVP_CAMERA_SCAN1 = 0} $top] \
      [list {parameter integer ENABLE_NVP_ACQ1_EXECUTOR = 0} $top] \
      [list {ENABLE_NVP_CAMERA_SCAN1 != 0} $top] \
      [list {legacy_host_req_addr >= 17'h12000} $top] \
      [list {legacy_host_req_addr <= 17'h123ff} $top] \
      [list {legacy_host_req_addr >= 17'h12400} $top] \
      [list {legacy_host_req_addr <= 17'h127ff} $top] \
      [list {mmio_req_addr >= 17'h12000} $wrapper] \
      [list {mmio_req_addr <= 17'h123FF} $wrapper] \
      [list {mmio_req_addr >= 17'h12400} $wrapper] \
      [list {mmio_req_addr <= 17'h127FF} $wrapper] \
      [list {ACQ_MAGIC = 32'h4E56_4143} $executor] \
      [list {ACQ_VERSION = 32'h0001_0000} $executor] \
      [list {ACQ_CAPABILITIES = 32'h0000_00FF} $executor] \
      [list {g2b_nvp_camera_scan1 SCAN1_CORE} $wrapper] \
      [list {g2b_nvp_acq1_compat0_r2 ACQ_EXECUTOR} $wrapper]] {
    lassign $pair literal body
    if {[string first $literal $body] < 0} {
      error "SCAN1 source architecture is missing exact literal: $literal"
    }
  }
  if {$expected_diag_version ne "0x00010001" ||
      $expected_diag_capabilities ne "0x0000000F" ||
      $expected_acq_magic ne "0x4E564143" ||
      $expected_acq_version ne "0x00010000" ||
      $expected_acq_capabilities ne "0x000000FF"} {
    error "internal SCAN1 diagnostic identity constants are inconsistent"
  }
  if {[regexp -all -- {i2c_cmd_write = 1'b1;} $core] != 2 ||
      [regexp -all -- {i2c_cmd_reg = BANK_SELECT;} $core] != 5 ||
      [regexp -all -- {i2c_cmd_wdata =} $core] != 3} {
    error "SCAN1 source contains an unexpected I2C write architecture"
  }
  foreach forbidden {runtime_timeout speed_select nine_pulse_recovery generic_host_i2c} {
    if {[regexp -nocase -- $forbidden $core]} {
      error "SCAN1 source architecture contains forbidden token: $forbidden"
    }
  }
  return [dict create \
    READ_MANIFEST_ENTRIES 82 BANK_GROUPS 10 EXPECTED_CLEAN_TRANSACTIONS 105 \
    UNRESOLVED_SEMANTICS_ENTRIES 40 PROHIBITED_FUNCTIONAL_WRITES 0 \
    GENERIC_HOST_I2C_WRITE_PATH ABSENT PRODUCT_DEFAULT_SCAN1 0 \
    PRODUCT_DEFAULT_ACQ1 0 SCAN1_MMIO_RANGE 0x12000..0x123FF \
    ACQ1_MMIO_RANGE 0x12400..0x127FF ACQ1_MMIO_ALIAS_COUNT 0 \
    DIAG_VERSION $expected_diag_version DIAG_CAPABILITIES $expected_diag_capabilities \
    ACQ_MAGIC $expected_acq_magic ACQ_VERSION $expected_acq_version \
    ACQ_CAPABILITIES $expected_acq_capabilities \
    RESULT PASS]
}

proc gate_r1_result_storage_netlist {stage} {
  global evidence_root
  set wrapper_cells [get_cells -quiet -hier -regexp {.*NVP_CAMERA_ACQ1_COMPAT0_R2_CORE.*}]
  set scanner_cells [get_cells -quiet -hier -regexp {.*SCAN1_CORE.*}]
  set legacy_cells [get_cells -quiet -hier -regexp {.*NVP_VIDEO_DIAG_CORE.*}]
  set acq1_cells [get_cells -quiet -hier -regexp {.*ACQ_EXECUTOR.*}]
  set generic_host_cells [get_cells -quiet -hier -regexp {.*(generic.*i2c|host.*i2c).*}]
  set snapshot_cells [get_cells -quiet -hier -regexp {.*SCAN1_CORE.*snapshot_entry.*}]
  if {[llength $wrapper_cells] == 0 || [llength $scanner_cells] == 0 ||
      [llength $legacy_cells] != 0 || [llength $acq1_cells] == 0 ||
      [llength $generic_host_cells] != 0 ||
      [llength $snapshot_cells] == 0} {
    error "$stage ACQ1 netlist isolation/storage gate failed: wrapper=[llength $wrapper_cells] scanner=[llength $scanner_cells] legacy=[llength $legacy_cells] acq1=[llength $acq1_cells] generic_host=[llength $generic_host_cells] snapshot=[llength $snapshot_cells]"
  }
  write_lines [file join $evidence_root "${stage}_SCAN1_NETLIST_ARCHITECTURE.txt"] [list \
    "STAGE=$stage" \
    "ACQ1_WRAPPER_HIERARCHY_OBJECTS=[llength $wrapper_cells]" \
    "SCAN1_HIERARCHY_OBJECTS=[llength $scanner_cells]" \
    "SNAPSHOT_STORAGE_OBJECTS=[llength $snapshot_cells]" \
    "LEGACY_DIAG1_OBJECTS=[llength $legacy_cells]" \
    "ACQ1_OBJECTS=[llength $acq1_cells]" \
    "GENERIC_HOST_I2C_OBJECTS=[llength $generic_host_cells]" \
    {SINGLE_DOMAIN_SCANNER_EXECUTOR_AXI_I2C=YES} \
    {RESULT=PASS}]
  if {$stage eq "POST_SYNTH"} {
    set scan_first 0x12000
    set scan_last 0x123ff
    set acq_first 0x12400
    set acq_last 0x127ff
    set overlap_count 0
    for {set address 0} {$address <= 0x1ffff} {incr address} {
      set scan_hit [expr {$address >= $scan_first && $address <= $scan_last}]
      set acq_hit [expr {$address >= $acq_first && $address <= $acq_last}]
      if {$scan_hit && $acq_hit} { incr overlap_count }
    }
    if {$overlap_count != 0 || ($scan_last - $scan_first + 1) != 1024 ||
        ($acq_last - $acq_first + 1) != 1024} {
      error "post-synthesis MMIO no-alias gate failed"
    }
    write_lines [file join $evidence_root G2B_ACQ1_MMIO_POST_SYNTH_NO_ALIAS_PROOF.txt] [list \
      {STAGE=POST_SYNTH} {SCAN1_RANGE=0x12000..0x123FF} \
      {ACQ1_RANGE=0x12400..0x127FF} {SCAN1_ADDRESS_COUNT=1024} \
      {ACQ1_ADDRESS_COUNT=1024} "OVERLAP_ADDRESS_COUNT=$overlap_count" \
      "WRAPPER_HIERARCHY_OBJECTS=[llength $wrapper_cells]" \
      "SCAN1_HIERARCHY_OBJECTS=[llength $scanner_cells]" \
      "ACQ1_HIERARCHY_OBJECTS=[llength $acq1_cells]" \
      {GENERIC_HOST_I2C_OBJECTS=0} {RESULT=PASS}]
  }
  return PASS
}
...
proc fix1_r1_recovery_main {} {
  error "ROUTED_DCP_RECOVERY is disabled for ACQ1-COMPAT0-R2"
...
# Path and argument validation happens before the single full-build mode writes.
...
if {$bit_filename ne $expected_bit_filename} {
  error "BIT_FILENAME must be exactly $expected_bit_filename"
}
if {![regexp {^G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_[0-9]{8}T[0-9]{6}Z$} [file tail $task_root]] ||
    [path_key $evidence_root] ne [path_key [file join $task_root reports vivado_full]] ||
    [path_key $artifact_root] ne [path_key [file join $task_root signoff]]} {
  error "R1 task/evidence/artifact root layout mismatch"
}
...
if {$source_commit ne $expected_r1_source_commit || $source_tree ne $expected_r1_source_tree} {
  error "requested R1 source identity is not the final sealed commit/tree"
}

# Exact ACQ1 source identity.  Owner continuity makes broad PRODUCT/G2A/tag
# requalification out of scope; branch, HEAD/tree, tracked cleanliness, and
# the exact accepted SCAN1 direct parent are checked.
...
set actual_status [git_value status --porcelain --untracked-files=no]
...
if {$actual_status ne ""} { error "R1 source has tracked changes: $actual_status" }
...
set precommit_manifest_sha DISABLED
set precommit_manifest_line_count NOT_APPLICABLE
set precommit_manifest_display DISABLED
set actual_merge_base [git_value merge-base $expected_parent_commit $source_commit]
set direct_parent [git_value rev-parse "${source_commit}^"]
set integration_commit_count \
  [git_value rev-list --count "${expected_parent_commit}..${source_commit}"]
if {$actual_merge_base ne $expected_parent_commit ||
    $direct_parent ne $expected_parent_commit || $integration_commit_count != 1} {
  error "ACQ1 source is not the exact one-commit child of the accepted SCAN1 source"
}
set direct_base_relation PASS_EXACT_DIRECT_CHILD_OF_ACCEPTED_SCAN1
...
lassign [validate_committed_simulation_receipt] \
  simulation_receipt_sha simulation_receipt_values
set source_architecture [validate_r1_source_architecture]
...
# BUILD_FLAGS[11] identifies the read-only SCAN1 diagnostic.  Bits [1:0]
# retain the established clean/dirty manifest provenance semantics.
set profile_build_flag 0x00000800
...
append generics " ENABLE_NVP_VIDEO_DIAGNOSTIC=$enable_nvp_video_diagnostic"
append generics " ENABLE_NVP_CAMERA_SCAN1=$enable_nvp_camera_scan1"
append generics " ENABLE_NVP_ACQ1_EXECUTOR=$enable_nvp_acq1_executor"
set product_generics [string map [list \
  "BUILD_FLAGS=$build_flags" "BUILD_FLAGS=32'h00000002" \
  {ENABLE_NVP_CAMERA_SCAN1=1} {ENABLE_NVP_CAMERA_SCAN1=0} \
  {ENABLE_NVP_ACQ1_EXECUTOR=1} {ENABLE_NVP_ACQ1_EXECUTOR=0}] $generics]

# The evidence root is the only path created in provenance-only mode.
file mkdir $evidence_root
set provenance_lines [list \
  "TASK=$expected_task" \
  "CANDIDATE_CLASSIFICATION=$expected_candidate_classification" \
  "DIAGNOSTIC_MMIO=G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_MMIO_V1" \
  "DIAGNOSTIC_VERSION=$expected_diag_version" \
  "DIAGNOSTIC_CAPABILITIES=$expected_diag_capabilities" \
  "BUILD_PROFILE=$build_profile" \
  "ENABLE_RTRACK_DIAGNOSTICS=$enable_rtrack_diagnostics" \
  "ENABLE_NVP_VIDEO_DIAGNOSTIC=$enable_nvp_video_diagnostic" \
  "ENABLE_NVP_CAMERA_SCAN1=$enable_nvp_camera_scan1" \
  "ENABLE_NVP_ACQ1_EXECUTOR=$enable_nvp_acq1_executor" \
  ...
  "ACCEPTED_HARDENED_PRODUCT_COMMIT=$accepted_hardened_product_commit" \
  "ACCEPTED_HARDENED_PRODUCT_TREE=$accepted_hardened_product_tree" \
  ...
  "ACCEPTED_SCAN1_PARENT=$expected_parent_commit" \
  "MERGE_BASE_WITH_ACCEPTED_SCAN1=$actual_merge_base" \
  ...
  "SIMULATION_RECEIPT_SHA256=$simulation_receipt_sha" \
  {COMBINED_OFFLINE_GATE=44/44_PASS} \
  {SCAN1_SIMULATION_AND_HOST_GATE=24/24_PASS} \
  {ACQ1_EXECUTOR_INTEGRATION_GATE=20/20_PASS} \
  ...
  "BUILD_FLAGS_PRODUCT_BIT=0" \
  "BUILD_FLAGS_RESEARCH_DIAGNOSTIC_BIT=0" \
  "BUILD_FLAGS_SCAN1_COMPAT_PROFILE_BIT=1" \
  "ACQ1_EXECUTOR_PROFILE_ENABLED=$enable_nvp_acq1_executor" \
  "PROVENANCE_ROUND_TRIP=PASS" \
  "VIVADO_GENERIC_STRING=$generics" \
  "PROVENANCE_PREFLIGHT=PASS"]
lappend provenance_lines {PROVENANCE_ONLY_MODE=DISABLED}
write_lines [file join $evidence_root G2B_EXPECTED_RUNTIME_PROVENANCE.txt] $provenance_lines
set source_architecture_lines [list {RESULT_STORAGE_ARCHITECTURE_GATE=PASS}]
dict for {key value} $source_architecture {
  lappend source_architecture_lines "$key=$value"
}
write_lines [file join $evidence_root G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_SOURCE_ARCHITECTURE_GATE.txt] \
  $source_architecture_lines
puts "G2B_PROVENANCE_ROUND_TRIP=PASS REPOSITORY_HEAD=$actual_commit HEAD_TREE=$actual_tree IDENTITY=$source_identity_kind"
# No preflight-only exit exists: this invocation must continue through all gates.
...
set cdc_warning UNKNOWN
set cdc_unknown UNKNOWN
set cdc_critical_dispositioned 0
set cdc_warning_dispositioned 0
set unresolved_critical_cdc UNKNOWN
set unresolved_warning_cdc UNKNOWN
set cdc_disposition NOT_RUN
set methodology_errors UNKNOWN
set methodology_critical_warnings UNKNOWN
set methodology_warnings UNKNOWN
...
set bit_path [file join $artifact_root $bit_filename]
set ltx_filename "[file rootname $bit_filename].ltx"
set ltx_path [file join $artifact_root $ltx_filename]
...
set build_result [catch {
  ...
  dict set allowed_preexisting [path_key [file join $evidence_root \
    G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_SOURCE_ARCHITECTURE_GATE.txt]] 1
  ...
  create_project v41_g2b_nvp_camera_acq1_compat0_r2 $project_dir -part $expected_part
  ...
  # Source, elaboration and RTL-netlist proof for the protected PRODUCT
  # profile.  This does not optimize, place, route, write a checkpoint, or
  # create a PRODUCT bitstream.  The in-memory RTL design is closed before the
  # sole full SCAN1 synthesis begins.
  set stage PRODUCT_EXCLUSION_ELABORATION
  set_property generic $product_generics [get_filesets sources_1]
  synth_design -rtl -name product_exclusion_rtl -top $expected_top \
    -part $expected_part -flatten_hierarchy rebuilt
  set product_wrapper_cells [get_cells -quiet -hier -regexp {.*NVP_CAMERA_ACQ1_COMPAT0_R2_CORE.*}]
  set product_scan1_cells [get_cells -quiet -hier -regexp {.*SCAN1_CORE.*}]
  set product_scan1_ref_cells [get_cells -quiet -hier -filter {REF_NAME =~ g2b_nvp_camera_scan1*}]
  set product_legacy_diag_cells [get_cells -quiet -hier -regexp {.*NVP_VIDEO_DIAG_CORE.*}]
  set product_acq1_cells [get_cells -quiet -hier -regexp {.*ACQ_EXECUTOR.*}]
  set product_acq1_ref_cells [get_cells -quiet -hier -filter {REF_NAME =~ g2b_nvp_acq1_compat0_r2*}]
  set product_diag_master_cells [get_cells -quiet -hier -filter {REF_NAME =~ nvp_i2c_fixed_master*}]
  set product_scan1_decode_nets [get_nets -quiet -hier -regexp {.*scan1_nvp_diag_select.*}]
  set product_acq1_decode_nets [get_nets -quiet -hier -regexp {.*acq1_nvp_diag_select.*}]
  if {[llength $product_wrapper_cells] != 0 || [llength $product_scan1_cells] != 0 ||
      [llength $product_scan1_ref_cells] != 0 ||
      [llength $product_legacy_diag_cells] != 0 ||
      [llength $product_acq1_cells] != 0 ||
      [llength $product_acq1_ref_cells] != 0 ||
      [llength $product_diag_master_cells] != 0 ||
      [llength $product_scan1_decode_nets] != 0 ||
      [llength $product_acq1_decode_nets] != 0} {
    error "PRODUCT exclusion elaboration failed: wrapper=[llength $product_wrapper_cells] scan1_instance=[llength $product_scan1_cells] scan1_ref=[llength $product_scan1_ref_cells] legacy=[llength $product_legacy_diag_cells] acq1=[llength $product_acq1_cells] acq1_ref=[llength $product_acq1_ref_cells] diag_master=[llength $product_diag_master_cells] scan1_decode=[llength $product_scan1_decode_nets] acq1_decode=[llength $product_acq1_decode_nets]"
  }
  write_lines [file join $evidence_root G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_PRODUCT_EXCLUSION_ELABORATION.txt] [list \
    {RESULT=PASS} {PROFILE=PRODUCT} {SCAN1_INSTANCES=0} {ACQ1_INSTANCES=0} \
    {LEGACY_DIAG1_INSTANCES=0} {SCAN1_MMIO_DECODE_NETS=0} {ACQ1_MMIO_DECODE_NETS=0} \
    {SCAN1_I2C_MASTER_INSTANCES=0} {SCAN1_I2C_OWNERSHIP=CONSTANT_INACTIVE} \
    {SCAN1_SCL_SDA_DRIVE_LOW_CONTRIBUTIONS=ABSENT_OR_CONSTANT_RELEASED} \
    {PRODUCT_BITSTREAM_GENERATED=NO}]
  close_design
  set_property generic $generics [get_filesets sources_1]
  update_compile_order -fileset sources_1
  if {[get_property GENERIC [get_filesets sources_1]] ne $generics} {
    error "SCAN1 generics were not restored after PRODUCT exclusion elaboration"
  }
  ...
  set profile_wrapper_cells [profile_cells NVP_CAMERA_ACQ1_COMPAT0_R2_CORE \
    g2b_nvp_camera_scan1_acq1_compat0_r2]
  set profile_nvp_diag_core_cells [profile_cells SCAN1_CORE \
    g2b_nvp_camera_scan1]
  set profile_nvp_diag_i2c_cells [profile_cells NVP_DIAGNOSTIC_I2C_MASTER \
    nvp_i2c_fixed_master]
  set profile_legacy_diag_cells [profile_cells NVP_VIDEO_DIAG_CORE \
    g2b_nvp_video_diag]
  set profile_acq1_cells [profile_cells ACQ_EXECUTOR g2b_nvp_acq1_compat0_r2]
  ...
  set profile_wrapper_count [llength $profile_wrapper_cells]
  set profile_nvp_diag_core_count [llength $profile_nvp_diag_core_cells]
  set profile_nvp_diag_i2c_count [llength $profile_nvp_diag_i2c_cells]
  set profile_legacy_diag_count [llength $profile_legacy_diag_cells]
  set profile_acq1_count [llength $profile_acq1_cells]
  ...
  if {$profile_probe_count != 0 || $profile_lifecycle_count != 0 ||
      $profile_failed_history_count != 0 || $profile_r1h_count != 0 ||
       $profile_product_service_count == 0 || $profile_wrapper_count != 1 ||
       $profile_nvp_diag_core_count != 1 ||
       $profile_nvp_diag_i2c_count != 1 ||
       $profile_legacy_diag_count != 0 || $profile_acq1_count != 1} {
  ...
  write_lines [file join $evidence_root G2B_PROFILE_ELABORATION_RECEIPT.txt] [list \
    ...
    "NVP_CAMERA_ACQ1_COMPAT0_R2_WRAPPER_COUNT=$profile_wrapper_count" \
    "NVP_CAMERA_SCAN1_CORE_COUNT=$profile_nvp_diag_core_count" \
    "NVP_DIAGNOSTIC_I2C_MASTER_COUNT=$profile_nvp_diag_i2c_count" \
    "LEGACY_NVP_VIDEO_DIAG_CORE_COUNT=$profile_legacy_diag_count" \
    "ACQ1_EXECUTOR_COUNT=$profile_acq1_count" \
    "PROFILE_ELABORATION_GATE=$profile_gate"]
  if {$profile_gate ne "PASS"} {
    error "build-profile elaboration content mismatch"
  }
  set synthesis PASS
  gate_r1_result_storage_netlist POST_SYNTH
  set synth_dcp_path [file join $evidence_root G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_SYNTH.dcp]
  ...
  gate_r1_result_storage_netlist POST_OPT
  set post_opt_metrics [resource_metrics POST_OPT]
  set post_opt_dcp_path [file join $evidence_root G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_POST_OPT.dcp]
  ...
  if {$post_opt_resource_gate ne "PASS"} {
    ...
    if {[string match {LUT_*} $post_opt_resource_gate_reason]} {
      error "FAIL - NVP_CAMERA_ACQ1_COMPAT0_R2_R1_POST_OPT_LUT_GATE_FAILED: $post_opt_resource_gate_reason"
    }
    error "FAIL - NVP_CAMERA_ACQ1_COMPAT0_R2_R1_POST_OPT_RESOURCE_GATE_FAILED: $post_opt_resource_gate_reason"
  }
  ...
  set routed_dcp_path [file join $evidence_root G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_ROUTED.dcp]
  ...
  if {$bus_skew_violations != 0 || $bus_skew_met_constraints != 11} {
    error "active relative bus-skew gate failed: violations=$bus_skew_violations pass=$bus_skew_met_constraints/11"
  }
  fix1_r1_source_retired_absence_gate
  set promoted_replacement_checks [fix1_r1_run_promoted_replacements]
  if {$promoted_replacement_checks != 17} {
    error "promoted replacement-method gate failed: pass=$promoted_replacement_checks/17"
  }
  fix1_r1_structural_cdc_gate
  fix1_r1_write_all_groups_matrix
  ...
  set methodology_path [file join $evidence_root METHODOLOGY.rpt]
  report_methodology -file $methodology_path
  if {[catch {get_methodology_violations -quiet} methodology_violations]} {
    set methodology_gate FAIL
    error "methodology violation objects unavailable: $methodology_violations"
  }
  set methodology_errors 0
  set methodology_critical_warnings 0
  set methodology_warnings 0
  foreach violation $methodology_violations {
    set severity [string toupper [property_or_unknown SEVERITY $violation]]
    if {$severity eq "ERROR"} { incr methodology_errors }
    if {$severity eq "CRITICAL WARNING"} { incr methodology_critical_warnings }
    if {$severity eq "WARNING"} { incr methodology_warnings }
  }
  if {$methodology_errors != 0 || $methodology_critical_warnings != 0} {
    set methodology_gate FAIL
    error "methodology gate failed: errors=$methodology_errors critical_warnings=$methodology_critical_warnings"
  }
  set methodology_gate PASS
  write_lines [file join $evidence_root G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_METHODOLOGY_GATE.txt] [list \
    {METHODOLOGY=PASS} "METHODOLOGY_ERRORS=$methodology_errors" \
    "METHODOLOGY_CRITICAL_WARNINGS=$methodology_critical_warnings" \
    "METHODOLOGY_WARNINGS=$methodology_warnings" {NO_WAIVER_OR_MESSAGE_SUPPRESSION=YES}]
  ...
  foreach violation $cdc_violations {
    ...
    if {$severity eq "WARNING"} {
      incr cdc_warning
      lappend cdc_warning_objects $violation
    }
  ...
  if {$cdc_critical != 0} {
    ...
    set cdc_disposition PASS_RAW_PROFILE_COUNTS_PENDING_SCAN1_SEMANTIC_RECONCILIATION
  }
  ...
  fix1_r1_check_rule_counts CDC_WARNING $cdc_warning_objects \
    [dict create CDC-6 13 CDC-15 861] {WARNING} \
    [file join $evidence_root G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_CDC_WARNING_OBJECTS.csv]
  set cdc_warning_dispositioned $cdc_warning
  set unresolved_critical_cdc [expr {$cdc_critical - $cdc_critical_dispositioned}]
  set unresolved_warning_cdc [expr {$cdc_warning - $cdc_warning_dispositioned}]
  if {$unresolved_critical_cdc != 0 || $unresolved_warning_cdc != 0} {
    set cdc_gate FAIL
    error "CDC unresolved-finding gate failed: critical=$unresolved_critical_cdc warning=$unresolved_warning_cdc"
  }
  set cdc_gate PASS

  if {$bus_skew_violations != 0 || $bus_skew_met_constraints != 11 ||
      $promoted_replacement_checks != 17} {
    set bus_skew_gate FAIL
    error "governed timing-method gate failed: active_violations=$bus_skew_violations active_pass=$bus_skew_met_constraints/11 promoted_pass=$promoted_replacement_checks/17"
  }
  ...
  write_lines [file join $evidence_root G2B_FINAL_CDC_GATE.txt] [list \
    ...
    "CDC_WARNING=$cdc_warning" \
    "CDC_CRITICAL_DISPOSITIONED=$cdc_critical_dispositioned" \
    "CDC_WARNING_DISPOSITIONED=$cdc_warning_dispositioned" \
    "UNRESOLVED_CRITICAL_CDC=$unresolved_critical_cdc" \
    "UNRESOLVED_WARNING_CDC=$unresolved_warning_cdc" \
    ...
    "PROMOTED_REPLACEMENT_CHECKS=$promoted_replacement_checks" \
    {PROMOTED_REPLACEMENT_GROUPS=6} \
    {PROMOTED_REPLACEMENT_GROUP_IDS=9,13,14,15,16,17} \
  ...
  write_lines [file join $evidence_root G2B_PRE_BITSTREAM_HARD_GATE.txt] [list \
    ...
    "DIAGNOSTIC_VERSION=$expected_diag_version" \
    "DIAGNOSTIC_CAPABILITIES=$expected_diag_capabilities" \
    "ACQ_MAGIC=$expected_acq_magic" \
    "ACQ_VERSION=$expected_acq_version" \
    "ACQ_CAPABILITIES=$expected_acq_capabilities" \
    "SOURCE_IDENTITY_KIND=$source_identity_kind" \
    "REPOSITORY_HEAD=$source_commit" "REPOSITORY_HEAD_TREE=$source_tree" \
    "PRECOMMIT_INPUT_MANIFEST_SHA256=$precommit_manifest_sha" \
    ...
    "BUS_SKEW_VIOLATIONS=$bus_skew_violations" \
    "BUS_SKEW_MET_CONSTRAINTS=$bus_skew_met_constraints" \
    {ACTIVE_BUS_SKEW_GROUP_COUNT=11} \
    {PROMOTED_REPLACEMENT_CHECK_COUNT=17} \
    {PROMOTED_REPLACEMENT_GROUP_COUNT=6} \
    {PROMOTED_REPLACEMENT_GROUP_IDS=9,13,14,15,16,17} \
    "BUS_SKEW_REPORT_SHA256=$bus_skew_report_sha" \
  ...
  # SCAN1 requires a profile-specific semantic CDC reconciliation against the
  # exact fresh routed DCP.  No bitstream or debug-probe output is permitted in
  # this implementation-stage process.  The separate sign-off harness is the
  # sole authorized bitstream writer after that reconciliation passes.
  set stage ROUTED_BUILD_HANDOFF
  write_lines [file join $evidence_root G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_ROUTED_BUILD_HANDOFF.txt] [list \
    {RESULT=PASS} \
    "SOURCE_COMMIT=$source_commit" \
    "SOURCE_TREE=$source_tree" \
    "ROUTED_DCP=$routed_dcp_path" \
    "ROUTED_DCP_SHA256=$routed_dcp_sha" \
    {SEMANTIC_CDC_MANIFEST_V3=PENDING} \
    {BITSTREAM_PRODUCED=NO} \
    {WRITE_BITSTREAM_COUNT=0} \
    {WRITE_DEBUG_PROBES_COUNT=0}]

  if {![source_seal_is_current]} {
    error "source identity or sealed input manifest changed after build"
  }
  set source_post_build PASS
  set stage COMPLETE_ROUTED_BUILD_HANDOFF
...
set terminal_lines [list \
  ...
  "REPOSITORY_HEAD=$source_commit" "REPOSITORY_HEAD_TREE=$source_tree" \
  "SOURCE_CLEAN=$source_clean" \
  "ACQ1_SOURCE_PARENT=$expected_parent_commit" \
  "ACQ1_SOURCE_DIRECT_PARENT=$direct_parent" \
  "ACQ1_SOURCE_COMMITS_AFTER_ACCEPTED_SCAN1=$integration_commit_count" \
  "PRECOMMIT_INPUT_MANIFEST_SHA256=$precommit_manifest_sha" \
  "SIMULATION_RECEIPT=$simulation_receipt" \
  "SIMULATION_RECEIPT_SHA256=$simulation_receipt_sha" \
  {COMBINED_OFFLINE_GATE=44/44_PASS} \
  ...
  "BUS_SKEW_MET_CONSTRAINTS=$bus_skew_met_constraints" \
  "PROMOTED_REPLACEMENT_CHECKS=[expr {[info exists promoted_replacement_checks] ? $promoted_replacement_checks : {UNKNOWN}}]" \
  {ACTIVE_BUS_SKEW_GROUP_COUNT=11} \
  {PROMOTED_REPLACEMENT_CHECK_COUNT=17} \
  {PROMOTED_REPLACEMENT_GROUP_COUNT=6} \
  {PROMOTED_REPLACEMENT_GROUP_IDS=9,13,14,15,16,17} \
  ...
  "BITSTREAM_SIZE=$bit_size" "BITSTREAM_SHA256=$bit_sha" "LTX_PATH=$ltx_path" \
  ...
  "NVP_CAMERA_SCAN1_RTL_INCLUDED=YES" \
  "NVP_ACQ1_COMPAT0_R2_EXECUTOR_RTL_INCLUDED=YES" \
...
foreach action {WRITE_BITSTREAM WRITE_DEBUG_PROBES} {
  if {[dict get $action_counts $action] != 0} {
    lappend terminal_lines "BUILD=FAIL" "ERROR=unauthorized pre-reconciliation output count for $action"
    write_lines [file join $evidence_root G2B_BUILD_RESULT.txt] $terminal_lines
    catch {close_project}
    exit 1
  }
}
lappend terminal_lines "BUILD=PASS_ROUTED_HANDOFF" \
  "SOURCE_TO_BIT_PROVENANCE=$source_to_bit_provenance"
write_lines [file join $evidence_root G2B_BUILD_RESULT.txt] $terminal_lines
catch {close_project}
puts "G2B_NVP_CAMERA_ACQ1_COMPAT0_R2_ROUTED_BUILD_HANDOFF_PASS ROUTED_DCP_SHA256=$routed_dcp_sha BITSTREAM_PRODUCED=NO"
exit 0
