...
$target=[regex]::Match($text,'(?m)^R1G_FULL_JTAG_TARGET_PATH=([^\r\n]+)\r?$')
...
$out=Join-Path $script:R1gPrecheckRoot 'FORMAL_START_READY_RECEIPT.txt'
if(Test-Path -LiteralPath $out){throw 'refusing to overwrite formal-start receipt'}
Write-R1gUtf8NoBom -Path $out -Lines @(
    'RECEIPT_TYPE=FORMAL_READY_RECEIPT','RECEIPT_STATUS=PASS','PHASE_TOKEN=EXISTING_FORMAL_START',
    "R1G_FULL_JTAG_TARGET_PATH=$([string]$binding.selectedFullJtagTargetPath)",'FPGA_DONE=1',
