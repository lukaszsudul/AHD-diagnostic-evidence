...
proc write_region_inventory {evidence_root label pattern} {
  ...
  set filename [format "R1F_%s_INVENTORY.txt" $label]
  write_lines [file join $evidence_root $filename] $lines
  if {[llength $cells] + [llength $nets] == 0} {
    error "required routed R1f design region has no cells or nets: $label"
  }
...
proc verify_prebuild_manifest {
  ...
  required_source_paths required_log_labels
} {
...
if {[llength [directory_entries $build_root]] != 0} {
  error "R1f build root must be new or empty: $build_root"
}
...
if {$merge_base ne $exact_r1e_base_commit} {
  error "R1f source is not based on exact R1e commit $exact_r1e_base_commit"
}
...
set manifest_summary [verify_prebuild_manifest \
  ...
  $source_commit $source_tree $required_manifest_sources $required_log_labels]
...
write_lines [file join $evidence_root R1F_PREBUILD_MANIFEST_BINDING.txt] \
  $manifest_summary
file copy -force $prebuild_manifest \
  [file join $evidence_root R1F_BOUND_PREBUILD_MANIFEST.txt]
write_lines [file join $evidence_root R1F_FROZEN_INPUT_SHA256.txt] \
  $frozen_input_lines
...
write_lines [file join $evidence_root R1F_PLANNED_SOURCE_AND_CONSTRAINT_ORDER.txt] \
  $planned_inventory
...
set flow_rc [catch {
  ...
  write_lines [file join $evidence_root R1F_XDMA_IMPORTED_PROPERTY_AUDIT.txt] \
    $xdma_property_lines
  ...
  write_lines [file join $evidence_root R1F_XDC_ORDER_AND_SHA256.txt] \
    $xdc_order_lines
  ...
  foreach required [list \
    ...
    if {[compile_order_index $actual_compile_names $required] >=
        $top_compile_index} {
      error "R1f SystemVerilog dependency is not before the queried top compile unit: $required"
    }
  }
  write_lines [file join $evidence_root R1F_QUERIED_SYNTHESIS_COMPILE_ORDER.txt] \
    $actual_compile_lines

  write_lines [file join $evidence_root R1F_BUILD_PROVENANCE.txt] [list \
    "SOURCE_GIT_COMMIT=$source_commit" \
    "SOURCE_GIT_TREE=$source_tree" \
    "SOURCE_BRANCH=$actual_branch" \
    "SOURCE_TREE_CLEAN=YES" \
  ...
  report_timing_summary -delay_type min_max -max_paths 100 \
    -file [file join $evidence_root R1F_timing_post_synth.rpt]
  ...
  set drc_report [file join $evidence_root R1F_drc.rpt]
  set bus_skew_report [file join $evidence_root R1F_bus_skew.rpt]
  set cdc_report [file join $evidence_root R1F_cdc.rpt]
  set util_report [file join $evidence_root R1F_utilization.rpt]
  set power_report \
    [file join $evidence_root R1F_power_aggregate_and_hierarchical.rpt]
  ...
  report_timing -delay_type max -max_paths 1000 -nworst 20 \
    -file [file join $evidence_root R1F_timing_complete_setup.rpt]
  report_timing -delay_type min -max_paths 1000 -nworst 20 \
    -file [file join $evidence_root R1F_timing_complete_hold.rpt]
  report_drc -file $drc_report
  check_timing -verbose \
    -file [file join $evidence_root R1F_check_timing.rpt]
  report_exceptions -coverage \
    -file [file join $evidence_root R1F_exception_coverage.rpt]
  report_bus_skew -file $bus_skew_report
  report_clock_interaction \
    -file [file join $evidence_root R1F_clock_interaction.rpt]
  ...
  report_utilization -hierarchical -hierarchical_depth 20 \
    -file [file join $evidence_root R1F_utilization_hierarchical.rpt]
  report_clock_utilization \
    -file [file join $evidence_root R1F_clock_utilization.rpt]
  report_design_analysis -congestion \
    -file [file join $evidence_root R1F_congestion.rpt]
  report_methodology \
    -file [file join $evidence_root R1F_methodology.rpt]
  report_power -hierarchical_depth 20 -file $power_report
  report_io -file [file join $evidence_root R1F_io.rpt]
  report_clocks -file [file join $evidence_root R1F_clocks.rpt]
  report_property -file [file join $evidence_root R1F_design_properties.txt] \
    [current_design]
  ...
  report_property \
    -file [file join $evidence_root R1F_nvp_iobuf_properties.txt] \
    $nvp_iobuf_cells
  ...
  report_timing -delay_type max -to $nvp_t_pins -max_paths 32 -nworst 16 \
    -file [file join $evidence_root R1F_nvp_oen_to_iobuf_paths.rpt]
  ...
  write_lines [file join $evidence_root R1F_nvp_synchronizer_placement.txt] \
  ...
  report_timing -delay_type max -from $nvp_o_pins -to $sync_first_d \
    ...
    -file [file join $evidence_root R1F_nvp_pad_to_synchronizer_paths.rpt]
  write_lines [file join $evidence_root R1F_nvp_output_fanin.txt] \
    [lsort [get_property NAME [all_fanin -flat -to $nvp_t_pins]]]
  write_lines [file join $evidence_root R1F_nvp_input_fanout.txt] \
  ...
  report_timing -delay_type max -from $vdo_ports -max_paths 32 -nworst 4 \
    -file [file join $evidence_root R1F_vdo_setup_paths.rpt]
  report_timing -delay_type min -from $vdo_ports -max_paths 32 -nworst 4 \
    -file [file join $evidence_root R1F_vdo_hold_paths.rpt]
  ...
  report_property -all \
    -file [file join $evidence_root R1F_REQP_1839_CHECK_PROPERTIES.txt] \
    $reqp_check
  set reqp_result_name R1F_REQP_1839_SEMANTIC
  set reqp_semantic_report \
    [file join $evidence_root R1F_REQP_1839_SEMANTIC.rpt]
  ...
  foreach violation_name $reqp_1839_names {
    ...
    report_property -all \
      -file [file join $evidence_root \
        [format "R1F_REQP_1839_VIOLATION_%02d_PROPERTIES.txt" $reqp_index]] \
      $one_violation
    lappend reqp_1839_lines \
      "VIOLATION_INDEX=$reqp_index|OBJECT=$violation_name"
    incr reqp_index
  }
  write_lines [file join $evidence_root R1F_REQP_1839_VIOLATION_OBJECTS.txt] \
    $reqp_1839_lines
  ...
  write_lines [file join $evidence_root R1F_ram_bram_distributed_inventory.txt] \
    $ram_lines
