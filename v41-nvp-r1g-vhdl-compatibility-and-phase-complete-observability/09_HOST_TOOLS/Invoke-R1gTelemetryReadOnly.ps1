...
$readerSha=[string]$binding.r1gReader.sha256
$readerPayload=ConvertTo-GzipBase64([IO.File]::ReadAllBytes($readerPath))
$expect=if($phase.Image-ceq'R1G'){'r1f'}else{'formal'}
$remoteOutput="/home/vcdeagent1/FPGA_AHD_HOST/v41_nvp_r1g/$($PhaseToken.ToLowerInvariant())_telemetry"
...
&$helper -PlinkPath $script:R1gAcceptedTools.Plink084.Path -HostKey 'SHA256:yunI1fwP5I6WfGcSVkyaPxd0siCbdSiOOXVrP0wtEu8' `
    ...
    -EvidenceKind("R1G_FULL_TELEMETRY_{0}_{1}"-f$PhaseToken,$expect.ToUpperInvariant()) -SendPasswordToStdin -SudoPasswordCopies 1 -TimeoutSeconds 180
$rc=$LASTEXITCODE
if($rc-ne0){exit $rc}
...
"R1G_READER_PATH=$readerPath"
"R1G_READER_SHA256=$readerSha"
