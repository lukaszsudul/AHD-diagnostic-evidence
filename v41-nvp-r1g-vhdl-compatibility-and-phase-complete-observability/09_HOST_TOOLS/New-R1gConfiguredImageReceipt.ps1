...
if($ReceiptKind-eq'FormalReady' -and $phase.Kind-eq'ARM_A'){throw 'an R1g Arm-A result cannot issue a formal-ready receipt'}
if($ReceiptKind-ne'FormalReady' -and $phase.Kind-ne'ARM_A'){throw 'only an R1g Arm-A phase can issue an Arm-A receipt'}
...
$output=if($PhaseToken-eq'Bootstrap') {
    Join-Path $script:R1gPrecheckRoot 'FORMAL_START_READY_RECEIPT.txt'
} else {
...
if($ReceiptKind-eq'ArmATerminalSafeDone1') {
    ...
    $lines.Add('ARM_A_TERMINAL_CLASSIFICATION=R1G_INFRASTRUCTURE_INVALID')
} else {
    ...
    if($ReceiptKind-eq'ValidArmA') {
        ...
        $lines.Add("R1G_SOURCE_COMMIT=$([string]$binding.r1gSourceCommit)")
        ...
        $lines.Add("FORMAL_READY_SOURCE=R1G_${PhaseToken}_EXACT_FORMAL_CONTROL")
        $lines.Add('FINAL_PINNED_DRIVER_LOADED=YES')
    }
}
Write-R1gUtf8NoBom -Path $output -Lines $lines.ToArray()
$lines
