...
$image = Get-R1gImageBinding -Document $binding -PhaseSpec $phase
Assert-R1gProgramBudget $phase
...
if ($PhaseToken -eq 'Bootstrap') {
    ...
    $formalStartReceipt = Join-Path $script:R1gPrecheckRoot 'FORMAL_START_READY_RECEIPT.txt'
    ...
    $startGatePath = Join-Path $script:R1gPrecheckRoot 'R1G_FRESH_FORMAL_START_GATE.txt'
    ...
    $receipt = Read-R1gReceipt -Path $ConfiguredImageReceiptPath -ExpectedSha256 $ExpectedConfiguredImageReceiptSha256
    $configuredReceiptResolved = $receipt.Path
    $configuredReceiptSha = $receipt.Sha256
    if ($receipt.Fields.R1G_FULL_JTAG_TARGET_PATH -cne [string]$binding.selectedFullJtagTargetPath) {
        throw 'configured-image receipt selected-target path mismatch'
    }
    $expectedReceipt = Get-R1gExpectedReceiptPath $PhaseToken
...
$programTcl = $script:R1gAcceptedTools.ModeAwareObserverTcl.Path
$observerParser = $script:R1gAcceptedTools.ProgramObserverParser.Path
...
$requiredWait = if ($phase.Image -ceq 'R1G') {
...
$header = @(
    ...
    "PROGRAM_TCL_SHA256=$($script:R1gAcceptedTools.ModeAwareObserverTcl.Sha256)",
    "OBSERVER_PARSER_PATH=$observerParser",
    "OBSERVER_PARSER_SHA256=$($script:R1gAcceptedTools.ProgramObserverParser.Sha256)",
    "TARGET_SELECTOR_SHA256=$($script:R1gAcceptedTools.SelectedTargetSelector.Sha256)",
...
Write-R1gUtf8NoBom -Path $supervisorLog -Lines @($header + $recordLines)
...
Write-R1gUtf8NoBom -Path $timingReceipt -Lines @(
    "PHASE_TOKEN=$PhaseToken",
    "PROGRAM_ROLE=$($phase.ProgramRole)",
    "OBSERVER_MODE=$($phase.ObserverMode)",
    "PROVEN_CONFIGURED_IMAGE_RECEIPT=$configuredReceiptLiteral",
