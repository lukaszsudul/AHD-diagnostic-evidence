#!/usr/bin/env python3
"""R3R4R4 persistent 2500-record capture with parent-only MMIO authority."""
...
P = Path
ROOT = P('/home/vcdeagent1/vcde_artifacts/g2b_hw0_product_r3r4r4/20260907T135724Z')
LOCK = P('/tmp/ahd-g2b-hw0-product-r3r4r4-20260907T135724Z.lock/receipt.json')
...
def _read_exact_nofollow(path: Path, expected_bytes: int) -> bytes:
    ...
    try:
        ...
        while remaining:
            ...
            require(bool(chunk), 'R3R4R4_PERSISTED_FILE_SHORT_REREAD')
            chunks.append(chunk)
            remaining -= len(chunk)
        require(os.read(fd, 1) == b'', 'R3R4R4_PERSISTED_FILE_LONG_REREAD')
...
def _append_jsonl(path: Path, row: Mapping) -> None:
    require(not _contains_bytes(row), 'R3R4R4_JSONL_RAW_BYTES_FORBIDDEN')
...
def combined_session_normalization(read_error: Callable[[], int],
    ...
    if mask:
        ...
        post = read_error()
        require((post & 0x3F) == 0,
                'R3R4R4_SESSION_NORMALIZATION_DID_NOT_CLEAR')
    return {
...
class MMIO:
    ...
    def write(self, offset: int, value: int, purpose: str,
        ...
        if offset == 0x380C and value == 0x00000004:
            ...
            raise GateError('R3R4R4_MMIO_WRITE_ALLOWLIST_VIOLATION')
...
class RecordSink:
    ...
    def __init__(self, private_dir: Path, contract: AbiContract,
        ...
        self.first_thread = threading.Thread(
            target=self._persist_first, name='r3r4r4-first-record-persister',
            daemon=False,
        )
        ...
        self.checkpoint_thread = threading.Thread(
            ...
            name='r3r4r4-primary-durability-worker', daemon=False,
        )
    ...
    def _journal(self, event: str, **fields) -> None:
        ...
        require(not _contains_bytes(row), 'R3R4R4_PRIVATE_JOURNAL_RAW_BYTES_FORBIDDEN')
        ...
        try:
            ...
            with self.first_path.open('xb', buffering=0) as handle:
                ...
                require(written == RECORD_BYTES,
                        'R3R4R4_FIRST_RECORD_SHORT_PERSIST')
                handle.flush()
                os.fsync(handle.fileno())
            del blob
            ...
            require(assessment.structurally_valid and not assessment.session_fatal and
                    not errors, 'R3R4R4_FIRST_RECORD_ABI_VALIDATION_FAILED:' +
                    ';'.join(errors[:8]))
            record = assessment.record
            require(record is not None, 'R3R4R4_FIRST_RECORD_PARSE_MISSING')
            require(record.reset_epoch == self.armed_epoch,
                    'R3R4R4_FIRST_RECORD_STALE_EPOCH')
            ...
            with self.payload_path.open('xb', buffering=0) as handle:
                ...
                require(written == self.contract.payload_bytes,
                        'R3R4R4_FIRST_PAYLOAD_SHORT_PERSIST')
                handle.flush()
                os.fsync(handle.fileno())
            ...
            require(persisted_payload == persisted_payload_slice,
                    'R3R4R4_REREAD_PAYLOAD_RECORD_SLICE_MISMATCH')
            ...
            receipt = {
                'schema': 'R3R4R4_PERSISTED_FIRST_RECORD_RECEIPT_V1',
                'task': TASK,
    ...
    def _request_checkpoint(self, record_count: int) -> None:
        require(record_count in PRIMARY_CHECKPOINT_COUNTS,
                'R3R4R4_PRIMARY_CHECKPOINT_COUNT_NOT_ALLOWED')
        require(record_count not in self.checkpoint_requested,
                'R3R4R4_PRIMARY_CHECKPOINT_DUPLICATE_REQUEST')
        ...
        try:
            while True:
                ...
                require(persisted_size >= required_size,
                        'R3R4R4_PRIMARY_CHECKPOINT_FILE_TOO_SHORT')
                row = {
                    'schema': 'R3R4R4_PRIMARY_DURABILITY_CHECKPOINT_V1',
                    'task': TASK,
            ...
            problem = {
                'blocker': 'R3R4R4_PRIMARY_CHECKPOINT_FAILED:' +
    ...
    def _wait_checkpoint(self, record_count: int, timeout: float = 5.0) -> dict:
        ...
        with self.checkpoint_condition:
            while (record_count not in self.checkpoint_results and
                ...
                require(remaining > 0,
                        f'R3R4R4_PRIMARY_CHECKPOINT_TIMEOUT:{record_count}')
    ...
    def _await_primary_target_prerequisites(self) -> None:
        ...
        require(not self.first_thread.is_alive(),
                'R3R4R4_FIRST_RECORD_PERSISTER_EXIT_TIMEOUT')
        require(self.first_error is None and self.first_durable and self.first_valid,
                (self.first_error or {}).get(
                    'blocker', 'R3R4R4_FIRST_RECORD_DURABLE_BEFORE_PRIMARY_TARGET'))
        self._wait_checkpoint(1024)
        self._wait_checkpoint(2048)

    def accept(self, chunk: bytes) -> None:
        require(not self.closed, 'R3R4R4_RECORD_SINK_ALREADY_CLOSED')
        require(isinstance(chunk, bytes), 'R3R4R4_READER_CHUNK_NOT_BYTES')
        if not chunk:
            return
        ...
        while len(self.pending) >= RECORD_BYTES:
            ...
            if self.primary_count < self.primary_target:
                ...
                require(written == RECORD_BYTES, 'R3R4R4_PRIMARY_SHORT_WRITE')
                ...
                require(self.drain_count < self.drain_limit,
                        'R3R4R4_DRAIN_RECORD_LIMIT_EXCEEDED')
                written = self.drain_file.write(blob)
                require(written == RECORD_BYTES, 'R3R4R4_DRAIN_SHORT_WRITE')
    ...
    def finalize(self, *, result: str, blocker: str,
        ...
        if self.first_thread.is_alive():
            result = 'BLOCKED'
            blocker = 'R3R4R4_FIRST_RECORD_PERSISTER_EXIT_TIMEOUT'
        ...
        if self.checkpoint_thread.is_alive() and result == 'PASS':
            result = 'BLOCKED'
            blocker = 'R3R4R4_PRIMARY_CHECKPOINT_WORKER_EXIT_TIMEOUT'
        ...
        require(bool(blocker), 'R3R4R4_BLANK_BLOCKER_FORBIDDEN')
        try:
            ...
            if result == 'PASS':
                result = 'BLOCKED'
                blocker = 'R3R4R4_CAPTURE_FINAL_SYNC_FAILED:' + _nonempty_problem(exc)
                exception_type = type(exc).__name__
                exception_repr = repr(exc)
                traceback_text = traceback_module.format_exc()
        state = {
...
def reader_worker(c2h_node: str, private_dir: str, contract: AbiContract,
    ...
    sink = None
    result = None
    blocker = 'R3R4R4_READER_UNFINISHED'
    exception_type = None
    exception_repr = None
    trace = None
    quiet_since = None
    try:
        st = os.stat(c2h_node)
        require(stat.S_ISCHR(st.st_mode), 'R3R4R4_C2H_NODE_NOT_CHAR_DEVICE')
    ...
    with path.open('rb', buffering=0) as handle:
        while True:
            ...
            if len(blob) != RECORD_BYTES:
                raise FailureError(
                    f'R3R4R4_CAPTURE_FILE_TRAILING_BYTES:{path.name}:{len(blob)}')
            yield blob
...
def validate_capture(contract: AbiContract, armed_epoch: int,
    ...
    require(primary_path.stat().st_size == PRIMARY_BYTES,
            'R3R4R4_PRIMARY_FILE_SIZE_INVALID')
    require(drain_path.stat().st_size % RECORD_BYTES == 0,
            'R3R4R4_DRAIN_FILE_SIZE_INVALID')
    require(trailing_path.stat().st_size == 0,
            'R3R4R4_INCOMPLETE_TRAILING_BYTES_NONZERO')
    ...
    require(total == PRIMARY_TARGET + drain_records,
            'R3R4R4_HOST_RECORD_COUNT_FILE_MISMATCH')
    require(first_primary_record is not None, 'R3R4R4_FIRST_PRIMARY_RECORD_MISSING')
...
def reconcile_counters(before: Mapping, after: Mapping, total: int,
    ...
    if not all(checks):
        raise FailureError('R3R4R4_COUNTER_RECONCILIATION_FAILED:' +
...
def _drain_messages(messages, state: dict, *, wait: float = 0.0) -> None:
    ...
    while True:
        ...
        require(not _contains_bytes(message),
                'R3R4R4_RAW_RECORD_CONTROL_IPC_FORBIDDEN')
...
def run() -> dict:
    ...
    result = {
        ...
        'blocker': 'R3R4R4_CAPTURE_UNFINISHED', 'primary_target': PRIMARY_TARGET,
    ...
    try:
        require(not _open_holders(proof['c2h']),
                'R3R4R4_C2H_READER_ALREADY_ATTACHED')
        mmio = MMIO('T3T4_PARENT')
        control = mmio.read(0x380C)
        status = mmio.read(0x3810)
        require(control == 0 and not (status & 0x100),
                'R3R4R4_DISABLED_BASELINE_NOT_CLEAN')
        ...
        require((normalization['post_normalization_error_status'] & 0x3F) == 0,
                'R3R4R4_POST_NORMALIZATION_ERROR_STATUS_NONZERO')
        result.update(
        ...
        require(quiescent(mmio.read(0x380C), status) and
                ...
                'R3R4R4_FINAL_PRE_ENABLE_BASELINE_FAILED')
        require(mmio.read(0x3838) == epoch, 'R3R4R4_UNEXPECTED_EPOCH_CHANGE')
        ...
        while 'READER_READY' not in result and time.monotonic() < ready_deadline:
            ...
            require(child.is_alive() or 'READER_READY' in result,
                    'R3R4R4_READER_EXITED_BEFORE_READY')
        require('READER_READY' in result, 'R3R4R4_READER_READY_TIMEOUT')
            ...
            if 'FIRST_RECORD_VALID' in result and not \
                ...
                raise FailureError(
                    result['FIRST_RECORD_VALID'].get(
                        'blocker', 'R3R4R4_FIRST_RECORD_ABI_VALIDATION_FAILED'))
            if 'ERROR' in result:
                raise GateError(result['ERROR'].get('blocker',
                                                    'R3R4R4_READER_ERROR'))
            require(child.is_alive(), 'R3R4R4_READER_EXITED_BEFORE_PRIMARY_TARGET')
            require(time.monotonic() < primary_deadline,
                    'R3R4R4_PRIMARY_2500_TIMEOUT')
        require(result['PRIMARY_TARGET_REACHED'].get('first_record_durable') and
                ...
                'R3R4R4_PRIMARY_TARGET_ORDERING_PROOF_MISSING')
        ...
        require(quiescence_pass,
                'R3R4R4_POST_DISABLE_HARDWARE_NOT_QUIESCENT')
        ...
        require(not child.is_alive(), 'R3R4R4_READER_COOPERATIVE_EXIT_TIMEOUT')
        _drain_messages(messages, result, wait=0.2)
        require('READER_QUIET' in result, 'R3R4R4_READER_QUIET_WINDOW_MISSING')
        require('READER_EXITED' in result, 'R3R4R4_READER_RESULT_MISSING')
        reader_result = result['READER_EXITED']['reader_result']
        result['reader'] = reader_result
        require(reader_result.get('result') == 'PASS',
                reader_result.get('blocker') or 'R3R4R4_READER_FAILED')
        require(reader_result.get('primary_records') == PRIMARY_TARGET,
                'R3R4R4_PRIMARY_RECORD_COUNT_INVALID')
        ...
        require('FIRST_RECORD_DURABLE' in result and
                ...
                result['FIRST_RECORD_VALID'].get('valid'),
                'R3R4R4_FIRST_RECORD_PERSISTENCE_OR_VALIDATION_MISSING')
        ...
        require(quiescent(final_control, final_status),
                'R3R4R4_FINAL_STREAM_OR_DMA_NOT_QUIESCENT')
        if final_error != 0:
            raise FailureError('R3R4R4_NEW_ERROR_STATUS_DURING_FINITE_CAPTURE')
        require(final_snapshot['epoch'] == epoch,
                'R3R4R4_UNEXPECTED_EPOCH_CHANGE')
        result.update(
        ...
        if child is not None and child.is_alive():
            result['rollback'] = 'R3R4R4_ROLLBACK_UNSAFE_ACTIVE_DMA'
        ...
        require(bool(str(result.get('blocker', '')).strip()),
                'R3R4R4_BLANK_BLOCKER_FORBIDDEN')
