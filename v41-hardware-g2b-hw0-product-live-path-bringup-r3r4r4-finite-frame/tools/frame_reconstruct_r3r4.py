            ...
            while True:
                ...
                if len(blob) != RECORD_BYTES:
                    raise ReconstructionError(
                        f"R3R4R4_FRAME_INPUT_TRAILING_BYTES:{path}:{len(blob)}"
                    )
                yield blob
...
def uyvy_to_rgb_scanlines(raw: bytes, width: int, height: int) -> Iterator[bytes]:
    ...
    if len(raw) != expected:
        raise ReconstructionError(
            f"R3R4R4_UYVY_SIZE_INVALID:{len(raw)}:{expected}"
        )
    stride = width * 2
...
def reconstruct_first_frame(
    ...
    *,
    ...
    if completed is None:
        raise ReconstructionError("R3R4R4_COMPLETE_FRAME_NOT_FOUND")

    expected_size = contract.pixels_per_line * contract.lines_per_frame * 2
    if len(completed.raw_uyvy) != expected_size:
        raise ReconstructionError(
            f"R3R4R4_FRAME_SIZE_INVALID:{len(completed.raw_uyvy)}:{expected_size}"
        )
    if completed.line_sequences != tuple(range(contract.lines_per_frame)):
        raise ReconstructionError("R3R4R4_FRAME_LINE_SET_INVALID")
