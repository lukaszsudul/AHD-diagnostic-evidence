#!/usr/bin/env python3
"""Build the sanitized, fail-closed R3R4R4 blocked evidence package."""
...
RUN = Path(r"C:\FPGA\G2B_HW0_PRODUCT_R3R4R4_20260907T135724Z")
...
EVIDENCE_NAME = (
    "v41-hardware-g2b-hw0-product-live-path-bringup-r3r4r4-finite-frame"
)
OUT = RUN / "evidence-staging" / EVIDENCE_NAME
TASK = "G2B-HW0-PRODUCT-R3R4R4"
BLOCKER = "R3R4R4_CAPTURE_TOOL_SELFTEST_FAILED:PARENT_MULTI_SAMPLE_QUIESCENCE_PASS"
...
def main() -> int:
    ...
    authority = json.loads((
        ...
        "G2B_HW0_PRODUCT_R3R4R4_AUTHORITY.json"
    ).read_text(encoding="utf-8"))
    if authority.get("result") != "PASS" or not all(
            authority.get("checks", {}).values()):
        raise RuntimeError("R3R4R4_PREDECESSOR_AUTHORITY_FAILURE")

    boundary = json.loads((
        RUN / "artifacts" / "protected-after.comparison.json"
    ).read_text(encoding="utf-8"))
    if boundary.get("result") != "PASS":
        raise RuntimeError("R3R4R4_IMMUTABLE_BOUNDARY_FAILED")
    failure = json.loads((RUN / "artifacts" /
                          "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_FAILURE.json").read_text(
                              encoding="utf-8"))
    analysis = json.loads((RUN / "artifacts" /
                           "G2B_HW0_PRODUCT_R3R4R4_SELFTEST_FAILURE_ANALYSIS.json").read_text(
                               encoding="utf-8"))
    ordering_proof = json.loads((RUN / "artifacts" /
                                 "G2B_HW0_PRODUCT_R3R4R4_EVENT_ORDERING_PROOF.json").read_text(
                                     encoding="utf-8"))
    delta = json.loads((RUN / "artifacts" /
                        "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_DIFF.json").read_text(
                            encoding="utf-8"))
    ...
    helper = RUN / "scripts" / "Invoke-R3R4R4DutConnection.ps1"
    ...
    selftest_public = {
        "schema": "R3R4R4_CAPTURE_TOOL_SELFTEST_RESULT_V1",
    ...
    state = {
        ...
        "all_connections_used_r3r4r4_helper": "NOT_REACHED",
        ...
        "final_execution_point":
            "HARD STOP AFTER G2B-HW0-PRODUCT-R3R4R4 FINITE-FRAME QUALIFICATION",
    }

    write_json(OUT / "raw" / "G2B_HW0_PRODUCT_R3R4R4_AUTHORITY.json", authority)
    write_json(OUT / "raw" / "G2B_HW0_PRODUCT_R3R4R4_BOUNDARY.json", boundary)
    write_json(OUT / "raw" /
               "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_SELFTEST_FAILURE.json", failure)
    write_json(OUT / "raw" /
               "G2B_HW0_PRODUCT_R3R4R4_SELFTEST_FAILURE_ANALYSIS.json", analysis)
    write_json(OUT / "raw" /
               "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_DIFF.json", delta)

    shutil.copy2(RUN / "artifacts" /
                 "G2B_HW0_PRODUCT_R3R4R4_TOOL_BASELINE_RECEIPT.md",
                 OUT / "G2B_HW0_PRODUCT_R3R4R4_TOOL_BASELINE_RECEIPT.md")
    shutil.copy2(RUN / "artifacts" /
                 "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_DIFF.md",
                 OUT / "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_DIFF.md")
    shutil.copy2(RUN / "artifacts" /
                 "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_DIFF.patch",
                 OUT / "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_DIFF.patch")
    write_json(OUT / "G2B_HW0_PRODUCT_R3R4R4_EVENT_ORDERING_PROOF.json",
               ordering_proof)

    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_AUTHORIZATION_RECEIPT.md", report(
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_AUTHORITY_VERIFICATION.md", report(
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_BOUNDARY_RECEIPT.md", report(
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_PERSISTED_BYTES_PROOF.md", report(
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_PRIMARY_DURABILITY_REPORT.md", report(
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_EVENT_ORDERING_PROOF.md", report(
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_QUIESCENCE_PROOF.md", report(
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_W1C_PROOF.md", report(
        f"{TASK} W1C Proof", [
            ("Source-delta presence", "PASS"),
    ...
    write_json(OUT / "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_SELFTEST.json",
               selftest_public)
    selftest_rows = [(name, selftest_cases[name]) for name in
                     EXISTING_CASES + NEW_CASES]
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_SELFTEST.md", report(
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_CAPTURE_TOOL_AUDIT.md", report(
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_CREDENTIAL_HELPER_AUDIT.md", report(
    ...
    placeholders = {
        "G2B_HW0_PRODUCT_R3R4R4_DUT_LOCK_RECEIPT.md": "DUT Lock Receipt",
        "G2B_HW0_PRODUCT_R3R4R4_RECOVERY_DECISION.md": "Recovery Decision",
        "G2B_HW0_PRODUCT_R3R4R4_PRELOAD_INVENTORY.md": "Preload Inventory",
        "G2B_HW0_PRODUCT_R3R4R4_DRIVER_VERIFICATION.md": "Driver Verification",
        "G2B_HW0_PRODUCT_R3R4R4_DRIVER_LOAD_PROBE.md": "Driver Load Probe",
        "G2B_HW0_PRODUCT_R3R4R4_NODE_TO_BDF_PROOF.md": "Node-to-BDF Proof",
        "G2B_HW0_PRODUCT_R3R4R4_MMIO_DECODED.md": "MMIO Decoded",
        "G2B_HW0_PRODUCT_R3R4R4_SESSION_START_RECEIPT.md": "Session Start Receipt",
        "G2B_HW0_PRODUCT_R3R4R4_FIRST_RECORD_REPORT.md": "First-Record Report",
        "G2B_HW0_PRODUCT_R3R4R4_FINITE_CAPTURE_REPORT.md": "Finite Capture Report",
        "G2B_HW0_PRODUCT_R3R4R4_COUNTER_RECONCILIATION.md": "Counter Reconciliation",
        "G2B_HW0_PRODUCT_R3R4R4_FRAME_RECONSTRUCTION_REPORT.md": "Frame Reconstruction",
        "G2B_HW0_PRODUCT_R3R4R4_PCIE_AER_KERNEL_REVIEW.md": "PCIe, AER and Kernel Review",
    }
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_CLEANUP_RECEIPT.md", report(
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_FINAL_HARDWARE_STATE.md", report(
    ...
    write_csv(OUT / "G2B_HW0_PRODUCT_R3R4R4_NODE_MAP.csv",
              ["DynamicIndex", "Node", "BDF", "Result"], [])
    write_csv(OUT / "G2B_HW0_PRODUCT_R3R4R4_MMIO_RAW.csv",
              ["Timestamp", "Session", "UserNode", "BDF", "Offset",
               "Operation", "Value", "Result"], [])
    write_csv(OUT / "G2B_HW0_PRODUCT_R3R4R4_MMIO_WRITE_LEDGER.csv",
              ["Timestamp", "Session", "UserNode", "BDF", "Offset",
               "Operation", "Value", "Purpose", "Authorized", "Precondition",
               "Result"], [])
    write_csv(OUT / "G2B_HW0_PRODUCT_R3R4R4_FIRST_RECORD_HEADER.csv",
              ["Field", "Value", "Result"], [])
    write_csv(OUT / "G2B_HW0_PRODUCT_R3R4R4_FINITE_CAPTURE_METRICS.csv",
              ["Metric", "Value", "Result"], [
    ...
    write_csv(OUT / "G2B_HW0_PRODUCT_R3R4R4_QUIESCENCE_SAMPLES.csv",
              ["Timestamp", "Sample", "Consecutive", "SpanMs", "Result"], [])
    with (OUT / "G2B_HW0_PRODUCT_R3R4R4_PRIMARY_DURABILITY_CHECKPOINTS.jsonl").open(
    ...
    write_csv(OUT / "G2B_HW0_PRODUCT_R3R4R4_GATE_MATRIX.csv",
              ["Gate", "Result"], [{"Gate": gate, "Result": value}
                                    for gate, value in gates])
    write_json(OUT / "G2B_HW0_PRODUCT_R3R4R4_STATE.json", state)

    tool_names = (
        "capture_r3r4.py", "capture_r3r4_selftest.py",
        "frame_reconstruct_r3r4.py", "abi_v1.py",
        "V41_C2H_TRANSPORT_ABI_V1.json", "Invoke-R3R4R4DutConnection.ps1",
        "Collect-ImmutableBoundary.ps1", "audit_tool_delta_r3r4r4.py",
        "audit_capture_architecture_r3r4r4.py",
        "verify_authority_r3r4r4.py", "build_evidence_r3r4r4.py",
    )
    for name in tool_names:
        shutil.copy2(RUN / "scripts" / name, OUT / "tools" / name)
    write_csv(OUT / "G2B_HW0_PRODUCT_R3R4R4_TOOL_INVENTORY.csv",
              ["Tool", "SHA256", "Executed"], [
                  {"Tool": name, "SHA256": sha(OUT / "tools" / name),
                   "Executed": "YES" if name in {
                       ...
                       "audit_tool_delta_r3r4r4.py", "Collect-ImmutableBoundary.ps1",
                       "verify_authority_r3r4r4.py",
                       "build_evidence_r3r4r4.py"} else "NO"}
                  for name in tool_names
              ])

    write(OUT / "V41_G2B_HW0_PRODUCT_R3R4R4_MAIN_REPORT.md",
          "\n".join([
              ...
              "## Nonclaims", "",
              "R3R4R4 does not prove persistent first-record hardware capture, a finite "
    ...
    if missing:
        raise RuntimeError("R3R4R4_REQUIRED_PUBLIC_FILE_MISSING:" + ",".join(missing))
    forbidden_suffixes = {".bin", ".bit", ".dcp", ".ko", ".uyvy", ".png"}
    forbidden = [str(path.relative_to(OUT)) for path in OUT.rglob("*")
                 if path.is_file() and path.suffix.lower() in forbidden_suffixes]
    if forbidden:
        raise RuntimeError("R3R4R4_PROHIBITED_PUBLIC_BINARY:" + ",".join(forbidden))

    index_lines = ["# R3R4R4 Evidence Index", ""]
    ...
    write(OUT / "G2B_HW0_PRODUCT_R3R4R4_EVIDENCE_INDEX.md",
          "\n".join(index_lines))
    manifest_path = OUT / "G2B_HW0_PRODUCT_R3R4R4_SHA256_MANIFEST.txt"
    manifest_lines = []
