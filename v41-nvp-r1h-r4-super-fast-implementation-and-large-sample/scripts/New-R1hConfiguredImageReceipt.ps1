...
function Get-ExactValue([string]$Path,[string]$Key) {
    if(-not(Test-Path -LiteralPath $Path -PathType Leaf)){throw "required receipt input is absent: $Path"}
    $text=[IO.File]::ReadAllText($Path)
    $matches=[regex]::Matches($text,'(?m)^'+[regex]::Escape($Key)+'=([^\r\n]*)\r?$')
    if($matches.Count-ne1){throw "$Key exact-line count is $($matches.Count), expected 1: $Path"}
...
$programRetries=Get-ExactValue $timing PROGRAM_RETRIES
if($programRetries-cnotin@('0','1')){throw 'R1h-R4 PROGRAM_RETRIES must be 0 or 1'}
if($programRetries-ceq'1'){
    $globalRetry=Join-Path $script:R1hPrecheckRoot 'GLOBAL_PROGRAM_RETRY_RESERVATION.txt'
    Require-ExactLine $globalRetry GLOBAL_PROGRAM_RETRY_RESERVED YES
    Require-ExactLine $globalRetry RETRY_PHASE_TOKEN $PhaseToken
    Require-ExactLine $globalRetry RETRY_ELIGIBILITY_GATE PASS_INFRASTRUCTURE_ONLY
}
...
foreach($line in @(
    ...
    'FPGA_DONE=1',"PROGRAM_RETRIES=$programRetries",'RECEIPT_CREATION_LIVE_ACTIONS=0',
...
if($ReceiptKind-eq'ArmATerminalSafeDone1') {
    ...
    $hashInputs=[ordered]@{WAIT=$wait;WARM_REBOOT=$reboot;HOST_CYCLE=$cycle;PRELOADER_ADAPTER=$preloaderAdapter;PRELOADER=$preloader;LOADER=$loader;RUNTIME_PROVENANCE=$runtimeProvenance;FINAL_DONE=$finalDone}
    if($PhaseToken-ne'Bootstrap'){$hashInputs['TELEMETRY']=$telemetry}
