...
$required = if($phase.Image -ceq 'R1H') {[Math]::Max(10.0,[double]$binding.r1hBit.requiredWaitSeconds)} else {5.0}
...
Require $timing R1H_FULL_JTAG_TARGET_PATH ([string]$binding.selectedFullJtagTargetPath)
Require $timing PROGRAM_RESULT PASS_STARTUP_HIGH_DONE_1
$retryValue = [string]$timing.PROGRAM_RETRIES
if ($retryValue -cnotin @('0','1')) { throw 'PROGRAM_RETRIES must be 0 or 1 in R1h-R4' }
if ($retryValue -ceq '1') {
    $globalRetry = Join-Path $script:R1hPrecheckRoot 'GLOBAL_PROGRAM_RETRY_RESERVATION.txt'
    if (-not (Test-Path -LiteralPath $globalRetry -PathType Leaf)) { throw 'retry timing receipt lacks global retry reservation' }
    $globalText = [IO.File]::ReadAllText($globalRetry)
    foreach ($required in @('GLOBAL_PROGRAM_RETRY_RESERVED=YES',"RETRY_PHASE_TOKEN=$PhaseToken",'RETRY_ELIGIBILITY_GATE=PASS_INFRASTRUCTURE_ONLY')) {
        if ([regex]::Matches($globalText,'(?m)^'+[regex]::Escape($required)+'\r?$').Count -ne 1) { throw "global retry gate lacks $required" }
    }
}
...
Write-R1hUtf8NoBom -Path $outputPath -Lines @(
