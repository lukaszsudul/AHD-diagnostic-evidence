...
$script:R1hTaskRoot = 'C:\FPGA\V41_NVP_R1H_R4_SUPER_FAST'
$script:R1hToolRoot = Join-Path $script:R1hTaskRoot 'scripts'
$script:R1hPrecheckRoot = Join-Path $script:R1hTaskRoot 'hardware'
...
$script:R1hAcceptedTools = [ordered]@{
    ...
    RuntimeProvenancePayload = [pscustomobject]@{
        ...
        Bytes = 3666L
        Sha256 = 'D7ABAD02148FED6845BF228194863B8E0DC193A9664780B6F5DBE80AFFE94691'
    }
    FormalTelemetryReader = [pscustomobject]@{
        Path = Join-Path $script:R7TaskRoot 'scripts\read_nvp_r1e.py'
        Bytes = 8385L
        Sha256 = '0BE8AD0ECEF0FC333FEDFFAC9C7D94D2851E7FC319EEB88579D7EA3B2AEA7037'
    }
    FrozenR1hStatistics = [pscustomobject]@{
        Path = 'C:\FPGA\WORKTREES\V41_NVP_R1H_BRAM_BACKED_LARGE_SAMPLE\scripts\v41\r1f_statistics.py'
        Bytes = 38404L
        Sha256 = 'C0188FF2AB7AC03034DAA7F412F447E3DBC21C15FB5458B126C0A96FEB771CCD'
    }
...
$script:R1hPhaseTable = [ordered]@{
    Bootstrap = [pscustomobject]@{
        Sequence = 0; Directory = 'hardware\01_BOOTSTRAP'; Image = 'FORMAL'
    ...
    A1 = [pscustomobject]@{
        Sequence = 1; Directory = 'hardware\02_A1'; Image = 'R1H'
    ...
    B1 = [pscustomobject]@{
        Sequence = 2; Directory = 'hardware\03_B1'; Image = 'FORMAL'
    ...
    A2 = [pscustomobject]@{
        Sequence = 3; Directory = 'hardware\04_A2'; Image = 'R1H'
        ProgramRole = 'ARM_A_R1E'; ObserverMode = 'TRANSITION_FROM_PROVEN_CONFIGURED_IMAGE'
        ReceiptType = 'FORMAL_READY_RECEIPT'; RequiredWaitFloorSeconds = 33.536673744
        RemoteDriverDirectory = 'a2_driver'; Kind = 'ARM_A'
    }
    B2 = [pscustomobject]@{
        Sequence = 4; Directory = 'hardware\05_B2'; Image = 'FORMAL'
        ProgramRole = 'ARM_B_FORMAL'; ObserverMode = 'TRANSITION_FROM_PROVEN_CONFIGURED_IMAGE'
        ReceiptType = 'VALID_ARM_A_RECEIPT'; RequiredWaitFloorSeconds = 5.0
        RemoteDriverDirectory = 'b2_driver'; Kind = 'ARM_B'
    }
    A3 = [pscustomobject]@{
        Sequence = 5; Directory = 'hardware\06_A3'; Image = 'R1H'
        ProgramRole = 'ARM_A_R1E'; ObserverMode = 'TRANSITION_FROM_PROVEN_CONFIGURED_IMAGE'
        ReceiptType = 'FORMAL_READY_RECEIPT'; RequiredWaitFloorSeconds = 33.536673744
        RemoteDriverDirectory = 'a3_driver'; Kind = 'ARM_A'
    }
    B3 = [pscustomobject]@{
        Sequence = 6; Directory = 'hardware\07_B3'; Image = 'FORMAL'
        ProgramRole = 'ARM_B_FORMAL'; ObserverMode = 'TRANSITION_FROM_PROVEN_CONFIGURED_IMAGE'
        ReceiptType = 'VALID_ARM_A_RECEIPT'; RequiredWaitFloorSeconds = 5.0
...
function Get-R1hPhaseSpec {
    ...
    [pscustomobject]@{
        ...
        RemoteDriverAbsolutePath = "/home/vcdeagent1/FPGA_AHD_HOST/v41_nvp_r1h_r4_61ec5f55/$($base.RemoteDriverDirectory)"
        Kind = $base.Kind
    }
}
...
function Get-R1hBindingDocument {
    ...
    if ([string]$document.r1hSourceCommit -cne 'c4f4bfcf577c92c3021d1fe83c05878dd12e001c' -or
        [string]$document.r1hSourceTree -cne '161e561f007912d73dba93c5ecd78e3cc3a6955b') {
        throw 'R1h-R4 exact scientific source commit/tree binding mismatch'
    }
    ...
    $releasePrefix = (Join-Path $script:R1hTaskRoot 'implementation') + [IO.Path]::DirectorySeparatorChar
    if (-not $releasePath.StartsWith($releasePrefix,[StringComparison]::OrdinalIgnoreCase)) {
        throw 'R1h-R4 hardware-release receipt escaped the implementation evidence root'
    }
    ...
    foreach ($required in @(
        'R1H_R4_IMPLEMENTATION_RESULT=PASS','PLACE=PASS','ROUTE=PASS',
    ...
    if ([regex]::Matches($releaseText,'(?m)^R1H_SYNTH_DCP_SHA256=807D292909804FDE573867A681A3407366BF9AF0796E290E609951B7DD68E46E\r?$').Count -ne 1 -or
        [regex]::Matches($releaseText,'(?m)^R1H_SOURCE_COMMIT=c4f4bfcf577c92c3021d1fe83c05878dd12e001c\r?$').Count -ne 1 -or
        [regex]::Matches($releaseText,'(?m)^R1H_BIT_SHA256='+[regex]::Escape([string]$document.r1hBit.sha256)+'\r?$').Count -ne 1) {
        throw 'R1h-R4 implementation receipt identity binding failed'
    }
