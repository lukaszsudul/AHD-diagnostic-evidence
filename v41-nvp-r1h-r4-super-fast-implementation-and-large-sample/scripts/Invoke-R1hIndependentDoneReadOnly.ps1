...
$scriptPath = $script:R1hAcceptedTools.IndependentDoneTcl.Path
...
Write-R1hUtf8NoBom -Path $rawPath -Lines @(
    ...
    "READ_ONLY_JTAG_TCL_SHA256=$($script:R1hAcceptedTools.IndependentDoneTcl.Sha256)",
...
if ($canonical -cne $script:R1hCanonicalTarget -or $path -cne [string]$binding.selectedFullJtagTargetPath) {
    $failures.Add('SELECTED_TARGET_MISMATCH')
}
if ($deviceCount -cne '1' -or $part -cne $script:R1hExpectedPart -or $idcode -cne $script:R1hExpectedIdcode) {
...
foreach ($line in @(
    ...
    "READ_ONLY_JTAG_TCL_SHA256=$($script:R1hAcceptedTools.IndependentDoneTcl.Sha256)",
    "R1H_SELECTED_JTAG_CANONICAL_ID=$canonical",
    "R1H_FULL_JTAG_TARGET_PATH=$path",
...
Write-R1hUtf8NoBom -Path $receiptPath -Lines $lines.ToArray()
$lines
