...
function Append-R1hUtf8NoBom {
    param(
        [Parameter(Mandatory)][string]$Path,
        [Parameter(Mandatory)][AllowEmptyCollection()][object[]]$Lines
    )
    if ($Lines.Count -eq 0) { return }
    $text = (($Lines | ForEach-Object { [string]$_ }) -join [Environment]::NewLine) + [Environment]::NewLine
...
Assert-R1hProgramBudget $phase
$retryCount = if ($GlobalRetryAttempt) { 1 } else { 0 }
$globalRetryReceipt = Join-Path $script:R1hPrecheckRoot 'GLOBAL_PROGRAM_RETRY_RESERVATION.txt'
if ($GlobalRetryAttempt) {
    if (-not (Test-Path -LiteralPath $globalRetryReceipt -PathType Leaf)) {
        throw 'global retry attempt lacks its immutable eligibility/reservation receipt'
    }
    $retryText = [IO.File]::ReadAllText($globalRetryReceipt)
    foreach ($required in @('GLOBAL_PROGRAM_RETRY_RESERVED=YES',"RETRY_PHASE_TOKEN=$PhaseToken",'RETRY_ELIGIBILITY_GATE=PASS_INFRASTRUCTURE_ONLY')) {
        if ([regex]::Matches($retryText,'(?m)^'+[regex]::Escape($required)+'\r?$').Count -ne 1) {
            throw "global retry receipt lacks exact gate: $required"
        }
    }
}
...
if ($PhaseToken -eq 'Bootstrap') {
    ...
    $startGatePath = Join-Path $script:R1hPrecheckRoot 'R1H_R4_MINIMAL_HARDWARE_SAFETY_GATE.txt'
    if (-not (Test-Path -LiteralPath $startGatePath -PathType Leaf)) {
        throw 'mandatory R1h-R4 bootstrap requires the minimal read-only safety gate'
    }
    $startGateText = [IO.File]::ReadAllText($startGatePath)
    if ([regex]::Matches($startGateText,'(?m)^R1H_R4_MINIMAL_HARDWARE_SAFETY_GATE=PASS\r?$').Count -ne 1 -or
        [regex]::Matches($startGateText,'(?m)^READ_ONLY_GATE=YES\r?$').Count -ne 1) {
        throw 'minimal safety gate does not authorize the mandatory formal bootstrap'
    }
} else {
    ...
    $receipt = Read-R1hReceipt -Path $ConfiguredImageReceiptPath -ExpectedSha256 $ExpectedConfiguredImageReceiptSha256
    $configuredReceiptResolved = $receipt.Path
    $configuredReceiptSha = $receipt.Sha256
    if ($receipt.Fields.R1H_FULL_JTAG_TARGET_PATH -cne [string]$binding.selectedFullJtagTargetPath) {
        throw 'configured-image receipt selected-target path mismatch'
    }
    $expectedReceipt = Get-R1hExpectedReceiptPath $PhaseToken
...
$programTcl = $script:R1hAcceptedTools.ModeAwareObserverTcl.Path
$observerParser = $script:R1hAcceptedTools.ProgramObserverParser.Path
$settingsPath = $script:R1hAcceptedTools.VivadoSettings.Path
$vivadoPath = $script:R1hAcceptedTools.VivadoLauncher.Path
$bitPath = (Resolve-Path -LiteralPath ([string]$image.path) -ErrorAction Stop).Path
$requiredWait = if ($phase.Image -ceq 'R1H') {
...
Write-R1hUtf8NoBom -Path $reservationPath -Lines @(
    "PHASE_TOKEN=$PhaseToken",
    "PROGRAM_ROLE=$($phase.ProgramRole)",
    "OBSERVER_MODE=$($phase.ObserverMode)",
    "PROGRAM_RETRY_AUTHORIZED=GLOBAL_BUDGET_ONE_SEPARATE_ELIGIBILITY_GATE",
    "GLOBAL_RETRY_ATTEMPT=$(if($GlobalRetryAttempt){'YES'}else{'NO'})",
    "PROGRAM_INVOCATION_GLOBAL_MAX=$script:R1hMaximumPrograms",
    "BIT_PATH=$bitPath",
...
Write-R1hUtf8NoBom -Path $supervisorLog -Lines @($header + $recordLines)
...
Append-R1hUtf8NoBom -Path $supervisorLog -Lines $resultLines
...
try {
    ...
    Append-R1hUtf8NoBom -Path $supervisorLog -Lines @('MODE_AWARE_PREPROGRAM_GATE=FAIL',"MODE_AWARE_PREPROGRAM_ERROR=$($_.Exception.Message)")
    throw
}

if ($result.CLASSIFICATION -cne 'PASS_STARTUP_HIGH_DONE_1') {
    Append-R1hUtf8NoBom -Path $supervisorLog -Lines @('PROGRAM_SUPERVISOR_GATE=FAIL_PRIMARY_ATTEMPT','RETRY_ELIGIBILITY=REQUIRES_SEPARATE_R4_INFRASTRUCTURE_GATE')
    $resultLines
    'PROGRAM_SUPERVISOR_GATE=FAIL_PRIMARY_ATTEMPT'
    exit 1
}
...
Write-R1hUtf8NoBom -Path $timingReceipt -Lines @(
    "PHASE_TOKEN=$PhaseToken",
    "PROGRAM_ROLE=$($phase.ProgramRole)",
    "OBSERVER_MODE=$($phase.ObserverMode)",
    "PROVEN_CONFIGURED_IMAGE_RECEIPT=$configuredReceiptLiteral",
