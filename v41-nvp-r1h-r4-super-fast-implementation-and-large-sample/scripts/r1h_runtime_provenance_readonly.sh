#!/usr/bin/env bash
# R1h runtime identity/provenance leaf. This leaf opens only the already-proven
# XDMA user BAR node with O_RDONLY and uses pread. It has no MMIO-write, PCIe-state-change, driver,
# I2C, reboot, programming, DMA, or filesystem mutation path.

set -euo pipefail

role=${1:?role required}
expected_r1h_commit=${2:?expected R1h commit required}
node=${3:-/dev/xdma0_user}

case "$role" in
  r1h)
    [[ $expected_r1h_commit =~ ^[0-9a-f]{40}$ ]] || {
      printf 'RUNTIME_PROVENANCE_GATE=FAIL_EXPECTED_COMMIT_FORMAT\n'
      exit 64
    }
    ;;
  formal)
    [[ $expected_r1h_commit == NOT_APPLICABLE ]] || {
...
python3 - "$role" "$expected_r1h_commit" "$node" <<'PY'
import os
import struct
import sys
...
try:
    ...
    if role == "r1h":
        if git_sha != expected_commit:
            raise RuntimeError("R1h source-commit provenance mismatch")
        if build_flags != 0x00000002:
            raise RuntimeError("R1h BUILD_FLAGS mismatch")
        if diagnostic_word != 0x314B4C43:
            raise RuntimeError("R1h inherited lifecycle magic mismatch")
        print("RUNTIME_IMAGE=R1H_EXACT_SOURCE_AND_BUILD_FLAGS")
    else:
            ...
            for offset in range(0x20A0, 0x3600, 4):
                ...
                if value:
                    ...
                    if len(nonzero) == 8:
                        break
            if nonzero:
                print(f"FORMAL_COMPLETE_R1H_RANGE_ZERO=NO_OPTIONAL_NONBLOCKING")
                print(f"FORMAL_RANGE_NONZERO_SAMPLE={nonzero!r}")
            else:
                print("FORMAL_COMPLETE_R1H_RANGE_ZERO=YES")
        except Exception as optional_error:
            print("FORMAL_COMPLETE_R1H_RANGE_ZERO=NOT_AVAILABLE_OPTIONAL_NONBLOCKING")
            print(f"FORMAL_RANGE_OPTIONAL_READ_ERROR={optional_error!r}")
        print("RUNTIME_IMAGE=FORMAL_PHASE2_EXACT_IDENTITY_DIAGNOSTIC_MAGIC_ZERO")
finally:
    os.close(fd)
