#!/usr/bin/env python3
"""Independent structural hard gate for the R3R4R3 capture contract."""
...
def main() -> int:
    ...
    selftest = json.loads((
        ARTIFACTS / "G2B_HW0_PRODUCT_R3R4R3_CAPTURE_TOOL_SELFTEST.json"
    ...
    checks = {
        ...
        "PRIMARY_CHECKPOINT_WORKER_ASYNCHRONOUS":
            ...
            "r3r4r3-primary-durability-worker" in
    ...
    blocker = ("NONE" if not failed else
               "R3R4R3_CAPTURE_TOOL_ARCHITECTURE_HARD_GATE_FAILED:" + reason)
    result = {
        "schema": "R3R4R3_CAPTURE_TOOL_ARCHITECTURE_HARD_GATE_V1",
    ...
    write_exclusive(
        ARTIFACTS /
        "G2B_HW0_PRODUCT_R3R4R3_CAPTURE_TOOL_ARCHITECTURE_HARD_GATE.json",
        json.dumps(result, indent=2) + "\n",
    )
    lines = [
        "# G2B-HW0-PRODUCT-R3R4R3 Capture-Tool Architecture Hard Gate", "",
    ...
    text = "\n".join(lines) + "\n"
    write_exclusive(
        ARTIFACTS / "G2B_HW0_PRODUCT_R3R4R3_CAPTURE_TOOL_AUDIT.md", text)
    write_exclusive(
        ARTIFACTS /
        "G2B_HW0_PRODUCT_R3R4R3_CAPTURE_TOOL_ARCHITECTURE_HARD_GATE.md", text)
