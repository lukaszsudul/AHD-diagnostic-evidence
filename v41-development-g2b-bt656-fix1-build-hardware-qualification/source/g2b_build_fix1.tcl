...
if {$argc == 8} {
  ...
  if {$execution_mode ni {PROVENANCE_ONLY POSTCOMMIT_FULL_BUILD}} {
    error "eight-argument execution mode must be PROVENANCE_ONLY or POSTCOMMIT_FULL_BUILD"
  }
} elseif {$argc == 9} {
  set execution_mode [lindex $argv 7]
...
proc enforce_exact_cdc_disposition {cdc_path cdc_violations} {
  ...
  set expected_g2b_xdc_sha 2E371FB39215303CCCE7E7DEB06EB59D442C391C8366FA21A56F174E7737FDAF
  ...
  set family_counts [dict create \
    OWNERSHIP_AXI_TO_SOURCE [count_matching_rows $cdc1_rows {\|G2B_ONECH_C2H/axis_slot_reg\[1\]/C\|G2B_ONECH_C2H/(own_ok_hold_source|slot_state_source)_reg}] \
    ...
    RESET_RESULT_SOURCE_TO_AXI [count_matching_rows $cdc1_rows {\|G2B_ONECH_C2H/reset_commit_phase_hold_source_reg\[2\]/C\|}]]
  ...
  foreach chain_cell $async_chain_cells {
    if {[string toupper [property_or_unknown ASYNC_REG $chain_cell]] ne "TRUE"} {
  ...
  set generated_xdc_pattern [file join $build_root vivado_project *.gen sources_1 ip \
    xdma_v41_m1 ip_0 source xdma_v41_m1_pcie2_ip-PCIE_X0Y0.xdc]
  set generated_xdc_files [glob -nocomplain -types f -- $generated_xdc_pattern]
  if {[llength $generated_xdc_files] != 1} {
    error "expected one generated XDMA PCIe XDC, found [llength $generated_xdc_files]"
  }
  set generated_xdc [lindex $generated_xdc_files 0]
...
# Reporting all 17 overlapping groups together proved prohibitively expensive
# on the qualification host.  Export the exact routed timing constraints, remove
# only set_bus_skew, and reconstruct/report one accepted group at a time.  The
...
proc run_exact_routed_bus_skew_gate {evidence_root routed_dcp_path routed_dcp_sha} {
  ...
  if {$full_raw_count != 17} {
    error "routed timing export must contain exactly 17 bus-skew constraints; found $full_raw_count"
  }
  ...
  if {$removed_count != 17 || $base_raw_count != 0} {
  ...
  set operation_code [catch {
    ...
    for {set group_id 1} {$group_id <= 17} {incr group_id} {
      reset_timing -invalid
    ...
    if {[llength $group_names] != 17 || $met_count != 17 || $violation_count != 0} {
