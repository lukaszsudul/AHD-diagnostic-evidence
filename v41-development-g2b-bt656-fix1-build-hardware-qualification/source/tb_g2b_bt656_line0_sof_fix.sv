...
module tb_g2b_bt656_line0_sof_fix;
  ...
  logic [31:0] record_capture [0:15];
  logic [31:0] record_flags [0:15];
  logic [31:0] record_attempt [0:15];
  logic [31:0] record_global [0:15];
  ...
  initial begin : RUN
    ...
    integer attempted_before_tail;
    integer committed_before_tail;
    integer overflow_before_tail;
    integer capture_before_tail;
    ...
    attempted_before_tail = dut.records_attempted_source;
    committed_before_tail = dut.records_committed_source;
    overflow_before_tail = dut.overflow_count_source;
    capture_before_tail = dut.source_capture_sequence;
    ...
    if (dut.records_attempted_source !== attempted_before_tail)
      fail($sformatf("vertical tail attempted delta=%0d",
                     dut.records_attempted_source - attempted_before_tail));
    if (dut.records_committed_source !== committed_before_tail)
      fail($sformatf("vertical tail committed delta=%0d",
                     dut.records_committed_source - committed_before_tail));
    if (dut.overflow_count_source !== overflow_before_tail)
      fail($sformatf("vertical tail overflow delta=%0d",
                     dut.overflow_count_source - overflow_before_tail));
    if (dut.source_capture_sequence !== capture_before_tail + 21)
      fail($sformatf("vertical tail capture increment expected 21 got %0d",
                     dut.source_capture_sequence - capture_before_tail));
    ...
    if ((record_flags[2] &
      ...
      fail($sformatf("line1 flags expected 00000020 got %08x",
                     record_flags[2]));
    if (record_capture[1] !== record_capture[0] + 32'd22)
      fail($sformatf("line1079-to-line0 capture delta expected 22 got %0d",
                     record_capture[1] - record_capture[0]));
    if (record_capture[2] !== record_capture[0] + 32'd23)
      fail($sformatf("line1079-to-line1 capture delta expected 23 got %0d",
                     record_capture[2] - record_capture[0]));
    if (record_attempt[1] !== record_attempt[0] + 1'b1 ||
        record_attempt[2] !== record_attempt[1] + 1'b1)
      fail("attempt sequence not consecutive across frame boundary");
    if (record_global[1] !== record_global[0] + 1'b1 ||
        record_global[2] !== record_global[1] + 1'b1)
      fail("global sequence not consecutive across frame boundary");
    if (dut.source_lifetime_malformed !== malformed_before ||
        dut.source_lifetime_dropped !== dropped_before)
      fail("boundary introduced late malformed/drop event");

    if (errors == 0)
      $display("G2B_BT656_LINE0_SOF_FIX_PASS records=%0d line0_commit=1 line0_flags=%08x line1_commit=1 line1_flags=%08x malformed_delta=0 drop_delta=0 attempted_tail_delta=0 committed_tail_delta=0 overflow_tail_delta=0 capture_delta=22 attempt_delta=1 global_delta=1",
