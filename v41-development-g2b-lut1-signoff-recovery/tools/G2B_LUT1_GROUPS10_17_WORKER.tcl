# AHD v41 G2B-LUT1 routed Groups 10-17 bus-skew sign-off worker.
#
# One process analyzes exactly one governed group.  The worker first loads and
# validates the complete promoted recovery context:
#
#   G2B_BS3_FULL_BASE_WITHOUT_GROUP9.xdc
#   followed by the exact BS3 Group-9 candidate XDC.
#
# That context contains 16 unrelated set_bus_skew commands and the candidate's
# three Group-9 set_max_delay commands.  report_bus_skew has no per-constraint
# selector, so the query is then isolated exactly as in the qualified Gen12
# worker: the 16 bus-skew command lines are removed from a worker-local copy,
# the selected Group 10-17 relation is reconstructed with its qualified
# selectors, and one compact report_bus_skew query is executed.  Group 9 is
# rejected by construction and is never queried.

set EXPECTED_DCP_SHA256 EAE2DEF4B05A04241A36B79BEF154FB455A53DB91F8C6078FCFC75A30E2ACF83
set EXPECTED_BASE_XDC_SHA256 3680EE8998503D10713D930D7D9D44AD0D71B273A9252D364A3BEE2D0D6AD507
set EXPECTED_CANDIDATE_XDC_SHA256 AE4BD91C1A8C3B1AF2FB9B0EA9A9382E9F618FD8E223BACF98E4468C10EAD087
...
proc derive_bus_skew_free_base {base_text output_path} {
  ...
  if {$removed != 16} {
    error "full base did not remove exactly 16 unrelated bus-skew commands: $removed"
  }
  set derived_text [join $kept "\n"]
  if {[count_xdc_command_lines $derived_text set_bus_skew] != 0} {
    error "derived worker-local base still contains set_bus_skew"
  }
...
proc resolve_bus_skew_group {group_id} {
  ...
  switch -- $group_id {
    ...
    default {
      error "governed worker accepts only Groups 10..17; requested group=$group_id"
    }
  }
...
proc require_bs3_candidate_collections {} {
  foreach variable_name {
    ...
    if {![uplevel 1 [list info exists $variable_name]]} {
      error "BS3 candidate collection is absent: $variable_name"
    }
  }
  set slot_src [uplevel 1 {set g2b_bs3_ownership_slot_src}]
  set generation_src [uplevel 1 {set g2b_bs3_ownership_generation_src}]
  set epoch_src [uplevel 1 {set g2b_bs3_ownership_epoch_src}]
  set destination_cells [uplevel 1 {set g2b_bs3_ownership_payload_dst_cells}]
  set destination_pins [uplevel 1 {set g2b_bs3_ownership_payload_dst_d}]
  set counts [list [llength $slot_src] [llength $generation_src] \
      [llength $epoch_src] [llength $destination_cells] \
      [llength $destination_pins]]
  if {$counts ne [list 2 24 32 17 17]} {
    error "BS3 candidate collection count drift: $counts"
  }
  return $counts
}

proc main {checkpoint base_xdc candidate_xdc output_dir group_id expected_worker_sha} {
  global EXPECTED_DCP_SHA256 EXPECTED_BASE_XDC_SHA256
  global EXPECTED_CANDIDATE_XDC_SHA256
  global result_path selected_name selected_state

  if {![string is integer -strict $group_id] ||
      $group_id < 10 || $group_id > 17} {
    error "governed worker group_id must be 10..17"
  }
  foreach required [list $checkpoint $base_xdc $candidate_xdc [info script]] {
  ...
  if {[sha256_file $base_xdc] ne $EXPECTED_BASE_XDC_SHA256} {
    error "full base XDC hash mismatch"
  }
  if {[sha256_file $candidate_xdc] ne $EXPECTED_CANDIDATE_XDC_SHA256} {
  ...
  write_lines_atomic $result_path [list \
    ...
    "BS3_CANDIDATE_XDC=$candidate_xdc" \
    "BS3_CANDIDATE_XDC_SHA256=$EXPECTED_CANDIDATE_XDC_SHA256" \
  ...
  set candidate_text [read_text $candidate_xdc]
  if {[count_xdc_command_lines $base_text set_bus_skew] != 16 ||
      [count_xdc_command_lines $base_text set_max_delay] != 9 ||
      [count_xdc_command_lines $candidate_text set_bus_skew] != 0 ||
      [count_xdc_command_lines $candidate_text set_max_delay] != 3} {
  ...
  set full_candidate_counts [require_bs3_candidate_collections]
  ...
  read_xdc $local_base
  read_xdc $candidate_xdc
  set query_candidate_counts [require_bs3_candidate_collections]
  if {$query_candidate_counts ne $full_candidate_counts} {
    error "BS3 candidate collection identity changed in isolated query context"
  }
  if {[clock_signature] ne $pre_clock_signature ||
      [route_signature] ne $pre_route_signature} {
  ...
  if {[count_xdc_command_lines $isolated_text set_bus_skew] != 1} {
    error "$selected_name query context did not export exactly one bus-skew constraint"
  }
  ...
  if {[sha256_file $checkpoint] ne $EXPECTED_DCP_SHA256 ||
      ...
      [sha256_file $candidate_xdc] ne $EXPECTED_CANDIDATE_XDC_SHA256 ||
      [route_signature] ne $pre_route_signature} {
  ...
  write_lines_atomic $result_path [list \
    ...
    {FULL_CONTEXT_BASE_BUS_SKEW_COMMAND_COUNT=16} \
    {FULL_CONTEXT_CANDIDATE_BUS_SKEW_COMMAND_COUNT=0} \
    {FULL_CONTEXT_CANDIDATE_MAX_DELAY_COMMAND_COUNT=3} \
    "WORKER_LOCAL_REMOVED_BUS_SKEW_COMMAND_COUNT=$removed_count" \
    {QUERY_CONTEXT_BUS_SKEW_COMMAND_COUNT=1} \
    "BS3_SLOT_SOURCE_COUNT=[lindex $full_candidate_counts 0]" \
    "BS3_GENERATION_SOURCE_COUNT=[lindex $full_candidate_counts 1]" \
    "BS3_EPOCH_SOURCE_COUNT=[lindex $full_candidate_counts 2]" \
    "BS3_DESTINATION_CELL_COUNT=[lindex $full_candidate_counts 3]" \
    "BS3_DESTINATION_PIN_COUNT=[lindex $full_candidate_counts 4]" \
...
if {[llength $argv] != 6} {
  puts stderr "usage: worker.tcl CHECKPOINT BASE_XDC CANDIDATE_XDC OUTPUT_DIR GROUP_ID EXPECTED_WORKER_SHA"
  exit 2
}

lassign $argv checkpoint base_xdc candidate_xdc output_dir group_id expected_worker_sha
set checkpoint [file normalize $checkpoint]
set base_xdc [file normalize $base_xdc]
set candidate_xdc [file normalize $candidate_xdc]
...
set code [catch {
  main $checkpoint $base_xdc $candidate_xdc $output_dir \
      $group_id $expected_worker_sha
} message options]
if {$code != 0} {
  ...
  write_lines_atomic $result_path [list \
    ...
    {GLOBAL_GROUP9_REPORT_BUS_SKEW_EXECUTED=NO} \
    {HARDWARE_ACCESSED=NO}]
  puts stderr "G2B_GROUPS10_17_WORKER=ERROR GROUP_ID=$group_id ERROR=[safe_value $message]"
  exit 1
}
if {$selected_state ne "PASS"} {
  puts stderr "G2B_GROUPS10_17_WORKER=VIOLATION GROUP_ID=$group_id GROUP_NAME=$selected_name"
  exit 1
}
puts "G2B_GROUPS10_17_WORKER=PASS GROUP_ID=$group_id GROUP_NAME=$selected_name"
exit 0
