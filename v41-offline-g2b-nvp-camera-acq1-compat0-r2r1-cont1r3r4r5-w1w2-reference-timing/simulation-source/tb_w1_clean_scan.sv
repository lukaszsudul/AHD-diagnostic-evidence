...
module tb_scanner_stretch;
  ...
  always @(posedge clk) begin
    if(reset) begin
      ...
      $display("W1_ACCEPT t_ns=%0.3f seq=%0d scanner_state=%0d group=%0d entry=%0d write=%0d reg=%02h data=%02h",$realtime,txn_sequence+1,scanner.state,scanner.group_index,scanner.entry_index,cmd_write,cmd_reg,cmd_wdata);
  ...
  always @(negedge sda_pin) begin
    if(!reset && scl_pin===1'b1) begin
      ...
      $display("W1_EDGE t_ns=%0.3f edge=START seq=%0d repeated=%0d scanner_state=%0d group=%0d entry=%0d",$realtime,txn_sequence,starts>1,scanner.state,scanner.group_index,scanner.entry_index);
  ...
  always @(posedge sda_pin) begin
    if(!reset && scl_pin===1'b1 && active) begin
      $display("W1_EDGE t_ns=%0.3f edge=STOP seq=%0d scanner_state=%0d group=%0d entry=%0d",$realtime,txn_sequence,scanner.state,scanner.group_index,scanner.entry_index);
  ...
  initial begin
    ...
    if($test$plusargs("W1_CLEAN")) begin testcase=14; phase_mode=7; stretch_cycles=0; end
    ...
    start_scan(); guard=0;
    while(scanner.state!=((phase_mode==5 || phase_mode==7)?10:12) && guard<20_000_000) begin
      @(negedge clk); guard++;
    end
    if(guard>=20_000_000) $fatal(1,"REAL_MASTER_CASE_CYCLE_LIMIT");
    repeat(5) @(negedge clk);
    if (phase_mode==5 || phase_mode==7) begin
      if ((phase_mode==5 && !stretch_applied) || (phase_mode==7 && stretch_applied) || target_nack_count!=0 ||
      ...
      $display("W1_CLEAN_PASS gen=%0d valid=%0d tx=%0d accepted=%0d stretch=%0d",scanner.snapshot_generation,scanner.valid_entry_count,scanner.scan_transaction_count,command_count,stretch_applied);
      $finish;
    end
