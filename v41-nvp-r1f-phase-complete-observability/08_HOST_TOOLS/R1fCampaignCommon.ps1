...
$script:R1fTaskRoot = 'C:\FPGA\V41_NVP_R1F_PHASE_COMPLETE_OBSERVABILITY'
$script:R1fToolRoot = Join-Path $script:R1fTaskRoot '08_HOST_TOOLS'
$script:R1fPrecheckRoot = Join-Path $script:R1fTaskRoot '09_HARDWARE_PRECHECK'
...
$script:R1fCanonicalTarget = 'Xilinx/80802026a98b01'
$script:R1fCanonicalSuffix = '/Xilinx/80802026a98b01'
$script:R1fExpectedPart = 'xc7a35t'
$script:R1fExpectedIdcode = '0362D093'
$script:R1fMaximumPrograms = 7
$script:R1fMaximumWarmReboots = 7
$script:R1fMaximumDriverLoads = 7

$script:R1fAcceptedTools = [ordered]@{
...
$script:R1fPhaseTable = [ordered]@{
    Bootstrap = [pscustomobject]@{
        Sequence = 0; Directory = '10_BOOTSTRAP\FORMAL_BOOTSTRAP'; Image = 'FORMAL'
    ...
    A1 = [pscustomobject]@{
        Sequence = 1; Directory = '11_PAIR_1\A1_R1F'; Image = 'R1F'
        ProgramRole = 'ARM_A_R1E'; ObserverMode = 'TRANSITION_FROM_PROVEN_CONFIGURED_IMAGE'
        ReceiptType = 'FORMAL_READY_RECEIPT'; RequiredWaitFloorSeconds = 10.0
        RemoteDriverDirectory = 'a1_driver'; Kind = 'ARM_A'
    }
    B1 = [pscustomobject]@{
        Sequence = 2; Directory = '11_PAIR_1\B1_FORMAL'; Image = 'FORMAL'
    ...
    A2 = [pscustomobject]@{
        Sequence = 3; Directory = '12_PAIR_2\A2_R1F'; Image = 'R1F'
        ProgramRole = 'ARM_A_R1E'; ObserverMode = 'TRANSITION_FROM_PROVEN_CONFIGURED_IMAGE'
        ReceiptType = 'FORMAL_READY_RECEIPT'; RequiredWaitFloorSeconds = 10.0
        RemoteDriverDirectory = 'a2_driver'; Kind = 'ARM_A'
    }
    B2 = [pscustomobject]@{
        Sequence = 4; Directory = '12_PAIR_2\B2_FORMAL'; Image = 'FORMAL'
        ProgramRole = 'ARM_B_FORMAL'; ObserverMode = 'TRANSITION_FROM_PROVEN_CONFIGURED_IMAGE'
        ReceiptType = 'VALID_ARM_A_RECEIPT'; RequiredWaitFloorSeconds = 5.0
        RemoteDriverDirectory = 'b2_driver'; Kind = 'ARM_B'
    }
    A3 = [pscustomobject]@{
        Sequence = 5; Directory = '13_PAIR_3\A3_R1F'; Image = 'R1F'
        ProgramRole = 'ARM_A_R1E'; ObserverMode = 'TRANSITION_FROM_PROVEN_CONFIGURED_IMAGE'
        ReceiptType = 'FORMAL_READY_RECEIPT'; RequiredWaitFloorSeconds = 10.0
        RemoteDriverDirectory = 'a3_driver'; Kind = 'ARM_A'
    }
    B3 = [pscustomobject]@{
        Sequence = 6; Directory = '13_PAIR_3\B3_FORMAL'; Image = 'FORMAL'
        ProgramRole = 'ARM_B_FORMAL'; ObserverMode = 'TRANSITION_FROM_PROVEN_CONFIGURED_IMAGE'
        ReceiptType = 'VALID_ARM_A_RECEIPT'; RequiredWaitFloorSeconds = 5.0
...
function Assert-R1fAcceptedToolSet {
    foreach ($entry in $script:R1fAcceptedTools.GetEnumerator()) {
        [void](Assert-R1fExactFile -Path $entry.Value.Path -Bytes $entry.Value.Bytes -Sha256 $entry.Value.Sha256)
    }
}

function Get-R1fPhaseSpec {
    param([Parameter(Mandatory)][ValidateSet('Bootstrap','A1','B1','A2','B2','A3','B3')][string]$PhaseToken)
    $base = $script:R1fPhaseTable[$PhaseToken]
    [pscustomobject]@{
        Token = $PhaseToken
        Sequence = $base.Sequence
        Directory = Join-Path $script:R1fTaskRoot $base.Directory
        Image = $base.Image
        ...
        RemoteDriverAbsolutePath = "/home/vcdeagent1/FPGA_AHD_HOST/v41_nvp_r1f/$($base.RemoteDriverDirectory)"
        Kind = $base.Kind
    }
}

function Get-R1fBindingDocument {
    ...
    if ($document.schemaVersion -ne 1 -or $document.status -cne 'FROZEN_FOR_HARDWARE') {
        throw 'R1f binding document is not schemaVersion=1/status=FROZEN_FOR_HARDWARE'
    }
    if ($document.selectedJtagCanonicalId -cne $script:R1fCanonicalTarget -or
        [string]::IsNullOrWhiteSpace([string]$document.selectedFullJtagTargetPath) -or
        (-not ([string]$document.selectedFullJtagTargetPath).EndsWith($script:R1fCanonicalSuffix, [StringComparison]::Ordinal) -and
         [string]$document.selectedFullJtagTargetPath -cne $script:R1fCanonicalTarget)) {
        throw 'selected JTAG binding is not the exact R1f canonical target'
    }
    if ($document.fpgaPart -cne $script:R1fExpectedPart -or $document.fpgaIdcode -cne $script:R1fExpectedIdcode) {
        throw 'selected JTAG part/IDCODE binding mismatch'
    }
    foreach ($name in @('formalBit','r1fBit','r1fReader')) {
        if ($null -eq $document.$name) { throw "binding document lacks $name" }
    }
    foreach ($image in @($document.formalBit,$document.r1fBit)) {
        if ([string]$image.sha256 -notmatch '^[0-9A-F]{64}$' -or [long]$image.bytes -le 0) {
            throw 'bit binding has malformed SHA-256 or byte count'
        }
        [void](Assert-R1fExactFile -Path ([string]$image.path) -Bytes ([long]$image.bytes) -Sha256 ([string]$image.sha256))
    ...
    if ([string]$document.r1fReader.sha256 -notmatch '^[0-9A-F]{64}$' -or [long]$document.r1fReader.bytes -le 0) {
        throw 'R1f reader binding is unresolved'
    }
    [void](Assert-R1fExactFile -Path ([string]$document.r1fReader.path) -Bytes ([long]$document.r1fReader.bytes) -Sha256 ([string]$document.r1fReader.sha256))
    [double]$wait = [double]$document.r1fBit.requiredWaitSeconds
    if ($wait -lt 10.0) { throw 'frozen R1f Arm-A wait is below the mandatory 10-second floor' }
    return $document
}

function Get-R1fImageBinding {
    ...
    return $Document.r1fBit
}

function Assert-R1fPhaseDirectory {
    ...
    $taskPrefix = $script:R1fTaskRoot + [IO.Path]::DirectorySeparatorChar
    if (-not $resolved.StartsWith($taskPrefix, [StringComparison]::OrdinalIgnoreCase)) {
        throw 'phase directory escaped the exact R1f task root'
    }
    return $resolved
}

function Read-R1fReceipt {
    ...
    foreach ($key in @('RECEIPT_TYPE','RECEIPT_STATUS','R1F_FULL_JTAG_TARGET_PATH')) {
...
function Get-R1fExpectedReceiptPath {
    param([Parameter(Mandatory)][ValidateSet('A1','B1','A2','B2','A3','B3')][string]$PhaseToken)
    switch ($PhaseToken) {
        'A1' { return Join-Path $script:R1fPrecheckRoot 'FORMAL_START_READY_RECEIPT.txt' }
        'B1' { return Join-Path (Get-R1fPhaseSpec A1).Directory 'VALID_ARM_A_RECEIPT.txt' }
        'A2' { return Join-Path (Get-R1fPhaseSpec B1).Directory 'FORMAL_READY_RECEIPT.txt' }
        'B2' { return Join-Path (Get-R1fPhaseSpec A2).Directory 'VALID_ARM_A_RECEIPT.txt' }
        'A3' { return Join-Path (Get-R1fPhaseSpec B2).Directory 'FORMAL_READY_RECEIPT.txt' }
        'B3' { return Join-Path (Get-R1fPhaseSpec A3).Directory 'VALID_ARM_A_RECEIPT.txt' }
    }
}

function Assert-R1fProgramBudget {
    param([Parameter(Mandatory)][object]$PhaseSpec)
    $reservations = @(Get-ChildItem -LiteralPath $script:R1fTaskRoot -Filter PROGRAM_ATTEMPT_RESERVATION.txt -File -Recurse -ErrorAction SilentlyContinue)
    if ($reservations.Count -ge $script:R1fMaximumPrograms) {
        throw "FPGA program reservation limit already reached: $($reservations.Count)/$script:R1fMaximumPrograms"
    }
