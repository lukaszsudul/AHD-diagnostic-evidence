...
$required = if($phase.Image -ceq 'R1F') {[Math]::Max(10.0,[double]$binding.r1fBit.requiredWaitSeconds)} else {5.0}
...
Require $timing R1F_FULL_JTAG_TARGET_PATH ([string]$binding.selectedFullJtagTargetPath)
...
Write-R1fUtf8NoBom -Path $outputPath -Lines @(
