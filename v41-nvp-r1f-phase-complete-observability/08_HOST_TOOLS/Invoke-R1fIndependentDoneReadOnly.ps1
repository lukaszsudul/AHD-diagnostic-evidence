...
$scriptPath = $script:R1fAcceptedTools.IndependentDoneTcl.Path
...
Write-R1fUtf8NoBom -Path $rawPath -Lines @(
    ...
    "READ_ONLY_JTAG_TCL_SHA256=$($script:R1fAcceptedTools.IndependentDoneTcl.Sha256)",
...
if ($canonical -cne $script:R1fCanonicalTarget -or $path -cne [string]$binding.selectedFullJtagTargetPath) {
    $failures.Add('SELECTED_TARGET_MISMATCH')
}
if ($deviceCount -cne '1' -or $part -cne $script:R1fExpectedPart -or $idcode -cne $script:R1fExpectedIdcode) {
...
foreach ($line in @(
    ...
    "READ_ONLY_JTAG_TCL_SHA256=$($script:R1fAcceptedTools.IndependentDoneTcl.Sha256)",
    "R1F_SELECTED_JTAG_CANONICAL_ID=$canonical",
    "R1F_FULL_JTAG_TARGET_PATH=$path",
...
Write-R1fUtf8NoBom -Path $receiptPath -Lines $lines.ToArray()
$lines
