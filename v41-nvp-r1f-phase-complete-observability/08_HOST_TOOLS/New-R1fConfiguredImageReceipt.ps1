...
if($ReceiptKind-eq'FormalReady' -and $phase.Kind-eq'ARM_A'){throw 'an R1f Arm-A result cannot issue a formal-ready receipt'}
if($ReceiptKind-ne'FormalReady' -and $phase.Kind-ne'ARM_A'){throw 'only an R1f Arm-A phase can issue an Arm-A receipt'}
...
$output=if($PhaseToken-eq'Bootstrap') {
    Join-Path $script:R1fPrecheckRoot 'FORMAL_START_READY_RECEIPT.txt'
} else {
...
if($ReceiptKind-eq'ArmATerminalSafeDone1') {
    ...
    $lines.Add('ARM_A_TERMINAL_CLASSIFICATION=R1F_INFRASTRUCTURE_INVALID')
} else {
    ...
    $hashInputs=[ordered]@{WAIT=$wait;WARM_REBOOT=$reboot;HOST_CYCLE=$cycle;PRELOADER_ADAPTER=$preloaderAdapter;PRELOADER=$preloader;LOADER=$loader;TELEMETRY=$telemetry;FINAL_DONE=$finalDone}
    ...
    if($ReceiptKind-eq'ValidArmA') {
        $lines.Add("R1F_BIT_SHA256=$([string]$binding.r1fBit.sha256)")
        ...
        $lines.Add("FORMAL_READY_SOURCE=R1F_${PhaseToken}_EXACT_FORMAL_CONTROL")
        $lines.Add('FINAL_PINNED_DRIVER_LOADED=YES')
    }
}
Write-R1fUtf8NoBom -Path $output -Lines $lines.ToArray()
$lines
