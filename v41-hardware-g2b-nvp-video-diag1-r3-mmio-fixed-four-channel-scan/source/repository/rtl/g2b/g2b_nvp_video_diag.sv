...
module g2b_nvp_video_diag #(
  ...
  localparam logic [31:0] DIAG_VERSION = 32'h0001_0002;
  ...
  localparam logic [31:0] CAP_MMIO_WRITE_RESPONSE_PROTOCOL_FIXED =
      32'h0000_0800;
  ...
  // Bit 11 identifies the corrected write-no-response/read-response protocol.
    ...
    if (reset) begin
      ...
      if (mmio_req_valid && mmio_req_ready) begin
        if (mmio_req_write) begin
          // The shared AXI-Lite bridge generates BVALID after accepting a
          // write request and never consumes a downstream response for that
          // write.  Execute the validated side effect exactly once, but do
          // not create or retain an MMIO response here.
          ...
          // Reads alone create a downstream response.  The registered valid
          // and data remain stable until mmio_rsp_ready consumes them.
          mmio_rsp_valid <= 1'b1;
