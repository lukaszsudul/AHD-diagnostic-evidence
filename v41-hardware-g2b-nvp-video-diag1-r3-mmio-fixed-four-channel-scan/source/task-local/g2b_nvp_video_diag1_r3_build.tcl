# AHD v41 G2B NVP VIDEO DIAG1-R3: fresh routed implementation stage.
#
...
#
# R3 creates one fresh nonincremental routed implementation and all raw physical
# reports.  The physical CDC rows are deliberately handed to the separate V3
# semantic reconciliation gate.  This stage must not write a bitstream.

if {$argc != 8} {
  puts stderr "usage: g2b_nvp_video_diag1_r3_build.tcl REPO_ROOT BUILD_ROOT EVIDENCE_ROOT SOURCE_COMMIT SOURCE_TREE G2B_NVP_VIDEO_DIAG1_R3_MMIO_FIXED_FOUR_CHANNEL_SCAN.bit NVP_VIDEO_DIAGNOSTIC SIMULATION_RECEIPT"
  exit 2
}

lassign $argv repo_root build_root evidence_root source_commit source_tree bit_filename build_profile simulation_receipt
if {$build_profile ne "NVP_VIDEO_DIAGNOSTIC"} {
  error "BUILD_PROFILE must be exactly NVP_VIDEO_DIAGNOSTIC"
}
set enable_rtrack_diagnostics 0
set enable_nvp_video_diagnostic 1
...
set artifact_root [file normalize [file join $task_root artifacts]]
...
set failed_diag1_commit bed970032dd4acdfbb2cf254f3e21ebcfb60c6f1
set failed_diag1_tree a0dacf5d727cf051bfb1725136e394ae43317605
set expected_r1_source_commit fc37d815b5d64ef90dfbd99c57ae4cc09567b56f
set expected_r1_source_tree cdff3ea9d786141ff9bfd46f7099198324604663
set expected_simulation_receipt_sha 737BD2A3B08C627A1252E8E690614FF5E1A1BEF62410DD308DF4D1020602B3A9
...
set expected_task AHD_V41_G2B_NVP_VIDEO_DIAG1_R3
set expected_candidate_classification G2B_NVP_VIDEO_DIAG1_R3_MMIO_PROTOCOL_FIXED_CANDIDATE
set expected_bit_filename G2B_NVP_VIDEO_DIAG1_R3_MMIO_FIXED_FOUR_CHANNEL_SCAN.bit
set expected_diag_version 0x00010002
set expected_diag_capabilities 0x00000BFF
...
set post_opt_lut_target_max 19760
set post_opt_lut_preferred_max 19136
set required_lut_reduction 1922
...
proc validate_precommit_input_manifest {manifest_path expected_lines} {
  error "PRECOMMIT_FULL_BUILD is disabled for DIAG1-R1"
...
proc enforce_exact_cdc_disposition {cdc_path cdc_violations} {
  ...
  write_lines [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R3_CDC_PHYSICAL_HANDOFF.txt] $provisional_lines
...
proc validate_committed_simulation_receipt {} {
  ...
  set expected_path [file normalize [file join $task_root sim \
    r3-complete-iteration2 NVP_VIDEO_DIAG1_R3_SIMULATION_RECEIPT.txt]]
  ...
  foreach {key expected} {
    ...
    T20_CORE_WRITE_NO_RESPONSE PASS
    T21_AXI_LITE_INTEGRATION PASS
    T22_AXI_LITE_ORDERING_BACKPRESSURE PASS
    T23_1000_CYCLE_STRESS PASS
    T24_DIAGNOSTIC_PRODUCT_ADDRESS_ISOLATION PASS
    T25_BOUNDED_PROGRESS_PROPERTIES PASS
  ...
  foreach {key rel} {
    TESTED_CORE_SHA256 rtl/g2b/g2b_nvp_video_diag.sv
    TESTED_CORE_PROTOCOL_TB_SHA256 tests/nvp_video_diag/tb_g2b_nvp_video_diag_mmio_protocol.sv
    TESTED_AXI_INTEGRATION_TB_SHA256 tests/nvp_video_diag/tb_g2b_nvp_video_diag_axi_integration.sv
    TESTED_SHARED_BRIDGE_SHA256 rtl/v41/axi_lite_host_bridge.sv
    TESTED_MMIO_ROUTER_SHA256 rtl/g2b/v41_g2b_mmio_router.sv
  } {
    set actual_sha [sha256_file [file join $repo_root $rel]]
    require_receipt_value $values $key $actual_sha
  }
...
proc validate_r1_source_architecture {} {
  ...
  set core_path [file join $repo_root rtl g2b g2b_nvp_video_diag.sv]
  set text [read_text $core_path]
  foreach forbidden {result_words result_reset_index base_index table_word} {
    if {[regexp -nocase -- $forbidden $text]} {
      error "R1 source architecture contains forbidden retained-history token: $forbidden"
    }
  }
  set declarations [regexp -all -- \
    {logic[ \t]+\[31:0\][ \t]+current_result_word_[0-7][ \t]*;} $text]
  if {$declarations != 8} {
    error "R1 source architecture requires exactly eight explicit current-result declarations; found $declarations"
  }
  foreach index {0 1 2 3 4 5 6 7} {
    if {[regexp -all -- "current_result_word_${index}" $text] < 3} {
      error "current_result_word_$index is not declared, written, and exposed through MMIO"
    }
  }
  foreach metadata {current_result_valid current_result_session_id current_result_generation} {
    if {![regexp -- $metadata $text]} {
      error "R1 source architecture is missing $metadata"
    }
  }
  if {![regexp {DIAG_VERSION[ \t]*=[ \t]*32'h0001_0002} $text]} {
    error "R3 diagnostic source version is not 0x00010002"
  }
  if {$expected_diag_version ne "0x00010002" || $expected_diag_capabilities ne "0x00000BFF"} {
    error "internal R3 diagnostic identity constants are inconsistent"
  }
  if {![regexp {CAP_CURRENT_SESSION_SNAPSHOT_V1[ \t]*=[ \t]*32'h0000_0100} $text] ||
      ![regexp {CAP_HOST_OWNED_SESSION_HISTORY[ \t]*=[ \t]*32'h0000_0200} $text] ||
      ![regexp {CAP_ONCHIP_16_SESSION_HISTORY[ \t]*=[ \t]*32'h0000_0400} $text] ||
      ![regexp {CAP_MMIO_WRITE_RESPONSE_PROTOCOL_FIXED\s*=\s*32'h0000_0800} $text]} {
    error "R1 diagnostic capability constants are missing or changed"
  }
  set address_count 0
  foreach address {03d00 03d04 03d08 03d0c 03d10 03d14 03d18 03d1c 03d20 03d24 03d28} {
    set count [regexp -all -nocase -- "17'h${address}" $text]
    if {$count != 1} {
      error "R1 snapshot MMIO address 0x$address occurs $count times instead of exactly once"
    }
    incr address_count $count
  }
  set all_3d [regexp -all -nocase -- {17'h03d[0-9a-f][0-9a-f]} $text]
  set all_3e [regexp -all -nocase -- {17'h03e[0-9a-f][0-9a-f]} $text]
  if {$address_count != 11 || $all_3d != 11 || $all_3e != 0 ||
      ![regexp {default:[ \t]+mmio_read_word[ \t]*=[ \t]*32'b0} $text]} {
    error "R1 snapshot/reserved MMIO source architecture is not the exact sparse zero-default map"
  }
  return [dict create \
    RESULT_WORDS_ARRAY_REFERENCES 0 BULK_RESULT_TABLE_RESET_LOOPS 0 \
    ASYNC_128_WORD_MMIO_READ_MUX 0 ONCHIP_SESSION_HISTORY_ENTRIES 0 \
    ONCHIP_CURRENT_RESULT_WORDS 8 ONCHIP_CURRENT_SNAPSHOT_DATA_BITS 256 \
    ONCHIP_HISTORY_BITS 289 RESERVED_MMIO_ZERO_DEFAULT PASS \
...
proc gate_r1_result_storage_netlist {stage} {
  ...
  set data_family_count 0
  set data_sequential_cells 0
  set family_lines [list]
  for {set index 0} {$index < 8} {incr index} {
    set candidates [get_cells -quiet -hier -regexp ".*current_result_word_${index}_reg.*"]
    set sequential [filter $candidates {IS_SEQUENTIAL == 1}]
    set count [llength $sequential]
    if {$count == 0} {
      error "$stage netlist is missing current_result_word_$index sequential storage"
    }
    incr data_family_count
    incr data_sequential_cells $count
    lappend family_lines "CURRENT_RESULT_WORD_${index}_SEQUENTIAL_CELLS=$count"
  }
  set broad_memory_cells [list]
  foreach cell [get_cells -quiet -hier] {
    set name [get_property NAME $cell]
    if {![regexp {NVP_VIDEO_DIAG_CORE} $name]} { continue }
    set ref [string toupper [property_or_unknown REF_NAME $cell]]
    if {[regexp {^(RAMB|RAMD|RAMS|SRL)} $ref]} { lappend broad_memory_cells $cell }
  }
  if {[llength $old_cells] != 0 || [llength $old_nets] != 0 ||
      $data_family_count != 8 || [llength $broad_memory_cells] != 0} {
    error "$stage result-storage architecture gate failed: old_cells=[llength $old_cells] old_nets=[llength $old_nets] current_families=$data_family_count broad_memory=[llength $broad_memory_cells]"
  }
  write_lines [file join $evidence_root "${stage}_RESULT_STORAGE_ARCHITECTURE.txt"] [concat [list \
    "STAGE=$stage" \
    {ONCHIP_16_SESSION_HISTORY=ABSENT} \
    {ONCHIP_CURRENT_SNAPSHOT_WORDS=8} \
    {ONCHIP_CURRENT_SNAPSHOT_DATA_BITS=256} \
    "CURRENT_SNAPSHOT_OPTIMIZED_SEQUENTIAL_CELLS=$data_sequential_cells" \
    {BULK_RESET_MEMORY=ABSENT} \
    {ASYNC_128_TO_1_RESULT_MUX=ABSENT} \
    {HOST_OWNED_SESSION_HISTORY_CAPABILITY=PRESENT} \
    "FORBIDDEN_HISTORY_CELLS=[llength $old_cells]" \
    "FORBIDDEN_HISTORY_NETS=[llength $old_nets]" \
    "DIAGNOSTIC_BROAD_MEMORY_OR_SRL_CELLS=[llength $broad_memory_cells]"] $family_lines [list {RESULT=PASS}]]
  return PASS
}
...
proc fix1_r1_recovery_main {} {
  error "ROUTED_DCP_RECOVERY is disabled for DIAG1-R1"
...
if {![regexp {^G2B_NVP_VIDEO_DIAG1_R3_[0-9]{8}T[0-9]{6}Z$} [file tail $task_root]] ||
    [path_key $evidence_root] ne [path_key [file join $task_root reports vivado_full]] ||
    [path_key $artifact_root] ne [path_key [file join $task_root artifacts]]} {
...
# Exact R1 source identity.  Owner continuity makes broad PRODUCT/G2A/tag
# requalification out of scope; only branch, HEAD/tree, tracked cleanliness,
# and bounded ancestry from the frozen failed-DIAG1 commit are checked.
...
set actual_merge_base [git_value merge-base $failed_diag1_commit $source_commit]
set direct_parent [git_value rev-parse "${source_commit}^"]
set integration_commit_count \
  [git_value rev-list --count "${failed_diag1_commit}..${source_commit}"]
if {$actual_merge_base ne $failed_diag1_commit ||
    $integration_commit_count < 1 || $integration_commit_count > 3} {
  error "R3 source is not a bounded one-to-three-commit descendant of failed DIAG1"
}
set direct_base_relation PASS_BOUNDED_DESCENDANT_OF_FAILED_DIAG1
...
# BUILD_FLAGS[10] identifies the four-channel NVP video diagnostic.  Bits [1:0]
# retain the established clean/dirty manifest provenance semantics.
set profile_build_flag 0x00000400
...
set provenance_lines [list \
  ...
  "FAILED_DIAG1_PARENT=$failed_diag1_commit" \
  "MERGE_BASE_WITH_FAILED_DIAG1=$actual_merge_base" \
  ...
  "BUILD_FLAGS_NVP_VIDEO_DIAGNOSTIC_BIT=1" \
...
write_lines [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R1_SOURCE_ARCHITECTURE_GATE.txt] \
...
set build_result [catch {
  ...
  dict set allowed_preexisting [path_key [file join $evidence_root \
    G2B_NVP_VIDEO_DIAG1_R1_SOURCE_ARCHITECTURE_GATE.txt]] 1
  ...
  create_project v41_g2b_nvp_video_diag1_r3 $project_dir -part $expected_part
  ...
  set profile_nvp_diag_core_cells [profile_cells NVP_VIDEO_DIAG_CORE \
    g2b_nvp_video_diag]
  set profile_nvp_diag_i2c_cells [profile_cells NVP_VIDEO_DIAG_I2C_MASTER \
  ...
  if {$profile_probe_count != 0 || $profile_lifecycle_count != 0 ||
      ...
      $profile_nvp_diag_core_count == 0 ||
      $profile_nvp_diag_i2c_count == 0} {
  ...
  write_lines [file join $evidence_root G2B_PROFILE_ELABORATION_RECEIPT.txt] [list \
    ...
    "NVP_VIDEO_DIAG_CORE_COUNT=$profile_nvp_diag_core_count" \
    "NVP_VIDEO_DIAG_I2C_MASTER_COUNT=$profile_nvp_diag_i2c_count" \
  ...
  set synth_dcp_path [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R3_SYNTH.dcp]
  ...
  set post_opt_dcp_path [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R3_POST_OPT.dcp]
  ...
  if {$post_opt_resource_gate ne "PASS"} {
    ...
    if {[string match {LUT_*} $post_opt_resource_gate_reason]} {
      error "FAIL - NVP_DIAG1_R1_POST_OPT_LUT_GATE_FAILED: $post_opt_resource_gate_reason"
    }
    error "FAIL - NVP_DIAG1_R1_POST_OPT_RESOURCE_GATE_FAILED: $post_opt_resource_gate_reason"
  }
  ...
  set routed_dcp_path [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R3_ROUTED.dcp]
  ...
  write_lines [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R1_METHODOLOGY_GATE.txt] [list \
  ...
  if {$cdc_critical != 0} {
    ...
    set cdc_disposition PASS_RAW_PROFILE_COUNTS_PENDING_R3_SEMANTIC_MANIFEST_V3
  }
  ...
  fix1_r1_check_rule_counts CDC_WARNING $cdc_warning_objects \
    [dict create CDC-6 13 CDC-15 861] {WARNING} \
    [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R1_CDC_WARNING_OBJECTS.csv]
  ...
  # R3 requires a profile-specific semantic CDC V3 reconciliation against the
  ...
  write_lines [file join $evidence_root G2B_NVP_VIDEO_DIAG1_R3_ROUTED_BUILD_HANDOFF.txt] [list \
    {RESULT=PASS} \
...
set terminal_lines [list \
  ...
  "REPOSITORY_HEAD=$source_commit" "REPOSITORY_HEAD_TREE=$source_tree" \
  "SOURCE_CLEAN=$source_clean" \
  "R1_SOURCE_PARENT=$failed_diag1_commit" \
  "R1_SOURCE_DIRECT_PARENT=$direct_parent" \
  "R1_SOURCE_COMMITS_AFTER_FAILED_DIAG1=$integration_commit_count" \
  "PRECOMMIT_INPUT_MANIFEST_SHA256=$precommit_manifest_sha" \
  "SIMULATION_RECEIPT=$simulation_receipt" \
  "SIMULATION_RECEIPT_SHA256=$simulation_receipt_sha" \
  {INHERITED_DIAG1_SIMULATION_GATE=16/16_PASS} \
  ...
  "NVP_VIDEO_DIAGNOSTIC_RTL_INCLUDED=YES" \
...
puts "G2B_R3_ROUTED_BUILD_HANDOFF_PASS ROUTED_DCP_SHA256=$routed_dcp_sha BITSTREAM_PRODUCED=NO"
exit 0
