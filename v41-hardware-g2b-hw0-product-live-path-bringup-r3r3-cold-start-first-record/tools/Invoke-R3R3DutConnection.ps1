...
function Assert-PrivateAcl([string]$Path){
 ...
 if(-not $acl.AreAccessRulesProtected -or $rules.Count -ne 1 -or $rules[0].IdentityReference.Value -cne $sid.Value -or $rules[0].AccessControlType -ne 'Allow'){throw 'R3R3_PRIVATE_ACL_INVALID'}
}
if([IO.Path]::GetFullPath($PSScriptRoot) -cne (Join-Path $run 'scripts')){throw 'R3R3_HELPER_LOCATION_INVALID'}
if(Test-Path -LiteralPath $receipt){throw 'R3R3_RECEIPT_EXISTS'}
Assert-PrivateAcl $private
if(@(Get-ChildItem -LiteralPath $private -Force).Count){throw 'R3R3_PRIVATE_NOT_EMPTY'}
if((Get-FileHash -LiteralPath $plink).Hash -cne $plinkHash){throw 'R3R3_PLINK_HASH_MISMATCH'}
...
try {
 ...
 foreach($line in [IO.File]::ReadAllLines('C:\FPGA\VCDE-DUT-1.txt')){
  ...
  if($line -notmatch '^\s*([^:=]+?)\s*[:=]\s*(.*?)\s*$'){throw 'R3R3_CREDENTIAL_FORMAT'}
  $label=($Matches[1] -replace '[^A-Za-z0-9]','').ToLowerInvariant();$val=$Matches[2]
  $key=switch($label){ {$_ -in @('ip','adresip')}{'ip'} {$_ -in @('user','username','uzytkownik','sudouser')}{'user'} {$_ -in @('password','haslo')}{'password'} default {throw 'R3R3_CREDENTIAL_FIELD'}}
  if($fields.ContainsKey($key)){throw 'R3R3_DUPLICATE_CREDENTIAL_FIELD'}
  $fields[$key]=$val
 }
 if($fields.Count -ne 3 -or $fields.ip -cne $ip -or $fields.user -cne $login){throw 'R3R3_CREDENTIAL_IDENTITY'}
 $pw=[string]$fields.password;$fields.Clear();$val=$null;$line=$null
 if([string]::IsNullOrWhiteSpace($pw)){throw 'R3R3_EMPTY_CREDENTIAL'}
 ...
 if(-not [IO.Path]::GetFullPath($temp).StartsWith($private+'\',[StringComparison]::OrdinalIgnoreCase)){throw 'R3R3_TEMP_BOUNDARY'}
 ...
 for($i=0;$i -lt $argsList.Count;$i++){
  if($argsList[$i] -ceq '-pw'){throw 'R3R3_SECRET_ARGUMENT'}
  if($i -ne 6 -and $argsList[$i].Contains($pw,[StringComparison]::Ordinal)){throw 'R3R3_SECRET_ARGUMENT'}
 ...
 if(-not $started){throw 'R3R3_PROCESS_START'}
 ...
 if(-not $proc.WaitForExit($TimeoutSeconds*1000)){$proc.Kill($true);$proc.WaitForExit();$exitCode=124;throw 'R3R3_CONNECTION_TIMEOUT'}
 $stdout=$outTask.GetAwaiter().GetResult();$stderr=$errTask.GetAwaiter().GetResult();$exitCode=$proc.ExitCode
}catch{
 $problem=if($_.Exception.Message -like 'R3R3_*'){$_.Exception.Message}else{'R3R3_SANITIZED_LOCAL_EXCEPTION'}
}finally{
...
if(-not $deleted){$exitCode=126;$problem='R3R3_CREDENTIAL_DELETE_FAILED'}
$result=[ordered]@{task='G2B-HW0-PRODUCT-R3R3';helper=$PSCommandPath;helper_sha256=(Get-FileHash -LiteralPath $PSCommandPath).Hash;start_utc=$start;end_utc=[DateTime]::UtcNow.ToString('o');ip=$ip;host_key_pinned=$true;credential_in_process_arguments=$false;temporary_path_class='R3R3_PRIVATE';temporary_created=$created;temporary_deleted=$deleted;private_remaining=@(Get-ChildItem -LiteralPath $private -Force).Count;process_started=$started;exit_code=$exitCode;problem=$problem;stdout=$stdout;stderr=$stderr}
$json=$result | ConvertTo-Json -Depth 5
[IO.File]::WriteAllText($receipt,$json+"`n",[Text.UTF8Encoding]::new($false))
$pw=$null
Write-Output $json
if($exitCode -ne 0 -or $problem){throw 'R3R3_CONNECTION_FAILED_SEE_RECEIPT'}
