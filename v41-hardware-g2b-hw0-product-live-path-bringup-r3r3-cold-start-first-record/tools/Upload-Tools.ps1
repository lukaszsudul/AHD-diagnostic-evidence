...
$remoteRoot='/home/vcdeagent1/vcde_artifacts/g2b_hw0_product_r3r3/20260906T200624Z/scripts'
foreach($name in @('abi_v1.py','V41_C2H_TRANSPORT_ABI_V1.json','capture.py','t2.py','selftest.py')){
 ...
 for($offset=0;$offset -lt $bytes.Length;$offset+=7000){
  ...
  & (Join-Path $PSScriptRoot 'Invoke-R3R3DutConnection.ps1') -ReceiptName ('upload-'+$name.Replace('.','-')+'-'+$part) -Sudo -RemoteCommand $cmd | Out-Null
  $part++
 }
 ...
 & (Join-Path $PSScriptRoot 'Invoke-R3R3DutConnection.ps1') -ReceiptName ('verify-upload-'+$name.Replace('.','-')) -Sudo -RemoteCommand ("python3 - <<'VERIFY'`n$code`nVERIFY") | Out-Null
