...
param(
  ...
  [Parameter(Mandatory,ParameterSetName='Download')][string]$DownloadRemotePath,
  [Parameter(Mandatory,ParameterSetName='Download')][string]$DownloadLocalPath,
...
# Sanitized publication copy: governed account and its home-path component are
# redacted because the credential-remnant gate treats that literal as secret.

$run = 'C:\FPGA\G2B_HW0_VIDEO_CONTENT0_20260909T150127Z'
$linuxRun = '/home/<GOVERNED_ACCOUNT_REDACTED>/vcde_artifacts/g2b_hw0_video_content0/20260909T150127Z'
...
$login = '<GOVERNED_ACCOUNT_REDACTED>'
$ip = '10.132.1.111'
...
function Assert-PrivateAcl([string]$Path) {
  ...
  if (-not $acl.AreAccessRulesProtected -or $rules.Count -ne 1 -or
    ...
    throw 'BT656FIX1_PRIVATE_ACL_INVALID'
  }
}

if ([IO.Path]::GetFullPath($PSScriptRoot) -cne (Join-Path $run 'scripts')) {
  throw 'BT656FIX1_HELPER_LOCATION_INVALID'
}
if (Test-Path -LiteralPath $receipt) { throw 'BT656FIX1_RECEIPT_EXISTS' }
...
if (@(Get-ChildItem -LiteralPath $credentialTemp -Force).Count) {
  throw 'BT656FIX1_CREDENTIAL_TEMP_NOT_EMPTY'
}
...
try {
  ...
  foreach ($line in [IO.File]::ReadAllLines('C:\FPGA\VCDE-DUT-1.txt')) {
    ...
    if ($line -notmatch '^\s*([^:=]+?)\s*[:=]\s*(.*?)\s*$') {
      throw 'BT656FIX1_CREDENTIAL_FORMAT'
    }
    ...
    $key = switch ($label) {
      ...
      default { throw 'BT656FIX1_CREDENTIAL_FIELD' }
    }
    if ($fields.ContainsKey($key)) { throw 'BT656FIX1_DUPLICATE_CREDENTIAL_FIELD' }
    $fields[$key] = $val
  }
  if ($fields.Count -ne 3 -or $fields.ip -cne $ip -or $fields.user -cne $login) {
    throw 'BT656FIX1_CREDENTIAL_IDENTITY'
  }
  $pw = [string]$fields.password
  $fields.Clear()
  $val = $null
  $line = $null
  if ([string]::IsNullOrWhiteSpace($pw)) { throw 'BT656FIX1_EMPTY_CREDENTIAL' }
  ...
  if ($mode -eq 'UPLOAD') {
    ...
    $escapedRun = [Regex]::Escape($linuxRun)
    if ($DownloadRemotePath -notmatch ('^' + $escapedRun + '/(private|images|logs|reports)/[A-Za-z0-9._/-]+$')) {
      throw 'VIDEOCONTENT0_DOWNLOAD_SOURCE_BOUNDARY'
    }
    $resolvedDownload = [IO.Path]::GetFullPath($DownloadLocalPath)
    $allowedDownloadRoots = @(
      ((Join-Path $run 'private') + '\')
      ((Join-Path $run 'images') + '\')
      ((Join-Path $run 'logs') + '\')
      ((Join-Path $run 'reports') + '\')
    )
    if (-not ($allowedDownloadRoots | Where-Object {
      $resolvedDownload.StartsWith($_,[StringComparison]::OrdinalIgnoreCase)
    })) {
      throw 'VIDEOCONTENT0_DOWNLOAD_DESTINATION_BOUNDARY'
    }
    if (Test-Path -LiteralPath $resolvedDownload) {
      throw 'VIDEOCONTENT0_DOWNLOAD_DESTINATION_EXISTS'
    }
    $parent = Split-Path -Parent $resolvedDownload
    if (-not (Test-Path -LiteralPath $parent -PathType Container)) {
      throw 'VIDEOCONTENT0_DOWNLOAD_DESTINATION_PARENT_MISSING'
    }
    $remotePlain = "cat -- '$DownloadRemotePath'"
  } else {
  ...
  for ($i=0; $i -lt $argsList.Count; $i++) {
    if ($argsList[$i] -ceq '-pw') { throw 'BT656FIX1_SECRET_ARGUMENT' }
    if ($i -ne 6 -and $argsList[$i].Contains($pw,[StringComparison]::Ordinal)) {
      throw 'BT656FIX1_SECRET_ARGUMENT'
    }
  ...
  $proc.StartInfo = $psi
  $started = $proc.Start()
  if (-not $started) { throw 'BT656FIX1_PROCESS_START' }
  if ($mode -eq 'DOWNLOAD') {
    $downloadStream = [IO.File]::Open($resolvedDownload,'CreateNew','Write','None')
    $copyTask = $proc.StandardOutput.BaseStream.CopyToAsync($downloadStream)
  } else {
  ...
  if (-not $proc.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    $proc.WaitForExit()
    $exitCode = 124
    throw 'BT656FIX1_CONNECTION_TIMEOUT'
  }
  if ($mode -eq 'DOWNLOAD') {
    [void]$copyTask.GetAwaiter().GetResult()
    $downloadStream.Flush($true)
    $downloadStream.Dispose()
    $downloadStream = $null
    $downloadBytes = (Get-Item -LiteralPath $resolvedDownload).Length
    $downloadSha256 = (Get-FileHash -LiteralPath $resolvedDownload).Hash
    $stdout = '<BINARY_FILE_TRANSFER_REDACTED>'
  } else {
  ...
  $problem = if ($_.Exception.Message -like 'BT656FIX1_*' -or
                 $_.Exception.Message -like 'VIDEOCONTENT0_*') {
    $_.Exception.Message
  } else {
    'BT656FIX1_SANITIZED_LOCAL_EXCEPTION'
  }
} finally {
  if ($downloadStream) {
    $downloadStream.Dispose()
...
if (-not $deleted) { $exitCode=126; $problem='BT656FIX1_CREDENTIAL_DELETE_FAILED' }
$remaining = @(Get-ChildItem -LiteralPath $credentialTemp -Force).Count
if ($remaining -ne 0) { $exitCode=127; $problem='BT656FIX1_CREDENTIAL_REMNANT' }
$result = [ordered]@{
  task='G2B-HW0-VIDEO-CONTENT0'
  mode=$mode
  ...
  temporary_path_class='BT656FIX1_PRIVATE_CREDENTIAL_TEMP'
...
$pw = $null
Write-Output $json
if ($exitCode -ne 0 -or $problem) { throw 'BT656FIX1_CONNECTION_FAILED_SEE_RECEIPT' }
