if {$argc != 0} {
  puts stderr {usage: program-product-once-r3r4r6r2r2.tcl}
  exit 2
}
...
set invoked 0
set rc 0
emit TASK_ID G2B-HW0-PRODUCT-R3R4R6R2R2
emit PROGRAMMED_STORAGE FPGA_SRAM_VOLATILE_ONLY
emit OWNER_TRUSTED_BITSTREAM_HASH AF10C6108B5D99AD239E0F0008ACF7C790333CA1FDD69FD775394091CDEEF4B7
emit BITSTREAM_REHASH_PERFORMED NO
...
if {[catch {
  ...
  if {$part ne $expected_part || $idcode ne $expected_idcode} {
    error "programming target identity mismatch: $part $idcode"
  }
  ...
  if {$post_done ne {1}} { error "DONE not asserted after programming: $post_done" }
