#!/usr/bin/env python3
"""Disk-based fixed-boundary validation for the R3R4R6R2R2 primary window."""
...
def main() -> int:
    ...
    try:
        ...
        if len(raw) != PRIMARY_BYTES:
            raise RuntimeError(f"R3R4R6R2R2_PRIMARY_SIZE:{len(raw)}:{PRIMARY_BYTES}")
        ...
        if frame_start is not None and frame_end is not None and selected is not None:
            ...
            if len(frame) != 4_147_200:
                raise RuntimeError("R3R4R6R2R2_FRAME_SIZE_INVALID")
        ...
        result = {
            ...
            "blocker": None if primary_pass and frame_pass else
                "R3R4R6R2R2_PRIMARY_RECORD_OR_FRAME_QUALIFICATION_FAILED",
