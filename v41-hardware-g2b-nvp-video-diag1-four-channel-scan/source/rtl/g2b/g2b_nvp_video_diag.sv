...
module g2b_nvp_video_diag #(
  ...
  localparam logic [31:0] DIAG_VERSION = 32'h0001_0000;
  localparam logic [31:0] DIAG_CAPABILITIES = 32'h0000_00ff;
  ...
  localparam logic [63:0] SETTLE_CYCLES = SETTLE_TIME_MS * CYCLES_PER_MS;
  localparam logic [63:0] SAMPLE_INTERVAL_CYCLES =
      STATUS_SAMPLE_INTERVAL_MS * CYCLES_PER_MS;
  localparam logic [63:0] MAX_STATUS_WAIT_CYCLES =
  ...
  logic [63:0] delay_counter;
  logic [63:0] status_wait_counter;
  logic [7:0] config_verify_index;
  logic txn_pending;

  logic [31:0] result_words [0:127];
  ...
  function automatic [31:0] mmio_read_word(input logic [16:0] address);
    logic [8:0] table_word;
    begin
      table_word = (address - 17'h03d00) >> 2;
      case (address)
        ...
        default: begin
          if (address >= 17'h03d00 && address <= 17'h03eff &&
              address[1:0] == 2'b00 && table_word < 128)
            mmio_read_word = result_words[table_word];
          else
  ...
  always_ff @(posedge clk) begin
    if (reset) begin
      ...
      i2c_cmd_reg <= 8'b0;
      i2c_cmd_wdata <= 8'b0;
      for (result_reset_index = 0; result_reset_index < 128;
           result_reset_index = result_reset_index + 1)
        result_words[result_reset_index] <= 32'b0;
    end else begin
      i2c_cmd_valid <= 1'b0;
        ...
        case (state)
          ...
          WAIT_HOST_CAPTURE: begin
            if (host_response_pulse) begin
              ...
              if (host_response_data[31:16] != current_session_id)
                latch_error(ERR_HOST_PROTOCOL);
              else if (host_response_data[2])
                latch_error(ERR_OWNER_ABORT);
          ...
          STORE_RESULT: begin : STORE_RESULT_BLOCK
            integer base_index;
            base_index = (current_session_id - 1) * 8;
            result_words[base_index + 0] <=
                {current_session_id, 2'b0, current_round + 1'b1,
                 current_channel, current_classification};
            result_words[base_index + 1] <=
                {{5'b0, current_channel - 1'b1}, current_route_readback,
                 {4'b0, round_color(current_round, current_channel)},
                 stable_sample_count};
            result_words[base_index + 2] <=
                {raw_novid, raw_agc_lock, raw_cmp_lock, raw_h_lock};
            result_words[base_index + 3] <=
                {raw_channel_status, current_bg78_readback,
                 current_bg79_readback, total_status_samples};
            result_words[base_index + 4] <=
                {24'b0, round_position, current_round, current_channel};
            result_words[base_index + 5] <= i2c_transaction_count;
            result_words[base_index + 6] <= last_host_capture_response;
            result_words[base_index + 7] <= {16'b0, error_code};
