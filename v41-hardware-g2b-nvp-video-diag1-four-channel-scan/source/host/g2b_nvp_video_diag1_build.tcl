# AHD v41 G2B NVP VIDEO DIAG1: four-channel diagnostic, offline build only.
#
...
if {$argc != 7 && $argc != 8 && $argc != 9} {
  puts stderr "usage: g2b_nvp_video_diag1_build.tcl REPO_ROOT BUILD_ROOT EVIDENCE_ROOT SOURCE_COMMIT SOURCE_TREE BIT_FILENAME NVP_VIDEO_DIAGNOSTIC ?PROVENANCE_ONLY|POSTCOMMIT_FULL_BUILD?"
  exit 2
}
...
set actual_hardened_product_tree [git_value rev-parse "${accepted_hardened_product_commit}^{tree}"]
...
if {$actual_hardened_product_tree ne $accepted_hardened_product_tree} { error "BLOCKED - ACCEPTED_HARDENED_PRODUCT_TREE_MISMATCH" }
...
if {$precommit_mode} {
  ...
  if {$actual_merge_base ne $accepted_g2a_base_commit ||
      ...
      $direct_parent ne $accepted_hardened_product_commit ||
      $correction_commit_count ne "3"} {
    error "postcommit diagnostic source is not the direct child of the hardware-qualified hardened PRODUCT commit"
  }
  set direct_base_relation PASS_DIAGNOSTIC_CHILD_OF_HARDWARE_QUALIFIED_PRODUCT
...
set provenance_lines [list \
  "TASK=AHD_V41_G2B_NVP_VIDEO_DIAG1" \
...
set build_result [catch {
  ...
  create_project v41_g2b_nvp_video_diag1 $project_dir -part $expected_part
