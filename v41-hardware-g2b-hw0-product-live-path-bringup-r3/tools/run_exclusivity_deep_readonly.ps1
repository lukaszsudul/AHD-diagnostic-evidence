...
$evidencePath = Join-Path $taskRoot 'raw\EXCLUSIVITY_DEEP_READONLY.log'
...
if (Test-Path -LiteralPath $evidencePath) { throw 'DEEP_EXCLUSIVITY_EVIDENCE_EXISTS' }
...
if ($lock.state -cne 'HELD' -or $lock.task -cne 'G2B-HW0-PRODUCT-R3' -or $lock.linux_lock_state -cne 'HELD') {
    throw 'LOCK_STATE_INVALID_FOR_DEEP_EXCLUSIVITY'
}
...
& $helper `
    ...
    -EvidenceKind 'R3_EXCLUSIVITY_DEEP_READONLY' `
    -SendPasswordToStdin `
