...
$scriptPath = Join-Path $taskRoot 'tools\prelock_identity_exclusivity_readonly.sh'
$evidencePath = Join-Path $taskRoot 'raw\PRELOCK_IDENTITY_EXCLUSIVITY_READONLY.log'
...
if (Test-Path -LiteralPath $evidencePath) {
    throw 'PRELOCK_EVIDENCE_ALREADY_EXISTS'
}
...
if ($lock.state -cne 'HELD' -or $lock.task -cne 'G2B-HW0-PRODUCT-R3' -or $lock.linux_lock_state -cne 'NOT_YET_ACQUIRED') {
    throw 'CONTROLLER_LOCK_STATE_INVALID_FOR_PRELOCK'
}
...
& $helper `
    ...
    -EvidenceKind 'R3_PRELOCK_IDENTITY_EXCLUSIVITY_READONLY' `
    -TimeoutSeconds 45
exit $LASTEXITCODE
