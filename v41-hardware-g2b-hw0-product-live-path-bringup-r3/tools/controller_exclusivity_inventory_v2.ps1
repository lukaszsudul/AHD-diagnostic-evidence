...
$evidence = Join-Path $taskRoot 'raw\CONTROLLER_EXCLUSIVITY_INVENTORY_V2.log'
$controllerLockDir = 'C:\FPGA\.AHD_DUT_EXCLUSIVE_LOCK'
$controllerReceipt = Join-Path $controllerLockDir 'receipt.json'
if (Test-Path -LiteralPath $evidence) { throw 'CONTROLLER_EXCLUSIVITY_V2_EVIDENCE_EXISTS' }
...
$lines.Add('PHASE=CONTROLLER_EXCLUSIVITY_INVENTORY_V2')
...
$lines.Add("EXACT_EXPECTED_CONTROLLER_LOCK_ONLY=$(if ($receiptBearingLocks.Count -eq 1 -and $receiptBearingLocks[0].FullName -ceq $controllerLockDir) {'YES'} else {'NO'})")

$runningTasks = @()
if (Get-Command Get-ScheduledTask -ErrorAction SilentlyContinue) {
    $runningTasks = @(Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object { $_.State -eq 'Running' })
}
$hardwareScheduled = @($runningTasks | Where-Object { ($_.TaskPath + $_.TaskName) -match '(?i)ahd|g2b|xdma|jtag|fpga|vivado|reboot|shutdown|power' })
$lines.Add('RUNNING_HARDWARE_SCHEDULED_TASKS_BEGIN')
foreach ($scheduled in $hardwareScheduled) { $lines.Add("TASK_PATH=$($scheduled.TaskPath) TASK_NAME=$($scheduled.TaskName)") }
$lines.Add('RUNNING_HARDWARE_SCHEDULED_TASKS_END')
$lines.Add("RUNNING_HARDWARE_SCHEDULED_TASK_COUNT=$($hardwareScheduled.Count)")
...
$pass = $relevant.Count -eq 0 -and $tcp.Count -eq 0 -and $receiptBearingLocks.Count -eq 1 -and
    $receiptBearingLocks[0].FullName -ceq $controllerLockDir -and $hardwareScheduled.Count -eq 0
...
if (-not $pass) { throw 'CONTROLLER_EXCLUSIVITY_V2_FAILED' }
