...
$helper = Join-Path $taskRoot 'tools\Invoke-G2BR3Plink.ps1'
...
$scriptPath = Join-Path $taskRoot 'tools\verify_sealed_module_readonly.sh'
$evidencePath = Join-Path $taskRoot 'raw\SEALED_MODULE_IMMEDIATE_PRELOAD_VERIFICATION.log'

foreach ($required in @($controllerReceipt, $helper, $plink, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "DRIVER_VERIFY_PRECONDITION_MISSING=$required" }
}
if (Test-Path -LiteralPath $evidencePath) { throw 'DRIVER_VERIFY_EVIDENCE_ALREADY_EXISTS' }
...
if ($lock.state -cne 'HELD' -or $lock.task -cne 'G2B-HW0-PRODUCT-R3' -or $lock.linux_lock_state -cne 'HELD') {
    throw 'LOCK_STATE_INVALID_FOR_DRIVER_VERIFY'
}
...
& $helper `
    ...
    -EvidenceKind 'R3_SEALED_MODULE_IMMEDIATE_PRELOAD_VERIFICATION' `
    -TimeoutSeconds 60
exit $LASTEXITCODE
