...
$sourceTcl = Join-Path $taskRoot 'tools\r3_jtag_t0_session.tcl'
$selectorTcl = Join-Path $taskRoot 'tools\r3_select_jtag_target.tcl'
$sessionCsv = Join-Path $taskRoot 'raw\JTAG_T0_R3_LOCAL_SESSION.csv'
$targetProperties = Join-Path $taskRoot 'raw\JTAG_T0_R3_LOCAL_TARGET_PROPERTIES.tsv'
$deviceProperties = Join-Path $taskRoot 'raw\JTAG_T0_R3_LOCAL_DEVICE_PROPERTIES.tsv'
$vivadoLog = Join-Path $taskRoot 'raw\JTAG_T0_R3_LOCAL_VIVADO.log'
$vivadoJournal = Join-Path $taskRoot 'raw\JTAG_T0_R3_LOCAL_VIVADO.jou'
$controllerStdio = Join-Path $taskRoot 'raw\JTAG_T0_R3_LOCAL_CONTROLLER_STDIO.log'

foreach ($required in @($controllerReceipt, $linuxReceipt, $vivado, $sourceTcl, $selectorTcl)) {
...
& $vivado -mode batch -log $vivadoLog -journal $vivadoJournal -source $sourceTcl -tclargs 1 $sessionCsv $targetProperties $deviceProperties *>&1 |
...
Write-Output 'JTAG_T0_R3_LOCAL_GATE=PASS'
