...
$scriptPath = Join-Path $taskRoot 'tools\acquire_linux_lock.sh'
$evidencePath = Join-Path $taskRoot 'raw\LINUX_LOCK_ACQUIRE.log'
...
if (Test-Path -LiteralPath $evidencePath) { throw 'LINUX_LOCK_EVIDENCE_ALREADY_EXISTS' }
...
if ($lock.state -cne 'HELD' -or $lock.task -cne 'G2B-HW0-PRODUCT-R3' -or $lock.linux_lock_state -cne 'NOT_YET_ACQUIRED') {
    throw 'CONTROLLER_LOCK_STATE_INVALID_FOR_LINUX_LOCK'
}
...
& $helper `
    ...
    -EvidenceKind 'R3_LINUX_LOCK_ACQUIRE' `
    -TimeoutSeconds 45
...
if ($linuxReceipt.task_id -cne 'G2B-HW0-PRODUCT-R3' -or $linuxReceipt.lock_release_state -cne 'HELD') {
    throw 'LINUX_LOCK_RECEIPT_SEMANTICS_FAILED'
}
