...
$evidencePath = Join-Path $taskRoot 'raw\SEALED_MODULE_IMMEDIATE_PRELOAD_VERIFICATION_V2.log'
...
if (Test-Path -LiteralPath $evidencePath) { throw 'DRIVER_VERIFY_V2_EVIDENCE_ALREADY_EXISTS' }
...
if ($lock.state -cne 'HELD' -or $lock.task -cne 'G2B-HW0-PRODUCT-R3' -or $lock.linux_lock_state -cne 'HELD') {
    throw 'LOCK_STATE_INVALID_FOR_DRIVER_VERIFY_V2'
}
...
& $helper `
    ...
    -EvidenceKind 'R3_SEALED_MODULE_IMMEDIATE_PRELOAD_VERIFICATION_V2' `
    -TimeoutSeconds 60
exit $LASTEXITCODE
