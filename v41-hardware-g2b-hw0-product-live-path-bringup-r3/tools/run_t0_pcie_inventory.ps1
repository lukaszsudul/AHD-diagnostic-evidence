...
$scriptPath = Join-Path $taskRoot 'tools\t0_pcie_inventory_readonly.sh'
$evidencePath = Join-Path $taskRoot 'raw\T0_PCIE_AER_RUNTIME_INVENTORY.log'

foreach ($required in @($controllerReceipt, $helper, $plink, $scriptPath)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "T0_PRECONDITION_MISSING=$required" }
}
if (Test-Path -LiteralPath $evidencePath) { throw 'T0_PCIE_EVIDENCE_ALREADY_EXISTS' }
...
if ($lock.state -cne 'HELD' -or $lock.task -cne 'G2B-HW0-PRODUCT-R3' -or $lock.linux_lock_state -cne 'HELD') {
    throw 'LOCK_STATE_INVALID_FOR_T0_PCIE'
}
...
& $helper `
    ...
    -EvidenceKind 'R3_T0_PCIE_AER_RUNTIME_INVENTORY' `
    -SendPasswordToStdin `
