...
echo PHASE=PRELOCK_IDENTITY_EXCLUSIVITY_READONLY_V2
...
ps -eo pid=,ppid=,uid=,user=,stat=,comm= --sort=pid | awk -v p="$process_pattern" 'BEGIN {IGNORECASE=1} $6 ~ p {print}' || true
...
for node in /dev/xdma*; do
  ...
  if command -v fuser >/dev/null 2>&1; then fuser -v -- "$node" 2>&1 || true; fi
done
